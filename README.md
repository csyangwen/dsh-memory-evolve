...
- 🎨 **DSH UI 设置**：给 DSH web 界面加样式级小功能（**纯客户端注入**，不改框架源码、随 DSH 更新不掉）——第一版：**左侧会话列表「仅显示进行中」筛选**——默认只显示进行中的会话（正在生成/等审批/等回答/有子代理在跑/出错/已完成未查看），纯 idle 的折叠隐藏，一键切回全部；筛选条常驻列表顶部、状态本地记忆、实时跟随（会话开始跑自动出现、空闲自动隐藏）。后期扩展：主题更换等。**独立开关**（uiSettingsEnabled，默认关闭）
...
- **DSH UI 设置（dsh-ui-settings，默认禁用，独立子模块）**：给 DSH web 界面做**样式级小功能**的收容模块（**纯客户端注入**——CSS + DOM 增强，不改 DSH 框架源码、随 DSH 更新不掉；样式用 DSH 设计 token `--dsw-alias-*`，深浅色自动适配，后期主题更换可直接接管）。**第一版功能：左侧会话列表「仅显示进行中」筛选**——会话列表顶部出现「仅进行中 / 全部」分段筛选条（**默认只显示进行中的会话**，用户拍板语义：隐藏**纯 idle** 会话，保留 正在生成/等审批/等回答/有子代理在跑/出错/已完成未查看）：实现是纯 CSS 过滤规则（`html[data-dsh-ui-filter="on"] [role="tree"] div[role="treeitem"][aria-selected]:not(:has([data-state])) { display: none }`——会话行有稳定 ARIA 锚点，状态点 StateDot 带 `data-state` 属性，**纯 idle 行无状态点**；`:has()` 需 Chrome 105+）；React 状态变化重渲染后选择器**实时生效**（会话开始跑自动出现、空闲自动隐藏，无轮询）；搜索结果行是 `<button>` 天然排除、搜索时筛选让位；筛选偏好记在浏览器 localStorage（无记录默认开启）。注入的筛选条由 MutationObserver 保活（React 重渲染清掉后自动重注入，锚定 `[role="tree"]`）。模块自带「DSH UI 设置」Tab（**指南子 Tab** 与其他 Tab 同款，介绍模块功能）；设置 Tab「指南」页同步有模块说明行。**独立开关 `uiSettingsEnabled`**（默认关；开关在「Memory Evolve 设置」Tab 的「配置」里切换——宿主端提供状态端点 `/api/ui-settings/state`，模块关闭时端点 404，客户端探测失败即**不注入任何东西**；关闭时筛选条与样式全部移除、偏好保留）；
...
| `uiSettingsEnabled` | `false` | DSH UI 设置模块总开关（**独立子模块**）：左侧会话列表「仅显示进行中」筛选（纯客户端 CSS + DOM 注入，默认只显示进行中的会话、可一键切回全部）+「DSH UI 设置」Tab。默认禁用；关闭时筛选条与注入样式全部移除（宿主端状态端点随开关 404，客户端不注入任何东西），筛选偏好保留 |
