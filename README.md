...
- **入口**：会话页「记忆技能」Tab → 「技能管理」子 Tab（与「待确认记忆建议 / 待确认技能建议 / 运行时配置」并列，三栏布局：技能列表 + 目录树 + 文件查看/编辑器）。
...
会话页顶部的「记忆技能」标签页（`memoryTabEnabled` **默认开启**，仅可通过 config.yaml 配置——它不是运行时可改项，避免在 Tab 内关闭后无法找回）是插件的**唯一管理入口**（原设置面板的「记忆管理」已移除，功能全部并入本 Tab）。Tab 顶部是一行**功能 Tab + 文件页签**（竖线分隔），功能 Tab：
...
- 有未确认的记忆/技能时，会话页的「记忆技能」标签会显示 **🔴 小红点 + 计数**，提示你处理。
...
| `memoryTabEnabled` | `true` | 会话页「记忆技能」Tab 开关（插件的唯一管理入口：记忆文件预览/操作 + 待确认建议/技能 + 运行时配置）；**仅 config.yaml 可配置**（非运行时可改项，避免在 Tab 内关闭后无法找回）；默认开，变更后刷新页面生效 |
