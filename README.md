...
- **每回合实时记录**：快照内固定提示行要求模型**在完成用户请求后、输出最终回复前的最后一步**调用 memory 工具检查并写入项目记忆/每日日志——有实际产出（写文档/请假条/代码、创作、任务进展、踩坑）**必须记 1 条**，纯寒暄可跳过，另有"首写保险"（当天尚无日志则必写）；程序盖准确时间戳；可在设置面板分别关闭「每回合写入项目记忆 / 每日日志」；
...
- **缓存友好**：记忆快照走 user-role 尾部消息注入（变更检测），system prompt 与历史前缀保持稳定。**只注入低频变化的全局轨**（用户档案/全局事实）——项目记忆与每日日志随每回合主动写入而变化，若注入会导致每轮追加新的上下文快照、前缀缓存命中率下降，因此它们**默认不注入**，改为按需读取 + 快照中一行**固定提示**要求模型每回合调用 memory 工具检查写入（提示文本固定不变，不产生新快照）。
