...
装上插件后，打开任意会话，你会多出一排能力标签：**记忆 · 技能 · 待办 · 书签 · 提示词注入 · COI 调度 · 会话广播 · 临时信息**。AI 侧则多了一批工具：记忆读写、待办、技能管理、本地文件搜索、会话编排、会话广播、外部 AI 派单、提示词注入、模型查询……
...
1. **拉起团队**：对主会话的 AI 说「**创建 4 个会话：美工负责网站视觉、前端负责页面、后端负责接口、测试负责验收，把它们拉进同一个协作房间**」——它立刻创建 4 个标准会话（出现在左侧列表、随时可接管），自动继承你的模型与工作目录；
2. **协作沟通**：房间建好后，**谁改了什么、谁干完了，全员可见**；成员状态（正在生成 / 空闲）自动出现在「房间动态」里；
3. **派活与催活**：问 AI「**现在谁在干活、谁空闲了**」，它会查看各会话状态；对空闲的会话说「**叫前端把结果发过来**」，AI 自动唤醒它并派活——不会批量自动唤醒，一切由你（或你的主会话）有意识地指挥；
4. **防冲突**：同一项目多个会话并行改代码时，开工前对 AI 说「**声明一下你要改哪些文件**」——其他人（及其 AI）能看到「谁在改什么」；真撞车了，写入方会收到冲突警告，还有「工作区活动」随时显示谁在跑、在干什么；
...
4. 完成后结果摘要**自动写入项目日志与今日日志**——派出去的活自动变成你的记忆资产。
...
- **飞书通知**：对 AI 说「任务完成用飞书通知我」，重要结果推到飞书，电脑前/手机上立刻知道。
...
- **临时信息**：会话页一个持久化的 Markdown 便签，随手记、重启不丢；
- **会话搜索**：让 AI 搜索本机其他 AI 工具（如 Codex）的历史会话——「之前 Codex 里做过 XX」直接问；
- **模型配置**：插件侧维护各模型的启用状态、思考等级、备注，AI 选模型时一目了然；
