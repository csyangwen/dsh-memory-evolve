...
- **分层记忆（五轨）**：用户档案 · 全局事实 · 项目关键记忆（按工作目录隔离，**自动注入上下文**）· 项目日志（按工作目录隔离）· 每日日志，注入范围随层级收窄，互不污染；
- **每回合实时记录**：快照内固定提示行要求模型**在最终回复消息中先输出完整回复文本、再在文本之后附带工具调用**——每轮收尾向项目日志/每日日志**各写入 1 条**本回合进展，并**按重要性判断**是否向项目关键记忆（`key`）写入长期事实，最后调用 `memory_review_status` 检查审查是否到期（到达间隔时静默执行全局记忆建议与技能审查）；时间戳与每日日志的**项目标签**均由程序自动标注；可在设置面板分别关闭「每回合写入项目日志 / 每日日志 / 检查项目关键记忆」；
...
| 项目关键记忆（`key`） | 当前项目的长期事实（约定/决策/架构/踩坑） | **注入当前项目会话**（低频变化，缓存友好；按 cwd 隔离） | 每回合按重要性判断写入 + 记忆 Tab 手动添加 |
...
- `memory add target=key content=...`：写入当前项目的**关键长期记忆**（自动注入当前项目会话的上下文）；
...
- **运行时配置**：`reviewEnabled` / `reviewInterval` / `skillReviewEnabled` / `memoryTabEnabled` / `perTurnProjectWrites` / `perTurnDailyWrites` / `perTurnKeyWrites` 的表单修改，保存后**立即生效并持久化**（覆盖 config.yaml 对应项，重启不丢）；
...
开启「会话页记忆 Tab」（配置 `memoryTabEnabled`，默认关，开启后刷新页面生效）后，会话页顶部出现「记忆」标签页：直接预览 AGENTS.md 与全部记忆文件（只读预览——编辑请通过 memory 工具或用系统工具打开文件，避免破坏 § 分隔格式；**两个例外**：「项目关键记忆 KEY.md」页签提供输入框**手动添加**长期项目事实（保存后写入并在下一轮自动注入上下文）；美观视图的**每条记忆都带「删除」按钮**——点击后先弹出确认，确认后由宿主按**完整条目精确匹配**删除（非子串匹配，杜绝误删包含关系的长条目），删除不可恢复）。§ 分隔的结构化条目（项目日志/今日日志/全局记忆/项目关键记忆等）默认以**美观卡片视图**展示（时间徽标 + 内容 + 项目标签 + 删除按钮，支持关键词搜索过滤），可切换回纯文本视图；每行可一键用系统工具打开。
...
| `entryDatePrefix` | `true` | 记忆条目自动加时间前缀：全局轨与项目关键记忆 `[YYYY-MM-DD]`、项目日志 `[YYYY-MM-DD HH:MM]`、每日日志 `[HH:MM] [项目]`（项目标签由程序取自会话工作目录自动标注） |
...
| `memoryTabEnabled` | `false` | 会话页「记忆」Tab 开关：开启后会话页顶部可查看记忆文件（只读预览，项目关键记忆 KEY.md 支持输入框手动添加）；默认关，开启后刷新页面生效 |
