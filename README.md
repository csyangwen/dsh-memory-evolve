...
- 📌 **提示词管理器**：可复用的**指令范式资产库 + 注入执行器**——把常用工作范式（代码审查 / 调试 / PRD / 测试等）固化成提示词，**选中即注入：模型下一轮自动看到、不打断回复**（支持持续注入 / 间隔提醒，可随时停止）；内置 13 条 GitHub 真实提示词（SpecRoute / Claude-Code-Promts-Skills），分类可自定义；**默认关闭**，按需在「Memory Evolve 设置」Tab 的「配置」启用
