...
- **每回合实时记录**：快照内固定提示行要求模型**每个回合结束前调用 memory 工具**检查并写入项目记忆/每日日志——有实际产出（写文档/请假条/代码、创作、任务进展、踩坑）**必须记 1 条**，纯寒暄可跳过，另有"首写保险"（当天尚无日志则必写）；程序盖准确时间戳；可在设置面板分别关闭「每回合写入项目记忆 / 每日日志」；
