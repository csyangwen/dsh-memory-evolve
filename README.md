...
- **AI 工具 de_prompts**（随提示词管理器开关注册/注销，名字可配置 `promptToolName`）：AI 可 `list` 列出**启用中**的提示词（id/名称/简介/分类/标签，**不含正文**，支持 filter/limit）、`get <id>` 按 id 查**详情**（含正文全文与状态/统计；禁用提示词也可查）、`inject <id>` 把选中的提示词**注入当前会话**（写入注入轨模型下一轮生效；rounds=次数 0=无限、every=间隔 0=只注入一次，与 Web API 同规则；禁用/重复注入拒绝）。**正确用途**：给当前会话注入纪律/流程，或取正文用作**子会话/子代理/CLI 任务**的提示词（de_session / subagent / de_coi_dispatch）；
