...
为 DeepSeek Harness 带来「跨会话长期记忆 + 后台自我进化」能力的纯插件实现：**五轨记忆 · git 分支感知 · 回合内自我审查 · 技能自我进化与技能管理器 · 四轨待办 · COI 调度 · 会话广播 · 会话编排 · 会话搜索 · 提示词管理器 · 临时信息便签**——**零核心修改、零运行时依赖**，随装随用、卸载即净。
...
- **会话编排（de_session，默认禁用，独立子模块）**：让 AI **程序化创建/唤醒 DSH 会话**（"会话启动另一个会话"）——`spawn`：**新建标准会话**（与 GUI 手动打开完全同构：同样的系统提示词/工具/记忆快照/持久化，会出现在左侧会话列表、随时可接管），`prompt` = **完整提示词**（角色/任务自由组合的长文本，如"你是美工，负责…现在开始执行：…"），创建后立即自动开跑（等价替用户发消息）；**默认继承发起会话的 provider/model 与 cwd 工作目录**（保证同一项目内协作；可选 `roomId` 加入广播房间、`model`/`cwd` 显式覆盖）；`wake`：**唤醒已有会话**（`sessionId` + 提示词，等价替用户发消息，对方 AI 自动醒来处理、正在忙则排队；进程重启后自动 resume 再唤醒，**用会话自己的模型配置**——log 里的最后 provider/model，含 webUI 改过的）；`status`/`list`：查状态（running=正在生成 / idle=空闲 / offline=不在本进程，附 lastActiveAt）与 spawn 记录。**协作纪律**：不会自动唤醒任何会话——由拍板人（如产品经理）**有意识地** list/status 查状态、发现员工停止后主动 wake 派活（杜绝自动批量唤醒）；最终拍板人是人类用户。**独立开关 `sessionEnabled`** + 独立存储目录（`sessionDataDir`，默认 `<memoryDir>/session-orch`）；依赖 DSH agents 服务，**仅同进程会话可唤醒**（跨实例/跨机器无法程序化唤醒）；与广播松耦合（spawn 加房间经桥接，广播未启用只提示不阻断）；配合广播房间可实现"产品经理 AI 拉起美工/前端/后端/测试会话并逐个唤醒派活"的全自动协作；
...
- **spawn（新建会话）**：创建**标准 DSH 会话**——与 GUI 手动打开完全同构（同样的系统提示词/工具/记忆快照注入/持久化，会出现在左侧会话列表、随时可接管）。参数：
  ...
  - `cwd`（可选）：新会话工作目录（缺省=无即默认工作区）；
  - `roomId`（可选）：加入广播房间（需广播模块启用；房间不存在/广播未启用**只提示不阻断**创建——与广播是松耦合桥接）；
  - `model`（可选）：覆盖模型。
  - 创建后**立即自动开跑**（等价替用户发消息）；spawn 记录落盘（`<memoryDir>/session-orch/sessions.json`：谁建的/任务/房间/时间）。
- **wake（唤醒已有会话）**：`sessionId` + `prompt`（要对方做的事）——**等价替用户给对方发一条消息**，对方 AI 自动醒来处理（正在跑则排队，忙完前不要重复派活）；进程重启后的会话**自动 resume 再唤醒**；会话不存在/跨实例时明确报错。
