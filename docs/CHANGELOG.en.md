...
- **Memory-tab sub-tab row wrapped to two lines in narrow containers**: `.mt-file-tabs` (the memory/skills/todos/settings/models/sync/broadcast/COI/prompts/bookmarks/UI-settings sub-navigation) used `flex-wrap: wrap`, so with 11 sub-tabs in a narrowed container (e.g. the session column squeezed by the width handle or a split view) the row broke into two lines and doubled its height. Fix: single row with horizontal scroll (`flex-wrap: nowrap` + `overflow-x: auto` + `scrollbar-width: thin`), consistent with DSH conversation-view tabs; the TS source and the `lib/client.js` artifact are kept in sync (the block is inlined CSS text, byte-identical in both).
