...
- **广播投递即唤醒（de_broadcast send wake 参数）**：研究了「为什么广播有时能唤醒会话、有时不能」——根因在 DSH 核心的两个投递原语：广播投递一律用 `inject`（不唤醒），接收方 **running 时同回合可见（像被唤醒）**、**idle 时消息停在收件箱直到下次自然驱动（没动静）**、**offline 直接跳过**——三种状态三种观感。新增 `wake: true`（send 可选参数，默认 false 行为不变）：idle 接收方改走 `followup` 唤醒（投递到下一回合并启动驱动，等价替用户发消息，与 de_session wake / COI wakeOnComplete 同款机制；消息体带「发送方唤醒了你」标记让接收方明白回合来源）；running 接收方仍 inject（不打断）；offline 跳过（收件箱 + 回来补投兜底）。回执附「已唤醒 N 个空闲接收方」计数，发送方据此判断要不要改用 de_session wake 或等待。房间/项目伪接收者同样生效（逐成员判定状态）。系统通知（房间解散/踢人）与未读补投保持不唤醒。
...
- **记忆写入看门狗（长会话防遗忘）**：长会话中模型对「每轮收尾写 daily/project」的固定提示词逐渐失效（指令稀释），连续多轮不写记忆且程序侧毫无反馈，遗漏被静默吞掉。新增程序侧合规追踪：按会话统计「连续完成多少个用户回合未写任何 daily/project 记忆」（`writeGapCounter`，`agent/turn-stopping` 计数、subagent 不计、进程内存态），缺口达到阈值（`writeGuardThreshold`，默认 2，运行时可调）时快照注入置顶 ⚠️ 提醒，粘性直到下一次成功写入即消（memory 工具 add 成功自动归零；key 轨确认队列不算——看门狗只盯每轮进展日志）。提醒文案刻意静态（不嵌实时计数），一次欠账最多产生两条尾部快照（出现 + 消失），与「记忆审查已到期」提醒同款缓存代价。独立开关 `perTurnWriteGuard`（默认开）与阈值均已入 RUNTIME_KEYS / 校验 / 测试。
