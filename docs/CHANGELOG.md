...
## 2026-08-13 — web 站内通知：de_notify / de_channel_send 新增 web 渠道（右上角铃铛 + 弹窗 + 跳转会话）

### 需求（用户拍板）
通知除了飞书/QQ/微信/企微等 IM 渠道，还需要发到**本网页右上角站内通知铃铛**——「任务完成等需要用户知晓的节点」不依赖手机 IM 也能看到。

### 实现（`10edde9`）
- **存储层** `lib/notify-web.js`：web 通知落盘存储（六字段 `notifications.json` + 附件 + 原子写 + **30 天/500 条清理**）+ API（unread / list / read / readAll / delete / content / attachment）；
- **发送链路** `lib/notify.js`：`sendChannelNotify` / `sendChannelDirect` 加 web 内置分支（记录 sender），`channels` 枚举加 `web`；`notifyEnabled` 一个开关同时启用渠道通知 + web 通知，channelSend 与 notify 的 webStore 联动；
- **客户端** `notification-bell.tsx`：右上角全局铃铛 + 未读数字徽标 + 弹窗列表 + 全文查看 + 附件缩略图 + **跳转会话**（用 DSH client 全局 `ctx.sessions.open(sessionId)`，ui-workspace 同款路径）；探测 API 存在才注册，中英文案。

### 迭代打磨（同日上午 7 个提交）
- 铃铛下移 50px 避开顶部悬浮元素（`25b86e9`）→ 改 SVG 图标 + 再下移 35px + **移动端适配**（`2fa38f3`）；
- 通知详情**全文展示**——点开总是拉 /content（修复 200~8KB 中长通知截断，`a9caf26`）；
- 弹窗三处优化——标题颜色 / 会话名称显示 / 长文查看详情大弹窗（`8d45d1d`）；
- 通知列表交互重做——分行布局 + 已读按钮 + 标题跳转会话（`ae6e189`）；
- 铃铛 UI 重构（**grok 外援**）——邮件式去重 / 空行折叠 / 拖拽吸附 / 配色（`559eea0`）；
- README 补充 web 站内通知（场景八 + 能力清单，`c3df119`）。

---

## 2026-08-13 — 会话评审（Advisor）开关体系修复（用户反馈三轮）

### 背景
会话评审模块定稿后实机使用暴露开关语义问题，按用户反馈连续三轮修复：

- **advisorEnabled 改为模块总闸**（`7660b78`）：此前设置 Tab 关闭开关后模块功能仍启用（开关只管了部分能力）——改为总闸，关闭即整体停用；
- **会话级开关回归与刷新丢失**（`92d3f46`）：总开关开、会话关的场景下刷新页面后会话级开关被重置为开（override 未持久化、agent 重建后丢失）——修复为原子落盘持久化，agent/disposed、session/disposed 不再删除 override，重建后 Map 保留原值；
- **每个会话默认关闭（opt-in）**（`8d3184f`）：总闸开启后各会话评审**默认不启用**，需在悬浮面板手动开启——防止总闸一开所有会话被评审打扰；新增 211 行 opt-in 测试（advisor-optin.test.js），测试 667 全绿；
- README 补充「会话评审（Advisor）」完整场景与能力清单（`7040ecc`）。

---

## 2026-08-13 — 适配 DSH 0812 内测版服务名改名（workspace→workspaceRegistry、httpServer→webServer）

### 背景
DSH 0812 内测版对核心服务名做了纯改名（官方承诺行为不变）：`workspace` → `workspaceRegistry`、`httpServer` → `webServer`（host/webserver 提供 `register({kind,path,handler})`，API 形状不变）。旧名引用会导致路由静默未挂载（症状：API 返回 SPA 回退 HTML、前端列表空白）。

### 实现（`5308222` + `d3d9b64`）
- 全库 **15 个文件**改名适配（lib/api.js / bookmarks / coi / broadcast-api / ws-coord / prompts / session-orch / skills-manager / ui-settings / mermaid / advisor / index 等），覆盖所有 ctx 变量前缀（ctx./webCtx./skillCtx. 等），避免只扫 ctx. 漏改；
- 测试夹具服务名同步 0812（httpServer→webServer、workspace→workspaceRegistry），测试全绿。

---

## 2026-08-13 — COI 任务列表加分页（GUI 任务子 Tab）

### 实现（`02aa03c`）
任务多了之后 GUI 任务列表一次性渲染全部记录撑爆页面——加分页：
- API `/coi/tasks` 支持 `page`/`pageSize` 分页（传了才走分页返回 `tasks` + `total`；只传 limit 保持旧 list 语义向后兼容）；
- `tasks-store` 新增 `listPaged` 分页查询；
- CoIView 任务子 Tab 分页 UI + 样式 + 测试（+84 行断言）。

---

## 2026-08-12 — 会话评审（Advisor）模块定稿（2026-08-12~13 多轮用户拍板）

### 功能（`f11c2dd`，测试 665 全绿）
- **每会话挂独立评审员**：观察**可见表面**（用户输入 + Agent 回复，不含思考/工具），每轮实时评审，steer/inject 实时送达（**info / nit / concern / blocker 四级**，info 默认仅记录不注入，可配置注入）；
- **评审员持续会话**：完整上下文全量重放永不截断（像普通 LLM 长对话），每轮只注入一次本轮消息（`### Session update` 段唯一出现），评审收尾才提交进持续会话，覆盖成功/中止/失败全部路径；
- **注入伪装成用户指令**（2026-08-13 设计反转）：note 命令式用户口吻、无身份痕迹（实测带非用户指令标记致 Agent 质疑来源、执行力双降）；机器层 `source.kind=advisor` 保留（自评审排除）；GUI 折叠行 `[severity]` 前缀给用户识别；
- **三角色消息成对标签包裹**：`<用户对Agent说>` / `<Agent对用户说>` / `<用户对评审员指令>` / `<用户对评审员提问>`（内容与标记无歧义）；
- **约束五层**：系统提示词 / 全局 / 项目（cwd 键控）/ 会话 / 评审会话，越局部越优先，持久化于 advisor 数据目录，面板约束 Tab 编辑；
- **面板四 Tab**：约束 / 实时 / 记录 / 设置；新建评审会话（清空评审员上下文）、指令即时问答（回答仅面板展示不注入主会话）、评审员上下文 ≈K 显示（<1K 显示字数）、恢复默认提示词一键生效；
- **评审员默认提示词**：独立会话信息边界、指令响应规则、JSON 帧输出契约、保守原则（Nothing to add 宁缺毋滥）；
- **健壮性**：指令队列 bind/reserve/consume/release（问答绑定防抢、崩溃恢复）、records.jsonl 终态记录、`/advisor` 命令、HTTP API；
- docs-local 新增实现规则定稿文档（2026-08-13-memory-evolve-advisor-实现规则定稿.md）。

### 测试反馈修复（`820649f`，2026-08-13 四连，测试 667 全绿）
- 实时流清空 / 悬浮窗默认隐藏 / tool-call 按序显示 / zh 指南文案补齐（中文界面不再显示原始 key）。

---

## 2026-08-12 — 版本检测与更新模块（一期）

### 需求（用户拍板）
插件本体要能检测新版本并提示更新——用户装的还是旧版本，不知道有新功能。

### 实现（`6ff4afc` + `ca8c26f`）
- `lib/update.js`：**git tag 检测**新版本（对比当前 tag 与远端 tag）+ 手动更新（拉取 + checkout）；版本号逻辑修复（`ca8c26f`）；
- 设置 Tab 新增**版本页**（VersionTabView）：当前版本 / 最新版本 / 更新按钮 + **红点提醒**（有新版时设置 Tab 显示红点）；
- README 与配置同步；测试 892 行（update.test.js）。

---

## 2026-08-12 — COI 任务完成唤醒增强（wakeOnComplete + 移除 3 次限制 + 输出日志路径）

### wakeOnComplete（`9edb4c6`，0811 官方同款机制）
- `de_coi_dispatch` 新增 **`wakeOnComplete`** 参数：任务完成自动唤醒派单会话（空闲时开新回合投递完成摘要，忙碌时摘要注入下一步）——与 de_session wake 同语义，**无次数限制**；
- **防自激**：每会话连续唤醒上限 3 次（WAKE_BUDGET），超限降级 inject；预算重置对齐官方语义——监听 `agent/inbox/claimed`，用户输入 claimed 重置、插件 notice 不重置（旧运行时无事件则降级为永久上限）；
- 工具 description 更新：用户明确要求唤醒时才传 true（此前严禁承诺自动处理，wakeOnComplete=true 时例外）。

### 移除完成唤醒 3 次限制（`f3902c0`，2026-08-12 用户拍板）
用户拍板：**用户显式要求的唤醒每次生效**，不该有次数上限——移除 WAKE_BUDGET，与 de_session wake 同语义；测试改为「连续 5 次 idle 完成全部 followup（无上限）」。

### 输出日志路径（`1e8aa20`）
- `de_coi_status` / `de_coi_wait` 输出**开头给出完整日志文件路径**（第一行）——省去 AI 自行搜索日志的弯路；运行中无文件时提示「暂无输出文件」；
- 工具 description 同步：明确输出为尾部截断、路径在输出第一行、用 read 读；新增运行中/完成后两个分支测试。

---

## 2026-08-11 — 记忆同步 Tab 重排与全局轨链路补全（同日傍晚多轮迭代）

### 背景
CRLF 修复后当天傍晚又按用户拍板与反馈连续迭代多轮（18:08~20:16 九个提交），全部围绕记忆同步 Tab 交互与全局轨链路：

- **Tab 三子 Tab 化**（`547cac3`）：项目 / 全局 / 记忆远端三区彻底分离；二轮重排（`be5eae8`）合并开关 + A/B 单选切换 + 样式美化；按拍板重排（`3470904`）三分区 + 统一推拉按钮 + 只显示一个仓库；布局重构 + 服务端文案去指令化（`1817b9c`）；切换共享记忆仓库入口缺失修复（`f0ad90c`）；
- **「记忆远端」改「共享记忆库」** + 设备级启用/停用开关（`17647b3`，2026-08-11 用户拍板）；细节打磨（`5a7b49d`）删测试警示 + 按钮简化 + 修改并保存；
- **三态切换取代旧轨开关**（`f73d214`）：修复 tracks.project 遗留位卡死拉取/推送（轨位自愈成功 / off 联动 / setup 复位，499/499 全绿）；
- **状态文案「未提交」改「未推送」** + ahead 统计（`f80fc9f`）——拉取合并/推送语义一目了然；
- **补全全局轨冲突解决链路**（`b245120`）：ops.conflicts/resolve 支持全局轨 fileset、API /resolve 透传、前端全局子 Tab 冲突区——全局推送被冲突拦截后可直接解决，解决提交落本轨分支、解决后推送闭环（500/500 全绿）；
- 会话页顶部 Tab 顺序重排与命名调整（`c0ebc76`）；施工图 v1.2 修订（`97178bf`）；上线前三方终审修复（`d7f170a`，Grok/Codex/Kimi 交叉审查）。
...
### 背景
用户拍板：不可能每个项目建一个私有记忆仓库——模式 B 的完整形态是一个私有仓库装**所有**项目的记忆（"用户应该只建一个就够了"）。同时确认默认模式**保持模式 A**（隐私是 A/B 主要区别，UI 补模式说明引导）。

### 核心能力
- **共享记忆仓库**：模式 B 的私有仓库可被所有项目共用，每个项目自动使用专属分支 `dsh-shared/<项目身份>`（12 hex 稳定身份，跨设备一致、自动命名、互不干扰）。本地独立仓库与同步协议**零改动**（remoteBranch 早已参数化），只新增 setup 时的分支决策；
- **分支决策判定树**（worker 子进程 `decide` 子命令——网络命令不进主进程）：共享分支存在→续接；空仓库→fresh 用专属分支；main 存在且 PROVENANCE.projectId 精确匹配→**老单项目仓库（一期 main 分支）零迁移兼容**，继续用 main；main 属于其他项目/归属不明→本项目用专属分支，**绝不触碰 main**（串项目防护三层防线：决策只认 projectId + deviceBConnect expectedProjectId 校验 + worker checkProvenance）；
- **首次推送缺口修复**（一期遗留，模式 A/B 都有）：bootstrap 后第一次 `sync --push` 会因 fetch 不到远端分支而报错——现在「远端无此分支」识别为首次推送场景（远端视为空、单父提交），由 push 创建分支；
- **UI 模式引导**（用户拍板）：状态卡片新增「当前模式」（A/B）；操作区新增可折叠「模式说明：A / B 怎么选」——两种模式含义 + 推荐场景 + **隐私提醒**（模式 A 下公开仓库 = 记忆公开）；
- 顺带修复：模式 B setup 接入补传 `expectedProjectId`（一期漏传，共享仓库多项目并存下是硬防线）；模式 B setup 成功自动开模块开关（与模式 A 对齐）。

### 测试与验证
- 全量 **475/475 全绿**（新增 tests/sync-shared-branch.test.js 10 用例：命名空间 1 / 决策树 6 / e2e 3 场景含双设备认亲、老仓库兼容、串项目防护）；同步测试总数 88；
- 决策树逐分支验证：空仓库 fresh / 已有共享分支 shared / main 属于本项目 legacy / main 属于别人 shared-fresh / main 无 PROVENANCE 保守 shared-fresh / 远端不存在分类报错；
- e2e：A 初始化推送专属分支（main 不产生）→ B 认亲接入拿到记忆；老单项目 main 双设备兼容；别人的 main 在 A 推送后原封不动。

---

## 2026-08-11 — 记忆同步（跨设备项目记忆共享）一期交付

### 背景
用户拍板立项「项目记忆跨设备共享」（调研 v13 + 实现方案 v1.0 施工图，docs-local/），一期按施工图九步落地：身份解析 → 条目身份证 → 记忆仓库初始化 → 三路合并器 → sync-worker → 命令与状态面 → 冲突兜底 → 双设备端到端集成测试。外援交叉审查（grok/codex）发现 2 条 P0 + 17 条 P1 全部修复并补回归测试。重启实测后按用户反馈补齐 GUI 交互（记忆同步 Tab + 三层开关）。

### 核心能力
- **三层启用开关**（用户拍板 2026-08-11）：模块开关（设置面板「记忆同步」——只是功能可见）→ 项目开关（「记忆同步」Tab「本项目同步」，**逐项目 opt-in 默认关**——未打开的项目保持未安装本模块前的纯本地状态，不建 Git 仓库、不生成身份证）→ 轨开关（「同步范围：项目记忆轨」；全局轨跨设备同步为二期）；
- **两种远端模式**：模式 A（默认零配置，复用主代码仓库 remote，记忆存专属分支 `dsh-shared/memory`）/ 模式 B（`/memory_sync setup <url>` 指定私有记忆仓库，远端分支 main）；
- **同步机制**：写记忆实时落盘完全不碰 Git（攒一批合一次）；fetch 锁外 → 内存三方合并（共同祖先/本机/远端：各自新增保留、单侧修改自动采用、双侧改同一条进人工）→ 原子写回 → commit-tree 双父提交；**Git 冲突符号永不落盘**；
- **条目身份证**（`[id:xxxx]`，展示层全剥离）：add 随机 ID、replace 继承（替换不换身份）、老记忆确定性补发（sha1(内容归一化) 前 8 位——双设备同内容同 ID 才能对齐合并锚点）；
- **GUI「记忆同步」Tab**（用户拍板：模块 UI 化）：状态卡片（身份/分支/未提交/落后/冲突/迁移提示）+ 初始化（模式 A/B）+ 同步/同步并推送（点击即显式同意推送）+ 冲突逐条解决（ours/theirs/both）+ 停用；命令 `/memory_sync` 族为补充通道；
- **健壮性**：PROVENANCE 身份校验防接错仓库；allowlist 白名单入库（TODOS.md 等外部文件永不进 Git）；锁带 pid 存活检测 + fetch ref 残留清理（断电中断自动恢复）；non-fast-forward 绝不 force push。

### 测试与验证
- 全量 **465/465 全绿**（新增 sync 测试 5 个文件 78 用例：identity 13 / entryid 10 / repo 6 / merge 15 / worker 7 / index 11 / conflict 5 / e2e 4 / review-regression 8 / sync-api 5，另装配冒烟）；
- 双设备端到端：接入 → 双向并集 → 单侧修改采用 → 双侧冲突 → resolve → 收敛一致；归档生效；真实 kill 中断恢复；
- 审查闭环：grok/codex 对前 3 步深度审查（2 P0 + 17 P1）全部修复（双 ID、迁移丢数据、Tab 精确操作失效、非 canonical 补发破坏、adopt 覆盖、白名单泄漏等）并补 8 个回归用例；
- 重启实测反馈修复：命令组始终注册（开关关闭也可用）、设置面板 syncEnabled 字段、记忆同步 Tab、三层开关拆分、模块开关命名修正（去「项目」二字）。

### 文档
- 新增 [docs/记忆同步.md](记忆同步.md)（对外模块文档）；README.md 场景指南新增「多台电脑共享项目记忆」场景与引用；README-详细说明.md 新增特性条目与「记忆同步」章节。
...
- **活动快照段重复注入**（渲染时刻时间戳 → 状态驱动时间）：快照投影按【整体文本】diff，活动段文本曾含 `${Date.now()}`（渲染时刻秒级时间戳）→ 每次渲染必变 → 回合进行中每步重复注入（用户实测 10 秒一条刷屏）。修复：时间用**状态的一部分**（活跃起始时刻 HH:MM，只在成员/锁变化时更新）——同状态连续渲染文本完全相同、只注入一次；秒级实时时间锚点由 de_ws_status 工具输出提供；
- **活动段补"开工前声明"纪律**（AI 主动性的入口）：文案从"动共享文件前先 de_ws_status 查询"扩为"开工前先用 de_ws_declare 声明 + 动手前 de_ws_status 查询"；
- **会话删除立即清理锁**（agent/disposed 监听）：曾实测已归档会话的锁还占着（活动段显示它、冲突提醒指向不存在的会话）——锁表持久化且 TTL 前不消失；修复=监听 agent/disposed 清该会话全部锁+会话记录；idle（停止回合）保留锁不变（先后写入保护）；
- **活动段/通知显示完整会话 ID**：截短 ID（session-58ae…）无法供 AI 调 de_session status 确认对方是否真的在运行——改为完整 ID（有别名=别名（完整ID）），快照活动段/冲突提醒/占用方通知全部一致。
...
## 2026-08-09 — ⚡ 立即注入：快照变更 + 插话，当前回合立即生效（只注入一次）
...
## 2026-08-08 — de_prompts 新增 create/update：模型可自行创建/修改提示词
...
## 2026-08-08 — de_prompts list 多维过滤：名称/场景/备注/分类 + 未命中明确提示
...
## 2026-08-08 — 提示词新增「简介」与「启用状态」字段 + de_prompts AI 工具（列表/详情/注入）
