...
- **已知边界**：仓库粒度共享（monorepo 内多子项目共享一份）；一期老分支（`dsh-shared/memory` / `main`）自动兼容；全局记忆同步为二期且仅共享记忆仓库可用；`file://` 等本地路径远端不共享（自动回退纯本地）；不填地址时公开仓库 = 记忆公开（填共享记忆仓库可隔离）；主仓库换 remote → 身份变化 → 旧目录成孤儿（`migrate` 可见，自动并入二期）；无 git/过旧 git 自动降级并有指引。
