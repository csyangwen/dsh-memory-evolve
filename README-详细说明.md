...
- **记忆同步（跨设备项目记忆共享，默认禁用，独立子模块）**：项目记忆（项目关键记忆 KEY + 项目日志 + 关键记忆归档）经 Git 与远端对账，实现**多台电脑共享**。**三层开关**（2026-08-11 用户拍板）：模块开关（设置面板「记忆同步」——只是功能可见）/ 项目开关（「记忆同步」Tab「本项目同步」——**逐项目 opt-in，默认关**，未打开的项目保持纯本地状态、不建仓库不生成身份证）/ 轨开关（「同步范围：项目记忆轨」——项目记忆轨是否参与；全局轨跨设备同步为二期）。**两种远端模式**：模式 A（默认零配置，复用主代码仓库 remote，记忆存专属分支 `dsh-shared/memory`，不污染代码 log）/ 模式 B（**共享记忆仓库**：一个私有仓库装所有项目的记忆，每项目专属分支 `dsh-shared/<项目身份>`；老单项目 main 分支自动兼容）。**同步机制**：写记忆照常实时落盘完全不碰 Git（攒一批合一次）；fetch 锁外 → 内存三方合并（共同祖先/本机/远端，各自新增保留、单侧修改自动采用、双侧改同一条才进人工）→ 原子写回 → commit-tree 双父提交；**Git 冲突符号永不落盘**；条目身份证 `[id:xxxx]`（展示层不可见）为跨设备合并锚点，老记忆启用时按内容**确定性补发**（双设备同内容同 ID 才能对齐）；PROVENANCE 身份校验防接错仓库；push 永远需用户显式触发。**GUI**：「记忆同步」Tab——状态卡片（身份/分支/未提交/落后/冲突/迁移提示）+ 初始化（模式 A/B）+ 同步/同步并推送 + 冲突逐条解决（ours/theirs/both）+ 停用；命令 `/memory_sync setup|sync|sync --push|status|conflict list|conflict resolve|off|migrate` 为补充通道。断电中断自动恢复（锁残留/ref 残留清理）。详见 [docs/记忆同步.md](docs/记忆同步.md)；
...
项目记忆（项目关键记忆 KEY、项目日志、关键记忆归档）在多台电脑之间共享——办公室改的结论，回家打开同一项目，记忆已经在。**默认全关**：逐项目启用，未启用的项目行为与未安装本模块时一致。**详细文档：[docs/记忆同步.md](docs/记忆同步.md)**（含三层开关、两种模式、安全保证与边界）。
...
- **三层开关**：
  ...
  3. **轨开关**：「同步范围：项目记忆轨」（默认开）——项目记忆轨（KEY/日志/归档）是否参与。项目记忆只是记忆模块的一部分，全局轨（全局记忆/用户档案/每日日志）的跨设备同步为二期，届时出现各自独立的轨开关。
- **两种远端模式**（**隐私是主要区别**，Tab「操作」区有可折叠的模式说明）：
  - **模式 A（默认，零配置）**：远端自动复用主代码仓库的 remote 地址，记忆存放在代码仓库的**专属分支 `dsh-shared/memory`**（独立历史、不污染代码 log、不影响代码分支）；**代码仓库公开 = 记忆也公开**，介意就用模式 B；
  - **模式 B（共享记忆仓库）**：`/memory_sync setup <url>` 或 Tab「初始化共享记忆仓库」指定一个**私有记忆仓库**——**一个仓库装所有项目的记忆**，每个项目自动使用专属分支 `dsh-shared/<项目身份>`（跨设备稳定、互不干扰），记忆与代码彻底隔离；一期老单项目仓库（分支固定 main）自动识别、继续用 main，零迁移兼容。代码公开但记忆想私藏、或想用一个专属仓库统一管理记忆远端时用 B。
- **同步机制**：
  - 写记忆照常**实时落盘、完全不碰 Git**——同步是"攒一批合一次"，零写入延迟；
  - 每次同步 = 拉取远端 → **内存三方合并**（共同祖先 / 本机 / 远端）：各自新增都保留、单侧修改自动采用、双侧都改了同一条才进人工；**Git 冲突符号永远不会出现在记忆文件里**；
  - **条目身份证**（`[id:xxxx]`，展示层不可见）是跨设备合并锚点：老记忆启用同步时按内容**确定性补发**（双设备同内容 → 同 ID），合并器才能把"同一条老记忆"对齐；
...
- **GUI（「记忆同步」Tab）**：状态卡片（项目身份/当前模式/远端分支/未提交 N 条/落后远端/冲突 K 条/待迁移旧目录提示）；操作按钮（初始化模式 A/B、同步、同步并推送——**点击即你显式同意推送**、停用本项目）；冲突列表逐条解决（共同版本/本机/远端 + 采用本机/采用远端/两者都要，解决后自动提交）。
...
- **已知边界**：仓库粒度共享（monorepo 内多子项目共享一份）；模式 B 共享仓库每项目一条分支（老单项目 main 分支自动兼容）；`file://` 等本地路径远端不共享（自动回退纯本地）；公开仓库 + 模式 A = 记忆公开（可改模式 B）；模式 A 主仓库换 remote → 身份变化 → 旧目录成孤儿（`migrate` 可见，自动并入二期）；无 git/过旧 git 自动降级并有指引。
