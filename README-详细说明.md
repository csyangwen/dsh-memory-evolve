...
- **两种远端模式**：
  - **模式 A（默认，零配置）**：远端自动复用主代码仓库的 remote 地址，记忆存放在代码仓库的**专属分支 `dsh-shared/memory`**（独立历史、不污染代码 log、不影响代码分支）；
  - **模式 B（可选）**：`/memory_sync setup <url>` 或 Tab「初始化私有仓库」指定**私有记忆仓库**（远端分支 main）——代码公开但记忆想私藏时用。
- **同步机制**：
...
- **GUI（「记忆同步」Tab）**：状态卡片（项目身份/远端分支/未提交 N 条/落后远端/冲突 K 条/待迁移旧目录提示）；操作按钮（初始化模式 A/B、同步、同步并推送——**点击即你显式同意推送**、停用本项目）；冲突列表逐条解决（共同版本/本机/远端 + 采用本机/采用远端/两者都要，解决后自动提交）。
...
- **已知边界**：仓库粒度共享（monorepo 内多子项目共享一份）；`file://` 等本地路径远端不共享（自动回退纯本地）；公开仓库 + 模式 A = 记忆公开（可改模式 B）；模式 A 主仓库换 remote → 身份变化 → 旧目录成孤儿（`migrate` 可见，自动并入二期）；无 git/过旧 git 自动降级并有指引。
