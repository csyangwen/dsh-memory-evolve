    ...
    if (viewerSession !== undefined || viewerCwd !== undefined) {
      items = items.filter((s) => {
        switch (s.scope) {
          case 'temporary':
          case 'session':
            return typeof viewerSession === 'string' && viewerSession.trim() !== '' && s.ownerSessionId === viewerSession
          case 'project':
            // 发起者工作区（ownerCwd）内的会话可见；旧记录无 ownerCwd →
            // 回退按 cwd 匹配（兼容历史数据，但绝不跨工作区泄漏）
            return typeof viewerCwd === 'string' && viewerCwd.trim() !== ''
              && (s.ownerCwd != null ? s.ownerCwd === viewerCwd : s.cwd === viewerCwd)
          default:
            return true
        }
      })
