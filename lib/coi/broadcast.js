...
export class BroadcastStore {
   ...
   * 长内容文件（broadcasts/<id>.txt）+ 图片附件文件（attachments/，
   * 消息 JSON 只存元数据，文件必须随消息生命周期走——避免孤儿文件）。
      ...
      render: (_args, value) => {
        ...
        if (value.messages !== undefined) {
          const lines = value.messages.map((m) => {
            ...
            // 附件提示行（图片只提示数量/文件名——快照与列表保持纯文本，
            // AI 要图内容走 read 拿文件路径，GUI 收件箱才渲染缩略图）
            const attLine = Array.isArray(m.attachments) && m.attachments.length > 0
              ? `\n📎 图片附件 ${m.attachments.length} 张（${m.attachments.map((a) => `${a.name} ${(a.size / 1024).toFixed(0)}KB`).join('、')}；read 拿文件路径）`
              : ''
