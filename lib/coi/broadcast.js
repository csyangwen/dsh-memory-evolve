...
export class RoomStore {
  ...
  /** 我所在的**活动**房间列表（按创建时间倒序；已解散的不再列出——成员已收到系统通知）。 */
  list(sessionId) {
    if (!sessionId) return []
    return Object.values(this.rooms)
      .filter((r) => r.members.includes(sessionId) && r.status !== 'dissolved')
...
export function messageToolDefinition(broadcast, presence, memoryDir) {
  return {
    name: 'de_broadcast',
    description: '会话广播：DSH 会话之间传递消息（独立模块，开关见记忆 Tab 运行时配置「会话广播」）。send：给其他会话发消息，recipients 传**接收方会话 ID 数组**（用户会告诉你对方的会话 ID；支持同时发给多个会话），content 为消息内容（超长自动写文件），subject 为主题（可选，缺省取内容首行）；**伪接收者（仅用户明确要求时用，默认一对一不要擅自扩大发送范围）**：recipients 可混入 room:<群id>（房间内所有成员可见——聊天室，跨工作目录，成员用 room-create/join/leave 管理；发送者须是成员）与 project:<绝对路径>（该目录内所有会话可见）。list：列出当前会话的消息（**收件箱式：每条只显示主题+简短简介，像邮件列表**；可选 **type 参数**：unread=只显示未读（**缺省**，收件箱视角省上下文）、all=全部（含已读的房间/项目消息与历史）、read=只看已读回看；显式接收者消息 read 后自动删除，房间/项目消息保留 30 天供回看）；read：查看消息全文并标记已读（快照「会话广播」提示随之消失；**批量：ids 传多个消息 id 数组一次读完**，返回全部全文，不可见/不存在自动跳过——AI 清空收件箱不必逐个调用）；delete：删除消息（发送方或可见者）。房间管理：room-create（name 可选，创建者自动入房）/ room-join（拿房间 id 加入）/ room-leave（退出）/ room-list（我所在的房间）/ room-rm（解散房间，仅创建者——向全体成员发系统通知）/ room-kick（踢出成员，仅创建者——向被踢者发系统通知）。**presence（在线状态查询）**：传 roomId 列出房间成员谁在线（running=正在生成可等它/发消息它回合内可见）、谁已结束回合（idle=等用户驱动，相当于离线，**不要傻等**）；传 sessionId 查单个会话；返回 lastActiveAt 供判断多久没动。**消息只对接收方/房间成员/项目内会话可见（定点注入提示）**，其他会话无感知。',
    parameters: {
      type: 'object',
      properties: {
