...
export function installCoiApi(ctx, svc) {
  const handler = async (req, res) => {
    ...
    // API 请求没有 DSH Agent invocation 可直接读取；GUI 用 query/body
    // 携带当前 DSH 会话 id。缺少身份时保留 null 哨兵，私有/项目任务不会
    // 因为过滤条件缺失而放宽为全量可见。
    const viewerFor = (body = {}, allowBodySessionId = false) => {
      const rawId = body?.dsSessionId !== undefined
        ? body.dsSessionId
        : allowBodySessionId && body?.sessionId !== undefined
          ? body.sessionId
          : url.searchParams.get('sessionId')
      const ownerSessionId = rawId === null || rawId === undefined || String(rawId).trim() === ''
        ? null
        : String(rawId).trim()
      const resolvedCwd = ownerSessionId === null ? null : svc.resolveCwd?.(ownerSessionId) ?? null
      return {
        ownerSessionId,
        sessionCwd: resolvedCwd === null || String(resolvedCwd).trim() === '' ? null : resolvedCwd,
      }
    }
    const taskAccessible = (taskId, viewer) => {
      const task = svc.tasks.get(taskId)
      return task !== undefined && svc.tasks.canAccess(task, viewer)
    }
    const hiddenTask = (taskId) => sendJson(res, 404, {
      ok: false,
      message: cap2('coi2.taskInaccessible', { id: taskId }),
    })
    const visibleSessions = (filter, viewer) => {
      return svc.sessions.list({
        ...filter,
        ownerSessionId: viewer.ownerSessionId,
        sessionCwd: viewer.sessionCwd,
      })
    }
    const sessionAccessible = (sessionId, viewer) => visibleSessions({}, viewer).some((session) => session.id === sessionId)
    try {
      ...
      if (req.method === 'POST' && path === `${base}/tasks`) {
        ...
        const cwd = body.cwd ?? viewer.sessionCwd
        // 图片附件：先异步解析（path/url/attachmentId 三来源），任一附件
        // 非法即 400 拒绝（不创建半成品任务）
        const resolved = await svc.scheduler.resolveAttachments(body.attachments, viewer.ownerSessionId ?? undefined)
        if (!resolved.ok) return sendJson(res, 400, resolved)
        if (body.refTaskId && !taskAccessible(body.refTaskId, viewer)) return hiddenTask(body.refTaskId)
        const result = svc.scheduler.dispatch({
          ...
          ownerSessionId: viewer.ownerSessionId,
          ownerCwd: viewer.sessionCwd, // 发起会话工作目录（项目层级可见性）
      ...
      if (req.method === 'GET' && segments.length === 5 && segments[2] === 'coi' && segments[3] === 'tasks') {
        ...
        if (!taskAccessible(taskId, viewerFor())) return hiddenTask(taskId)
      ...
      if (req.method === 'POST' && segments.length === 6 && segments[2] === 'coi' && segments[3] === 'tasks' && segments[5] === 'cancel') {
        const taskId = decodeURIComponent(segments[4])
        const body = await readBody(req)
        if (!taskAccessible(taskId, viewerFor(body, true))) return hiddenTask(taskId)
      ...
      if (req.method === 'POST' && path === `${base}/sessions/note`) {
        const body = await readBody(req)
        if (!sessionAccessible(body.id, viewerFor(body, true))) {
          return sendJson(res, 404, { ok: false, message: cap2('coi2.sessionInaccessible', { id: body.id }) })
        }
      ...
      if (req.method === 'DELETE' && segments.length === 5 && segments[2] === 'coi' && segments[3] === 'sessions') {
        ...
        if (!sessionAccessible(id, viewerFor())) {
          return sendJson(res, 404, { ok: false, message: cap2('coi2.sessionInaccessible', { id }) })
        }
      ...
      if (req.method === 'POST' && path === `${base}/export`) {
        const body = await readBody(req)
        if (!sessionAccessible(body.sessionId, viewerFor(body))) {
          return sendJson(res, 404, { ok: false, message: cap2('coi2.sessionInaccessible', { id: body.sessionId }) })
        }
