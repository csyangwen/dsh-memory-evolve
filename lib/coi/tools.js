...
- **预计较长**（kimi/codex/zcode 等）→ 先 de_coi_wait 一小段（如 30-60s）确认任务正常启动（拿到会话/状态），再决定：继续 wait（单次最长 10 分钟，可多次调用）或结束回合——**若结束回合，必须如实告诉用户**「任务在后台运行，发任意消息（如"看结果"）我就能看到完成摘要并整理」，**严禁承诺"完成后会自动处理/自动整理给你"**。`,
    parameters: {
      type: 'object',
      properties: {
