...
/** 当前工具调用的任务查看视角；无 agent 时用 null，不能放宽私有层级。 */
function taskViewer(exec) {
...
/** 任务详情/日志/控制共用同一层级可见性判断。 */
function canAccessTask(scheduler, taskId, exec) {
  const task = scheduler.tasks.get(taskId)
  return task !== undefined && scheduler.tasks.canAccess(task, taskViewer(exec))
}

function inaccessibleTask(taskId) {
  return { ok: false, message: ctl2('coi2.taskInaccessible', { id: taskId }) }
}
...
- **预计较长**（kimi/codex/zcode 等）→ 先 de_coi_wait 一小段（如 30-60s）确认任务正常启动（拿到会话/状态），再决定：继续 wait（单次最长 10 分钟，可多次调用）或结束回合——**若结束回合且用户希望完成后自动处理，可带 wakeOnComplete=true 派单**：任务完成时会自动唤醒本会话开新回合投递完成摘要（空闲时），AI 醒来直接处理结果、无需用户发消息；用户没要求唤醒时维持原状（如实告诉用户「任务在后台运行，发任意消息（如"看结果"）我就能看到完成摘要并整理」，**严禁承诺"完成后会自动处理/自动整理给你"**——除非本次派单已传 wakeOnComplete=true）。`,
    ...
    async execute(args, exec) {
      ...
      if (args.refTaskId && !canAccessTask(scheduler, args.refTaskId, exec)) return inaccessibleTask(args.refTaskId)
  ...
  const status = {
    ...
    execute: (args, exec) => {
      if (!canAccessTask(scheduler, args.taskId, exec)) return inaccessibleTask(args.taskId)
