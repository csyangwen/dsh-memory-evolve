...
- **预计较长**（kimi/codex/zcode 等）→ 先 de_coi_wait 一小段（如 30-60s）确认任务正常启动（拿到会话/状态），再决定：继续 wait（单次最长 10 分钟，可多次调用）或结束回合——**若结束回合且用户希望完成后自动处理，可带 wakeOnComplete=true 派单**：任务完成时会自动唤醒本会话开新回合投递完成摘要（空闲时），AI 醒来直接处理结果、无需用户发消息；用户没要求唤醒时维持原状（如实告诉用户「任务在后台运行，发任意消息（如"看结果"）我就能看到完成摘要并整理」，**严禁承诺"完成后会自动处理/自动整理给你"**——除非本次派单已传 wakeOnComplete=true）。`,
  ...
  const status = {
    name: 'de_coi_status',
    description: '查询 COI 任务状态与输出摘要（taskId 来自 de_coi_dispatch）。返回状态（running/completed/failed/killed/interrupted）、会话 id（若已捕获）、输出摘要。**⚠️ 输出摘要只是任务输出的尾部截断（前段省略）——完整输出的日志文件绝对路径始终写在输出响应的第一行，需要全量输出时直接用 read 工具读取该日志文件即可，不要自行搜索日志位置**。需要立即确认状态、拿完整摘要或查会话 id 时用它；也可在回合内轮询跟踪长任务。',
    parameters: {
      type: 'object',
      properties: {
