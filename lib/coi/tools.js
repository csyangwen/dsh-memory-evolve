...
【主动通知】任务完成后，其结果摘要会自动注入你**下一次**的上下文中（快照「COI 任务状态」段：运行中 + 最近完成各一行）——**无需轮询、无需阻塞等待也能获知结果**。派完任务可以继续做其他事，完成信息下次自动可见；需要立即拿结果时才用 de_coi_wait。`,
    parameters: {
      type: 'object',
      properties: {
  ...
  const status = {
    name: 'de_coi_status',
    description: '查询 COI 任务状态与输出摘要（taskId 来自 de_coi_dispatch）。返回状态（running/completed/failed/killed/interrupted）、会话 id（若已捕获）、输出摘要。⚠️ 任务完成摘要会自动注入你下一次的上下文（「COI 任务状态」段），**通常无需主动轮询**；需要立即确认状态、拿完整摘要或查会话 id 时才查询。',
    parameters: {
      type: 'object',
      properties: {
  ...
  const wait = {
    name: 'de_coi_wait',
    description: '阻塞等待一个 COI 任务完成（最多等 timeoutMs，默认 60 秒，上限 10 分钟）。任务完成即返回（不等满 timeout，不浪费等待时间）。⚠️ 等待期间会阻塞当前回合（点停止按钮可中断）；AI 代理任务通常数分钟到数小时，**任务完成摘要会自动注入你下一次的上下文（「COI 任务状态」段），通常无需 wait**——派完任务继续做其他事即可；只在任务预计很快完成且必须立即同步拿结果时才用 wait。任务完成返回结果摘要；超时返回当前状态。',
    parameters: {
      type: 'object',
      properties: {
        taskId: { type: 'string', description: '任务 id' },
