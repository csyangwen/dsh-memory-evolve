...
async function handle(svc, sub, rest, invocation) {
  switch (sub) {
    case 'run': {
      const { opts, positional } = parseOpts(rest, ['continue', 'all', 'inject-context'])
      const prompt = positional.join(' ')
      if (!prompt && !opts.template) return { kind: 'error', text: '用法：/de_coi run "<任务>" [--coi kimi] [--scope project] [--session <id>] [--branch <b>] [--model <m>] [--ref <taskId>] [--template <id>] [--continue] [--inject-context] [--context-text <文本>] [--inject-context] [--context-text <文本>]' }
      const result = svc.scheduler.dispatch({
        ...
        injectContext: opts['inject-context'],
