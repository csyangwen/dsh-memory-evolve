...
import { translate, getLocale, COI_DICT, COI2_DICT, HELP_EXTRA } from '../i18n.js'
...
/** 当前 slash 调用的 COI 查看视角；缺少身份时按未知会话处理。 */
function taskViewer(invocation) {
...
function canAccessTask(svc, taskId, invocation) {
  const task = svc.tasks.get(taskId)
  return task !== undefined && svc.tasks.canAccess(task, taskViewer(invocation))
}

function inaccessibleTask(taskId) {
  return translate(COI2_DICT, 'coi2.taskInaccessible', { id: taskId }, getLocale())
}

function canAccessSession(svc, sessionId, invocation) {
  const viewer = taskViewer(invocation)
  return svc.sessions.list(viewer).some((session) => session.id === sessionId)
}

function inaccessibleSession(sessionId) {
  return translate(COI2_DICT, 'coi2.sessionInaccessible', { id: sessionId }, getLocale())
}
...
async function handle(svc, sub, rest, invocation) {
  switch (sub) {
    case 'run': {
      ...
      if (opts.ref && !canAccessTask(svc, opts.ref, invocation)) return { kind: 'error', text: inaccessibleTask(opts.ref) }
      const viewer = taskViewer(invocation)
    ...
    case 'log': {
      ...
      if (!canAccessTask(svc, taskId, invocation)) return { kind: 'error', text: inaccessibleTask(taskId) }
    ...
    case 'stop': {
      ...
      const taskId = positional[0]
      if (!taskId && !opts.all) return { kind: 'error', text: cmt('coicmd.stopUsage') }
      const viewer = taskViewer(invocation)
      if (opts.all) {
        if (!opts.force) return { kind: 'error', text: cmt('coicmd.stopAllConfirm') }
        const running = svc.tasks.list({ status: 'running', ownerSessionId: viewer.ownerSessionId, sessionCwd: viewer.sessionCwd }).filter((t) => svc.scheduler.status(t.id).task?.status === 'running')
    ...
    case 'sessions': {
      ...
      if (action === 'list') {
        const viewer = taskViewer(invocation)
        const sessions = svc.sessions.list({ scope: opts.scope, branch: opts.branch, q: opts.q, adapterId: opts.coi, ownerSessionId: viewer.ownerSessionId, sessionCwd: viewer.sessionCwd })
      ...
      if (action === 'note') {
        ...
        if (!canAccessSession(svc, id, invocation)) return { kind: 'error', text: inaccessibleSession(id) }
    ...
    case 'export': {
      ...
      if (!canAccessSession(svc, sessionId, invocation)) return { kind: 'error', text: inaccessibleSession(sessionId) }
