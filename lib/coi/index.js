...
const RUNTIME_DEFAULTS = {
  ...
  coiDefaultInjectContext: false, // 默认是否向 COI 任务注入 DSH 记忆上下文（默认关：内容会发给外部 COI 服务；任务级参数可覆盖）
}
  ...
  for (const [key, value] of Object.entries(patch)) {
    switch (key) {
      ...
      case 'coiTaskTimeoutMs':
        ...
        break
      case 'coiDefaultInjectContext':
        if (typeof value !== 'boolean') throw new Error('coiDefaultInjectContext 必须是布尔值')
        break
      default:
...
function makeNotify(commandTemplate) {
  ...
  return ({ taskId, coi, status, summary }) => {
    const text = String(commandTemplate)
      ...
      .replaceAll('{summary}', String(summary ?? '').slice(0, 200).replaceAll('\n', ' '))
    try {
      const child = spawn('sh', ['-c', text], { stdio: 'ignore', detached: true })
