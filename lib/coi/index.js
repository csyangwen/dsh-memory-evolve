...
import { DynamicsStore } from './dynamics.js'
    ...
    if (channels && deps.sendChannelNotify) {
      try {
        ...
        void Promise.resolve(deps.sendChannelNotify({
          channels,
          // 固定模板 + 受控占位符（taskId/coi/status 来自插件内部，summary
          // 是文本参数走 JSON 字符串传递，无 shell 面）；summary 截断防刷屏
          content: `[COI] 任务 ${taskId ?? ''}（${coi ?? ''}）${status ?? ''}：${String(summary ?? '').replace(/\s+/g, ' ').slice(0, 120)}`,
        })).catch(() => { /* 渠道通知失败不影响任务 */ })
      } catch { /* 同步异常同样静默 */ }
    }
  }
...
export function installBroadcast(ctx, config) {
  ...
  // 房间动态队列（用户拍板 2026-08-09）：成员 running/idle 状态变化
  // **不再发收件箱系统通知**（未读会逼模型 read，高频低信息量不划算），
  // 改为写进 dynamics 队列 → 快照「房间动态」段直接注入模型上下文，
  // 渲染即消费（buildBroadcastBlock 渲染时同步推进水位，无需 read）。
  const dynamics = new DynamicsStore(dir)
  ...
  // onChange：成员 running⇄idle 切换 → 写动态队列（只记房间成员变化，
  // 非成员零事件；房间只有自己时事件无人可看，也跳过）
  const presence = new PresenceTracker(ctx, dir, (sessionId, prevStatus, nextStatus) => {
    try {
      ...
      for (const room of rooms) {
        ...
        dynamics.append(room.id, sessionId, nextStatus === 'running' ? 'running' : 'idle')
      }
    } catch { /* 队列写入失败不影响 presence 状态维护 */ }
  })
  ...
  disposers.push(ctx.effect(() => {
    ...
    const d = ctx.tools.register(messageToolDefinition(broadcast, presence, config.memoryDir, { agents: ctx.agents, sessionTitle: ctx.sessionTitle }))
    return () => d?.()
