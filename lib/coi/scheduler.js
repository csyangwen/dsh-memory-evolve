...
const WAKE_BUDGET = 3          // 完成唤醒连续次数上限（防 AI 自激链：唤醒回合又派带唤醒的
                               // 任务→完成又唤醒……官方 tool-tasks maxConsecutiveWakes 同款语义；
                               // 超出降级为注入，下次显式派单重置预算）
...
export class CoiScheduler {
  constructor(ctx, deps, opts = {}) {
    ...
    // 完成唤醒预算：每 Agent 连续被唤醒次数（WeakMap，不泄漏）。官方
    // tool-tasks 同款防自激：唤醒回合若又派带唤醒的任务，链式唤醒有上限。
    // 预算重置对齐官方语义：用户输入 claimed 时重置（用户接管对话节奏=链断）；
    // 插件自己排队的 notice 消息（source.kind 非 user）不重置。
    this.spentWakes = new WeakMap()
    if (typeof ctx.on === 'function') {
      try {
        const register = () => ctx.on('agent/inbox/claimed', ({ agent, message }) => {
          if (agent && message?.source?.kind === 'user') this.spentWakes.delete(agent)
        })
        if (typeof ctx.effect === 'function') ctx.effect(register)
        else register()
      } catch {
        /* 旧运行时无该事件：预算不重置（降级为永久上限，不影响主流程） */
      }
    }
...
- 需要当前会话中的 AI 进一步处理的内容（文件、产物、可运行地址），一律给出文件的**绝对路径**；不要只描述、不给出可访问的位置。`
  ...
  #finish(internal, patch) {
    ...
    // 完成唤醒（可选）：wakeOnComplete=true 时向派单会话投递完成通知——
    // 空闲 → followup（自动开新回合，AI 醒来处理结果）；忙碌 → inject
    // （注入下一步）；连续唤醒超预算 → 降级注入。失败静默不影响任务。
    if (done.wakeOnComplete && done.ownerSessionId && this.agentsService) {
      try {
        this.#wakeOwner(done, status)
      } catch {
        /* 唤醒失败不影响任务状态 */
      }
    }
  ...
  /**
   * 完成唤醒投递：与官方 tool-tasks 的 wakeup 投递同款机制——直接复用
   * Agent 的 followup/inject 两个 API（0811 起 Agent 接口暴露）：
   *   - followup = 投递到下一回合并唤醒（idle 的 driver 自动开新回合）
   *   - inject   = 投递到下一步、不唤醒（等下次生成才看到）
   * 筛选逻辑照抄官方：idle 且预算未耗尽 → followup；否则 inject。
   * 消息按官方 UserMessage 结构构造（source=插件 notice 表单，id 用
   * randomUUID），唤醒后 AI 像收到一条用户消息一样正常生成回复。
   * @param {object} done - 终态任务记录（含 ownerSessionId/coi/summary）。
   * @param {string} status - 终态：completed/failed/killed/interrupted。
   */
  #wakeOwner(done, status) {
    const agent = this.agentsService.get(done.ownerSessionId)
    if (!agent) return
    const spent = this.spentWakes.get(agent) ?? 0
    // 通知文本：适配器 + 任务 id + 状态 + 摘要开头（完整输出走 de_coi_status）
    const summaryText = done.summary ? ` 摘要：${String(done.summary).slice(0, 300)}` : ''
    const text = `【COI 任务完成】${done.coi} 任务 ${done.id} 状态：${status}${summaryText}（完整输出与详情用 de_coi_status 查询，taskId: ${done.id}）`
    const message = {
      id: randomUUID(),
      role: 'user',
      ...
      source: {
        ...
        form: 'notice',
        summary: `COI ${done.coi} 任务 ${done.id} ${status}`,
      },
    }
    if (agent.status === 'idle' && spent < WAKE_BUDGET) {
      this.spentWakes.set(agent, spent + 1)
