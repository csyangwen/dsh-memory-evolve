...
export class CoiScheduler {
   ...
   * @param {{preparedPrompt?:boolean}} [options] - 重试已启动过的任务时，复用已准备的完整提示词。
   * @returns {{ok:boolean, taskId?:string, status?:string, message?:string}}
   */
  dispatch(req, { preparedPrompt = false } = {}) {
    ...
    if (req.refTaskId && !preparedPrompt) {
      ...
      if (full) {
        if (full.length <= RELAY_INLINE_MAX) {
          ...
          try {
            ...
            return { ok: false, message: cot('coi.relayPrepareFailed', { detail: error.message }) }
          }
        }
      }
    }
    ...
    if (!preparedPrompt && attachmentFiles.length > 0 && adapter.image?.mode === 'prompt') {
    ...
    // 在创建任务和占用会话前读取背景，读取失败不留下半启动的任务。
    const injectTracks = !preparedPrompt && Array.isArray(req.injectTracks)
      ? req.injectTracks.filter((t) => t === 'memory' || t === 'user' || t === 'key')
      : []
    const contextText = preparedPrompt ? '' : String(req.contextText ?? '').trim()
    let memText = ''
    if (injectTracks.length > 0) {
      try {
        memText = String(this.memoryContext?.({ cwd, branch: req.branch ?? null, tracks: injectTracks }) ?? '').trim()
      } catch (error) {
        return { ok: false, message: cot('coi.contextReadFailed', { detail: error.message }) }
      }
    }
    ...
    // 任务先入队，占用失败则记录失败状态，不能留下未启动的 queued 记录。
    ...
    if (sessionId && adapter.type === 'ai-cli') {
      ...
      if (!lock.ok) {
        this.tasks.update(task.id, { status: 'failed', finishedAt: Date.now(), error: lock.message })
        return { ok: false, taskId: task.id, status: 'failed', message: lock.message }
      }
    }
    ...
    if (ctxParts.length > 0) {
      ...
      if (ctxBlock.length <= CONTEXT_INLINE_MAX) {
        ...
        try {
          ...
          const message = cot('coi.contextPrepareFailed', { detail: error.message })
          this.tasks.update(task.id, { status: 'failed', finishedAt: Date.now(), error: message })
          if (task.sessionId) this.sessions.release(task.sessionId, task.id)
          return { ok: false, taskId: task.id, status: 'failed', message }
        }
      }
    }
    ...
    if (adapter.type === 'ai-cli' && !preparedPrompt) {
      finalPrompt += `\n\n【输出约定】（请遵守）
...
- 需要当前会话中的 AI 进一步处理的内容（文件、产物、可运行地址），一律给出文件的**绝对路径**；不要只描述、不给出可访问的位置。`
    ...
    if (task.status !== 'running') {
      return { ok: false, taskId: task.id, status: task.status, message: task.error ?? cot('coi.failedToStart') }
    }
  ...
  #startProcess(adapter, task, { finalPrompt, mode }) {
    ...
    let child
    try {
      // Windows CLI（含命令包装器）不宜把长任务塞进命令行；完整内容由 CLI 读文件。
      if (process.platform === 'win32' && adapter.type === 'ai-cli' && Buffer.byteLength(finalPrompt, 'utf8') > 4000) {
        const promptFile = join(this.config.coiDataDir, 'contexts', `${task.id}-prompt.txt`)
        mkdirSync(dirname(promptFile), { recursive: true })
        writeFileSync(promptFile, finalPrompt)
        finalPrompt = cot('coi.taskFilePrompt', { path: promptFile })
      }
      ...
      if (args.some((arg) => arg.includes('{sessionId}')) && mode === 'resume') {
        this.#finish(internal, { status: 'failed', error: cot('coi.missingResumeSession') })
        return
      }
      ...
      this.#finish(internal, { status: 'failed', error: cot('coi.startFailed', { detail: error.message }) })
      return
    }
  ...
  cancel(taskId, opts = {}) {
    ...
    internal.buffer += '\n[任务被用户终止]\n'
    this.#finish(internal, { status: 'killed', error: '用户终止' })
  ...
  retry(taskId) {
    ...
    return this.dispatch({
      ...
      wakeOnComplete: task.wakeOnComplete,
      injectTracks: task.injectTracks,
      contextText: task.contextText,
    }, { preparedPrompt: task.startedAt != null })
  }
