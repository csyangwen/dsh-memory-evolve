...
import { translate, getLocale, COI_DICT, COI2_DICT } from '../i18n.js'
...
export class CoiScheduler {
  ...
  dispatch(req) {
    ...
    else if (adapter.type === 'ai-cli' && req.continueLast) {
      const recent = this.sessions.list({
        adapterId: adapter.id,
        cwd: cwd ?? undefined,
        ownerSessionId: req.ownerSessionId ?? null,
        sessionCwd: req.ownerCwd ?? null,
      })[0]
      if (!recent) return { ok: false, message: cot('coi.noAccessibleRecentSession') }
      sessionId = recent.id
      cwd ??= recent.cwd
      mode = 'resume'
    }

    if (sessionId && this.sessions.findById(sessionId)
      && !this.sessions.list({ ownerSessionId: req.ownerSessionId, sessionCwd: req.ownerCwd }).some((s) => s.id === sessionId)) {
      return { ok: false, message: translate(COI2_DICT, 'coi2.sessionInaccessible', { id: sessionId }, getLocale()) }
    }
    if (sessionId) cwd ??= this.sessions.findById(sessionId)?.cwd ?? null
...
- 需要当前会话中的 AI 进一步处理的内容（文件、产物、可运行地址），一律给出文件的**绝对路径**；不要只描述、不给出可访问的位置。`
      ...
      if (found) {
        ...
        if (internal.scope !== 'temporary') {
          this.sessions.upsert({
            ...
            ownerCwd: internal.ownerCwd ?? null,
          })
  ...
  #finish(internal, patch) {
    ...
    if ((done.scope === 'project' || done.scope === 'global') && this.writeSummary) {
      const line = `[COI] ${done.coi} 任务 ${done.id} ${status}${done.sessionId ? `（会话 ${done.sessionId}）` : ''}：${String(done.prompt ?? '').slice(0, 120)}`
      try {
        this.writeSummary({ scope: done.scope, cwd: done.ownerCwd ?? done.cwd, branch: done.branch, text: line })
      } catch {
        /* 记忆写入失败不影响任务 */
      }
    }
