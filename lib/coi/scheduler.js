  ...
  async wait(taskId, timeoutMs = 60000) {
    ...
    return new Promise((resolve) => {
      ...
      const tick = () => {
        ...
        if (!current) {
          resolve({ ok: false, message: `任务 ${taskId} 不存在` })
          return
        }
        if (current.status !== 'running' && current.status !== 'queued') {
          resolve({ ok: true, task: snapshot(current) })
          return
        }
        if (Date.now() >= deadline) {
          resolve({ ok: false, message: `等待超时（${timeoutMs}ms），任务仍在运行，可用 de_coi_status 再查`, task: snapshot(current) })
          return
        }
