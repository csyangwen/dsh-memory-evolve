...
export class TaskStore {
  ...
  /**
   * 判断任务是否对指定会话可见。
   * 未提供查看视角时保留底层存储的全量读取语义；调用方应传入
   * ownerSessionId/sessionCwd（未知身份用 null），这样 session/project
   * 不会因缺少上下文而放宽，global 仍对所有会话可见。
   * @param {object|string} taskOrId - 任务记录或任务 id。
   * @param {{ownerSessionId?:string|null, sessionCwd?:string|null}} filter
   */
  canAccess(taskOrId, filter = {}) {
    ...
    if (viewerSession === undefined && viewerCwd === undefined) return true
    switch (task.scope) {
      case 'temporary':
      case 'session':
        return viewerSession !== undefined
          && viewerSession !== null
          && String(viewerSession).trim() !== ''
          && task.ownerSessionId === viewerSession
      case 'project':
        if (viewerCwd === undefined || viewerCwd === null || String(viewerCwd).trim() === '') return false
        return task.ownerCwd != null
          ? task.ownerCwd === viewerCwd
          : task.cwd != null && task.cwd === viewerCwd
      default:
        return true // global 与历史未知层级保持全显
    }
  }
  ...
  #query(filter = {}) {
    ...
    if (viewerSession !== undefined || viewerCwd !== undefined) {
      items = items.filter((t) => this.canAccess(t, filter))
    }
