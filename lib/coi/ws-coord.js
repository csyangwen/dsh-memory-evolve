 ...
 *    显式 release 释放（**回合结束不释放 observed 锁**——保留 TTL 30 分钟，
...
export class WsCoordStore {
   ...
   * ⚠️ 默认释放策略=保留到 TTL（30 分钟）——曾由 agent/turn-stopping
   * 回合结束调用，2026-08-09 实测发现回合结束即释放会让"先后写入"
   * （A 改完 → B 再改）检测不到，已移除该监听。
...
export function buildWsCoordBlock(config, sessionId, cwd, store, sessionMeta, displayName) {
  ...
  return `【工作区活动】${fmtDateTime(Date.now())} 起 ${active.length} 个会话并行中：${parts.join(' / ')}。如要修改共享文件：**开工前先用 de_ws_declare 声明**（其他会话能看到你在做什么），动手前用 de_ws_status 查询占用，避免与他人的文件冲突`
}
...
export function installWsCoord(ctx, config, deps = {}) {
  ...
  // observed 锁**保留到 TTL（30 分钟）**自然过期，AI 可用 de_ws_release
