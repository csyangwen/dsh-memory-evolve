...
/** declared（声明）锁默认 TTL（分钟）。 */
const DEFAULT_TTL_MIN = 60
/** observed（自动登记）锁默认 TTL（分钟）：写过的文件短时间内别人最好别动。 */
const OBSERVED_TTL_MIN = 30
...
export class WsCoordStore {
   ...
   * @param {object} req - { sessionId, cwd, targets?, services?, ttlMinutes? }
   * @returns {{ declared: object[], conflicts: object[] }}
   *   declared=本次登记的锁；conflicts=与其他会话现有锁的重叠清单。
   */
  declare({ sessionId, cwd, targets, services, ttlMinutes }) {
    this.#pruneExpired()
    const now = Date.now()
    const ttl = Math.min(Math.max(Number(ttlMinutes) || DEFAULT_TTL_MIN, 1), 1440) * 60 * 1000
    const declared = []
    const conflicts = []
    const entries = []
  ...
  observe(sessionId, cwd, targetKey) {
    ...
    if (own) {
      own.expiresAt = now + OBSERVED_TTL_MIN * 60 * 1000
      ...
      this.locks.push({
        ...
        note: '',
        startedAt: now,
        expiresAt: now + OBSERVED_TTL_MIN * 60 * 1000,
        source: 'observed',
        cwd: cwd ?? '',
      })
    }
  ...
  /**
   * 回合结束释放 observed 锁（agent/turn-stopping）：回合结束 = 该回合
   * 自动登记的文件占用结束；declared 锁保留到 TTL 或显式 release。
   ...
   */
  activeFor(cwd, sessionMeta, now = Date.now()) {
    this.#pruneExpired()
    ...
    for (const [sessionId, meta] of sessionMeta) {
      ...
      out.push({ sessionId, status: 'running', note: '', lockCount: 0, since: meta.lastActiveAt ?? now })
    }
    return out
  }
}
...
/**
 * 活动感知快照段【工作区活动】：活跃会话 >= 2 时注入一行（带当前时间，
 * 其他会话据此判断新旧），单会话/无视角/开关关 = null（零开销）。
 ...
 * @returns {string | null}
 */
export function buildWsCoordBlock(config, sessionId, cwd, store, sessionMeta, displayName) {
  if (config.wsCoordEnabled !== true) return null
  if (config.wsCoordSnapshot !== true) return null
  if (!sessionId || !cwd) return null
  let active
  try {
    active = store.activeFor(cwd, sessionMeta)
  } catch { return null }
  ...
  return `【工作区活动】${fmtDateTime(Date.now())} 起 ${active.length} 个会话并行中：${parts.join(' / ')}。动共享文件前先 de_ws_status 查询占用`
}
 ...
 */
export function wsToolDefinitions(store, displayName, sessionMeta) {
  /** 锁 → 输出视图（只留对外字段，含别名显示）。 */
  const lockView = (l) => ({
    id: l.id,
  ...
  return [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          ttlMinutes: { type: 'integer', description: '锁有效期分钟（默认 60，上限 1440；写操作自动续期）' },
        },
      },
      output: {
        schema: {
      ...
      async execute(args, exec) {
        ...
        try {
          const { declared, conflicts } = store.declare({ sessionId: ctx.sessionId, cwd: ctx.cwd, targets: args.targets, services: args.services, ttlMinutes: args.ttlMinutes })
    ...
    },
    {
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          const active = store.activeFor(ctx?.cwd, sessionMeta ?? new Map())
          return {
            ok: true,
...
export function installWsCoord(ctx, config, deps = {}) {
  ...
  /** 会话显示名：别名优先（读 aliases.json，与广播模块同款），无别名截短 ID。 */
  const displayName = (sid) => {
    try {
      const aliases = config.memoryDir ? JSON.parse(readFileSync(join(config.memoryDir, 'aliases.json'), 'utf8')) : {}
      if (aliases && typeof aliases === 'object' && typeof aliases[sid] === 'string') return aliases[sid]
    } catch { /* 无别名文件：截短 ID */ }
  ...
  // 5. agent/turn-stopping：回合结束释放本回合自动登记的 observed 锁
  //    （declared 锁保留到 TTL 或显式 release）
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('agent/turn-stopping', (payload) => {
        try {
          const sessionId = payload?.agent?.session?.id
          if (sessionId) store.clearObserved(sessionId)
        } catch { /* 释放失败不影响回合结束 */ }
      }), 'dsh-memory-evolve: ws-coord turn-stopping')
    : () => {})

  // —— 工具注册（三个 de_ws_* 工具）——
  disposers.push(ctx.effect
    ? ctx.effect(() => {
        const defs = wsToolDefinitions(store, displayName, sessionMeta)
