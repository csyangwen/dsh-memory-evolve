...
async function finalize(paths, { query, exts, limit, kind = 'file' }) {
  ...
  const entries = await statEntries(matched, 32, 2000, kind)
...
export function renderSearchResult(value) {
  ...
  if (!value.results || value.results.length === 0) {
    return `没有找到匹配的${value.type === 'dir' ? '文件夹' : value.type === 'any' ? '文件或文件夹' : '文档'}（provider: ${value.provider ?? 'none'}）`
  }
  ...
  const head = `找到 ${value.count} 个${noun}${value.truncated ? '（已截断，可增大 limit）' : ''}（provider: ${value.provider}）：`
  return `${head}\n${lines.join('\n')}`
}
...
export function searchDocsToolDefinition(config, search) {
  ...
  return {
    ...
    description: `在本机所有磁盘/目录中按文件名搜索文件（只匹配文件名，不读取内容）。**主要用途：查找文档**——不传 exts 时默认只搜文档扩展名 ${JSON.stringify(defaultExts)}，不会枚举全部文件。**搜索所有类型（图片/视频/任意扩展名）必须显式传 allTypes=true**（或 exts=["*"]）：副作用是结果集可能很大、首次全盘扫描较慢，非必要不要用。type=dir 可搜索文件夹名（同样建议配合 query 缩小范围）。query 为文件名关键字（子串、大小写不敏感；**中文文件名请尝试多个说法/同义词**——如"年报"可能查不到，可换"述职"、"年度报告"、"年终总结"等文件名中实际出现的词，无结果时换词再查或留空列出最近文档）；exts 限定扩展名（数组或逗号字符串）；dir 可限定搜索目录；limit 默认 20 最大 100。结果按修改时间倒序返回路径、名称、大小与修改时间。`,
    parameters: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: '文件名关键字（子串匹配，大小写不敏感）；留空 = 列出最近修改的文件',
        },
        exts: {
          type: 'array',
          items: { type: 'string' },
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        return { ok: false, message: 'exts 参数格式不正确（应为扩展名数组或逗号分隔字符串）', provider: null, type: kind, count: 0, truncated: false, results: [] }
      }
      let dir
      ...
      try {
        const { provider, results } = await search({ query, exts, dir, limit, kind }, signal)
        return {
          ok: true,
          provider,
          type: kind,
        ...
        return { ok: false, message, provider: null, type: kind, count: 0, truncated: false, results: [] }
      }
    },
