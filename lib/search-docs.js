 ...
 * - 候选文件上限 CONTENT_CANDIDATE_CAP（内容模式内部扩容，再按 limit 截断返回）
 */
...
async function rgSearchFileContents(paths, contentQuery, opts = {}) {
  ...
  // 每批约 80 个路径：避免命令行过长，同时减少 spawn 次数
  const BATCH = 80
...
export function searchDocsToolDefinition(config, search) {
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      // 内容检索参数：仅对文件生效（type=dir 时忽略，保持文件夹名搜索语义）
      const contentArgs = resolveContentSearchArgs(args, query)
      const contentEnabled = contentArgs.enabled && kind !== 'dir'
      const effectiveContentQuery = contentArgs.contentQuery

      // 内容模式需要更多候选再过滤：内部扩容 limit，返回仍按用户 limit 截断
      const searchLimit = contentEnabled ? contentCandidateLimit(limit) : limit
      ...
      try {
        ...
        const { provider, results } = await search(
          { query, exts, dir, limit: searchLimit, kind },
          signal,
        )
