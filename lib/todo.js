  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        past: { type: 'boolean', description: 'list 时 true=同时查询每日待办的过往（今天之前的 daily 文件，条目带日期；默认不含）；过往条目默认不返回已过期的遗留，需配合 expired=true' },
        expired: { type: 'boolean', description: 'list 时 true=包含已过期的过往每日待办（仅与 past=true 配合生效；缺省只显示未过期的过往条目）' },
