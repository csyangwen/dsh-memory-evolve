...
export class MemoryStore {
   ...
   * @param {object} [options] - limits and scan switch.
   * @param {number} [options.memoryCharLimit=2200] - MEMORY.md char cap.
   * @param {number} [options.userCharLimit=1375] - USER.md char cap.
   ...
   * @param {number} [options.dailyCharLimit=8000] - per-day log char cap.
   * @param {number} [options.projectCharLimit=2200] - per-project char cap.
   */
  constructor(dir, options = {}) {
    ...
    this.dailyCharLimit = options.dailyCharLimit ?? 8000
    this.projectCharLimit = options.projectCharLimit ?? 2200
  }
   ...
   * @returns {{dir: string, file: string, limit: number} | undefined}
  ...
  locate(target, agent) {
    switch (target) {
      case 'memory':
        return { dir: this.dir, file: 'MEMORY.md', limit: this.memoryCharLimit }
      case 'user':
        return { dir: this.dir, file: 'USER.md', limit: this.userCharLimit }
      case 'daily':
        return { dir: join(this.dir, 'daily'), file: `${todayStamp()}.md`, limit: this.dailyCharLimit }
      case 'project': {
        const cwd = agent?.session?.header?.cwd
        if (!cwd) return undefined
        return { dir: join(this.dir, 'projects', projectHash(cwd)), file: 'MEMORY.md', limit: this.projectCharLimit }
      }
      default:
  ...
  /**
   * Stamp one entry with a time prefix: date stamp for long-term targets,
   * time-of-day stamp for the daily log. Idempotent for content that already
  ...
  /** Character cap of one target. */
  limitOf(target, agent) {
    return this.resolveTarget(target, agent).limit
  }
  ...
  add(target, content, agent) {
    ...
    const stamped = this.stampEntry(target, text)
    return withLock(loc.dir, () => {
      ...
      if (entries.includes(stamped)) {
        return {
          ...
          entries: [...entries], chars: this.charsOf(target, agent), limit: loc.limit,
        }
      }
      ...
      if (total > loc.limit) {
        return {
          ok: false,
          message: `记忆已达 ${this.charsOf(target, agent)}/${loc.limit} 字符，添加该条目（${stamped.length} 字符）会超限。请先用 replace 合并或用 remove 清理，再重试。`,
          target, entries: [...entries], chars: this.charsOf(target, agent), limit: loc.limit,
        }
      }
      this.write(target, next, agent)
      return {
        ok: true, message: `已添加（${target}：${entries.length} → ${next.length} 条）`, target,
        entries: [...next], chars: total, limit: loc.limit,
      }
    })
  }
  ...
  replace(target, match, content, agent) {
    ...
    return withLock(loc.dir, () => {
      ...
      const next = [...entries]
      next[index] = this.stampEntry(target, newContent)
      const total = next.join(ENTRY_DELIMITER).length
      if (total > loc.limit) {
        return {
          ok: false,
          message: `替换后记忆将达 ${total}/${loc.limit} 字符，超限。请缩短新内容或先清理其他条目。`,
          target, entries: [...entries], chars: this.charsOf(target, agent), limit: loc.limit,
        }
      }
      this.write(target, next, agent)
  ...
  remove(target, match, agent) {
    ...
    return withLock(loc.dir, () => {
      ...
      return {
        ...
        entries: [...next], chars: next.join(ENTRY_DELIMITER).length, limit: loc.limit,
      }
    })
  }
}
