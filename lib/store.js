...
export class MemoryStore {
  ...
  write(target, entries, agent) {
    ...
    renameSync(tmp, path)
    // U+FFFD（REPLACEMENT CHARACTER）检测：持久化记忆里出现替换字符，几乎必然
    // 来自上游某条写入链路的有损字节→文本解码——多字节字符在传输/解码边界被
    // 截断后按替换符落盘（2026-09-14 实证：一个三字节汉字被拆成两个 U+FFFD，
    // 且损坏随同步往返持续增长）。当场记录，由 memory 工具结果随写随报，让
    // 写它的会话立即修复，而不是静默累积数天才被发现。不阻断写入：正文里
    // 合理引用含替换字符的内容属合法场景。
    const chars = (text.match(/\uFFFD/g) ?? []).length
    this.lastCorruptWrite = chars > 0 ? { target, entries: entries.length, chars } : null
  }

  /**
   * Read-and-clear the last `write()`'s U+FFFD detection result (consume-once,
   * so each mutation's warning is reported exactly once, in that mutation's own
   * tool result). `null` = the last write carried no replacement characters.
   */
  takeCorruptWrite() {
    const hit = this.lastCorruptWrite ?? null
    this.lastCorruptWrite = null
    return hit
  }
