...
export function isProjectSyncEnabled(dir) {
  // 无记录（老仓库/未初始化）≠ 停用：只有显式 enabled=false 才算停用
  // （项目开关语义：默认参与；停用是明确动作，2026-08-11 用户拍板）
    ...
    try {
      ...
      if (typeof owner.pid === 'number' && owner.pid !== process.pid) {
        try {
          ...
          return true // 持有者已死（kill 后残留）→ stale
        }
      }
    } catch {
      // 旧格式锁文件（无 pid）→ 按 mtime 判断
    }
    return false
  } catch {
  ...
  replace(target, match, content, agent) {
    ...
    return withLock(loc.dir, () => {
      ...
      if (this.entryIdMode === 'on' && (target === 'key' || target === 'project')
        && isProjectSyncEnabled(loc.dir)) {
        // 替换不换身份（审查 P1）：旧 ID 是权威——先剥离 replacement 里任何
        // 自带/误写的 ID，再前置旧 ID；旧条目无 ID（理论不发生：on 项目
        // bootstrap 已补发）则按旧内容确定性补发（双设备一致）。
        const oldId = extractEntryId(matches[0]) ?? legacyIdFor(stripEntryId(matches[0]))
