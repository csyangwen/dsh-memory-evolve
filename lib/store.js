 ...
 * 落在这里。不注入任何会话；条目可在设置面板「移回主记忆」（转正，写回
 * 对应主轨）或删除。文件与主轨同格式（§ 分隔 + `[YYYY-MM-DD]` 时间戳），
 * 按原 target 分文件：`MEMORY-archive.md` / `USER-archive.md`。
 */
export class ArchiveStore {
  ...
  /** Resolve one archive file path; only memory/user have archive files. */
  fileOf(target) {
  ...
  entriesOf(target) {
    try {
      return parseEntries(readFileSync(this.fileOf(target), 'utf8'))
    } catch (error) {
      if (error.code === 'ENOENT') return []
      throw error
    }
  ...
  append(target, content) {
    return withLock(this.dir, () => {
      const entries = this.entriesOf(target)
      entries.push(content)
      const path = this.fileOf(target)
      const tmp = `${path}.tmp.${process.pid}`
