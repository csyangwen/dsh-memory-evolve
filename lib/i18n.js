...
export const COI_DICT = {
  ...
  'coi.failedToStart': ['任务启动失败', 'The task failed to start'],
  'coi.missingResumeSession': ['缺少 sessionId 无法恢复会话', 'A sessionId is required to resume a session'],
  'coi.contextReadFailed': ['读取任务背景失败: {detail}', 'Reading task context failed: {detail}'],
  'coi.contextPrepareFailed': ['准备任务背景失败: {detail}', 'Preparing task context failed: {detail}'],
  'coi.relayPrepareFailed': ['准备接力内容失败: {detail}', 'Preparing relay content failed: {detail}'],
  'coi.taskFilePrompt': ['完整任务文件：{path}\n请先完整读取该文件，再按其中的任务、背景和规则执行。不要只回复文件路径。', 'Complete task file: {path}\nRead the entire file first, then follow its task, context and instructions. Do not only reply with the file path.'],
