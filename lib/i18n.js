...
export const STORE_DICT = {
  ...
  'store.replacementChars': [
    '⚠️ 本次写入含 {chars} 个 U+FFFD 替换字符（{target} 轨、{entries} 个条目）——这几乎必然是上游某条写入链路的有损字节→文本解码把多字节字符截断所致。请立即检查刚写入的内容并重写损坏条目，不要带着损坏字符继续累积。',
    '⚠️ This write introduced {chars} U+FFFD replacement character(s) ({target} track, {entries} entries) — almost certainly a lossy byte-to-text decode upstream splitting a multi-byte character. Inspect what was just written and rewrite the damaged entries now; do not keep accumulating corrupted text.',
  ],
  'store.driftGuardOp': [
    '拒绝操作：{file} 的内容无法通过记忆工具解析往返。已备份到 {backup}。请先整理文件再重试。',
    'Operation refused: {file} does not round-trip through the memory tool parser. A backup was saved to {backup}. Reformat the file first, then retry.',
