...
export function installApi(ctx, deps) {
  const { store, archive, queue, getRuntime, updateRuntime } = deps
  ...
  const handler = async (req, res) => {
    ...
    try {
      if (req.method === 'GET' && path === '/memory-evolve/api/badge') {
        const pendingSkills = listPendingSkills(pendingSkillDir(deps)).length
        sendJson(res, 200, { count: queue.read().length + pendingSkills })
        return
      }
      ...
      if (req.method === 'GET' && path === '/memory-evolve/api/archive') {
        sendJson(res, 200, {
          entries: [
            ...archive.entriesOf('memory').map((content) => ({ target: 'memory', content })),
            ...archive.entriesOf('user').map((content) => ({ target: 'user', content })),
          ],
        })
        return
      }
      if (req.method === 'POST' && path === '/memory-evolve/api/archive/promote') {
        ...
        if (target !== 'memory' && target !== 'user') throw new Error('target 必须是 memory 或 user')
        if (!match) throw new Error('match 不能为空')
        const outcome = promoteArchived(store, archive, target, match)
      ...
      if (req.method === 'POST' && path === '/memory-evolve/api/archive/delete') {
        ...
        if (target !== 'memory' && target !== 'user') throw new Error('target 必须是 memory 或 user')
        if (!match) throw new Error('match 不能为空')
        const outcome = archive.remove(target, match)
        if (!outcome.ok) throw new Error(outcome.message)
        sendJson(res, 200, outcome)
        return
      }
      ...
      if (req.method === 'POST' && path === '/memory-evolve/api/memory/key') {
        ...
        const outcome = store.add('key', content, { session: { header: { cwd } } })
        if (!outcome.ok) throw new Error(outcome.message)
        sendJson(res, 200, outcome)
        return
      }
      if (req.method === 'POST' && path === '/memory-evolve/api/memory/delete') {
        ...
        const match = String(body?.match ?? '').trim()
        if (!match) throw new Error('match 不能为空')
        const ARCHIVE = { 'archive-memory': 'memory', 'archive-user': 'user' }
        ...
        if (target in ARCHIVE) {
          outcome = archive.removeExact(ARCHIVE[target], match)
        } else {
          const sessionId = String(body?.sessionId ?? '')
          const cwd = deps.resolveCwd?.(sessionId)
