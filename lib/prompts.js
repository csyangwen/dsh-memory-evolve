...
/** 提示词内容上限（与 scratch 一致，512 KiB 级）。 */
...
export function renderInjectionSnapshot(store) {
  ...
  for (const injection of injections) {
    ...
    const body = injection.content.split('\n').map((line) => `  ${line}`).join('\n')
    lines.push(body)
  }
  return lines.join('\n')
}
 ...
 */
export function promptsToolDefinition(config, promptStore, injectionStore) {
  return {
    ...
    description: '查询并注入「提示词库」（prompts）。**正确用途**：提示词库是标准工作范式资产库（代码审查/调试/测试/PRD/架构…）；需要给当前会话注入纪律或流程、或给子会话/子代理/CLI 任务（de_session / subagent / de_coi_dispatch）挑选现成提示词时，先 list 查看可用提示词（只显示启用中：id/名称/简介/分类/标签，不含正文），选定后 get <id> 取详情（含正文全文），或直接 inject <id> 注入当前会话（写入注入轨，模型下一轮自动看到并遵循）。list 支持 filter（名称/简介/标签/分类关键词）与 limit；inject 支持 rounds（次数：1=一次默认，N=有限 N 次，0=无限持续）与 every（间隔：1=每回合默认，0=只注入一次，N=每 N 回合一次）。禁用状态的提示词不出现在列表、也不能注入（get 仍可查详情）。',
    parameters: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['list', 'get', 'inject'],
          description: 'list=列出启用中的提示词；get=按 id 查详情（含正文全文）；inject=注入指定提示词到当前会话',
        },
        id: {
          type: 'string',
          description: 'get / inject 必填：提示词 id（来自 list 返回）',
        },
        filter: {
          type: 'string',
          description: 'list 可选：按名称/简介/标签/分类关键词过滤（大小写不敏感）',
        },
        limit: {
          type: 'integer',
        ...
        rounds: {
          type: 'integer',
          description: 'inject 可选：注入次数，1=只注入一次（默认），N=有限 N 次，0=无限持续直到手动停止',
        },
        every: {
          type: 'integer',
          description: 'inject 可选：注入间隔回合数，1=每回合（默认），0=只注入一次，N=每 N 回合出现一次',
        },
      },
      required: ['action'],
    },
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: (() => {
          ...
          if (value.action === 'get' && value.prompt) {
    ...
    async execute(args) {
      ...
      try {
        if (action === 'list') {
          // 只列启用中的提示词（禁用/草稿不进 AI 视野）；filter 关键词过滤；
          // limit 截断。列表不含正文——克制输出，正文留给 get 详情。
          ...
          const message = q && prompts.length === 0
            ? `没有匹配「${args.filter}」的启用提示词（可去掉 filter 重查）`
            : `启用中的提示词：${prompts.length} 条（确定后 get <id> 取详情，或 inject <id> 注入当前会话）`
        ...
        if (action === 'inject') {
          ...
          const cadence = every === 0 ? '只注入一次' : (every === 1 ? '每回合' : `每 ${every} 回合一次`)
          const times = rounds === 0 ? '无限' : `${rounds} 次`
          return {
            ok: true,
            message: `已注入「${p.name}」：${times}，${cadence}，模型下一轮生效`,
            action: 'inject',
            injection,
          }
        }
...
export function installPrompts(ctx, config) {
  ...
  disposers.push(ctx.effect(() => ctx.tools.register(
    promptsToolDefinition(config, promptStore, injectionStore),
  ...
  ctx.inject(['httpServer'], (webCtx) => {
    ...
    webCtx.effect(() => {
      disposers.push(webCtx.httpServer.register({
        kind: 'prefix',
