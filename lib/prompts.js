...
/**
 * 注入轨存储。
 * 条目：{ id, sessionId, sourcePromptId|null, title, content, roundsLeft, every,
 *        countdown, createdAt }
 ...
 * 每回合结束（agent/turn-stopping）调用 tickTurn，仅推进对应会话：
...
export class InjectionStore {
  ...
  hasSource(promptId, sessionId) {
    return this.list().some((i) => i.sourcePromptId === promptId && i.sessionId === sessionId)
  }

  /**
   * 写入一条注入。
   * @param {{sessionId: string, sourcePromptId?: string|null, title: string, content: string, rounds?: number, every?: number}} input
  ...
  add(input) {
    ...
    if (!sessionId) throw new Error(pt('promptmsg.targetSessionRequired'))
  ...
  /**
   * 一个对话回合结束：只推进该会话的出现计划（countdown 模型）。
  ...
  tickTurn(sessionId) {
    if (!sessionId) return []
    const data = this.backing.read({ injections: [] })
    if (!data.injections.some((i) => i.sessionId === sessionId)) return []
    const expired = []
    const alive = []
 ...
 * @returns {string} 快照文本。
 */
export function renderInjectionSnapshot(store, sessionId) {
  if (!sessionId) return ''
  const injections = store.list().filter((i) => i.sessionId === sessionId && (Number.isInteger(i.countdown) ? i.countdown : 0) === 0)
        ...
        properties: {
          ...
          injection: {
            ...
            properties: {
              ...
              title: { type: 'string' },
              content: { type: 'string' },
              roundsLeft: { oneOf: [{ type: 'integer' }, { type: 'null' }] },
              every: { type: 'integer' },
              countdown: { type: 'integer' },
              createdAt: { type: 'integer' },
            },
            required: ['id', 'sourcePromptId', 'sessionId', 'title', 'content', 'roundsLeft', 'every', 'countdown', 'createdAt'],
          },
        },
        required: ['ok', 'message'],
      },
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        if (action === 'inject') {
          ...
          if (injectionStore.hasSource(id, sessionId)) {
 ...
 *   3. agent/turn-stopping 只推进结束回合所属会话的注入；
...
export function installPrompts(ctx, config) {
  ...
  disposers.push(ctx.effect(() => {
    try {
      return ctx.systemPrompt.context({
        ...
        text: ({ agent }) => renderInjectionSnapshot(injectionStore, agent?.session?.id ?? agent?.session?.header?.id),
      })
    } catch (error) {
  ...
  // 2. 回合推进：只推进结束回合所属会话的注入，子任务不消耗父会话的次数。
  //    listener 全程
  ...
  disposers.push(ctx.on('agent/turn-stopping', (payload) => {
    try {
      ...
      if (!header) return
      const expired = injectionStore.tickTurn(payload.agent.session.id ?? header.id)
      if (expired.length > 0) {
    ...
    const handler = async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'GET' && path === `${base}/injections`) {
          ...
          const injections = injectionStore.list()
          return sendJson(200, { injections: sessionId === null ? injections : injections.filter((i) => i.sessionId === sessionId) })
        }
        ...
        if (match) {
          ...
          if (req.method === 'POST' && sub === 'inject') {
            ...
            if (body?.immediate === true) {
              ...
              promptStore.bumpUsage(id)
              const steered = steerImmediateInjection(ctx, sessionId, prompt.name)
