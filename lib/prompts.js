...
/**
 * 提示词库存储。
 * 条目：{ id, name, category, tags[], content, createdAt, updatedAt,
...
export class PromptStore {
  ...
  static DEFAULT_CATEGORIES = ['开发流程', '问题排查', '设计', '测试', '质量', '性能', '文档', '产品']
   ...
   * 目标名不能为空/超长/「未分类」，不能与现有其他分类重名；旧名不存在报错。
  ...
  renameCategory(oldName, newName) {
    ...
    const data = this.backing.read({ prompts: [] })
    const categories = Array.isArray(data.categories) ? data.categories : []
    if (!categories.includes(from)) throw new Error(`分类「${from}」不存在`)
    if (categories.includes(to)) throw new Error(`分类「${to}」已存在`)
    data.categories = categories.map((c) => (c === from ? to : c))
    let renamed = 0
   ...
   * （不删除提示词本身——分类只是组织维度）。
   * @returns {{removed: boolean, moved: number}} 是否命中 + 被移动的提示词数。
   */
  removeCategory(name) {
    const category = String(name ?? '').trim()
    ...
    const data = this.backing.read({ prompts: [] })
    const categories = Array.isArray(data.categories) ? data.categories : []
    if (!categories.includes(category)) throw new Error(`分类「${category}」不存在`)
    data.categories = categories.filter((c) => c !== category)
    let moved = 0
    for (const prompt of data.prompts) {
  ...
  /**
   * 新建提示词。
   * @param {{name: string, category?: string, tags?: string[], content: string}} input
   * @returns {object} 新条目。
   */
  create(input) {
...
export class InjectionStore {
  ...
  add(input) {
    ...
    const every = Number.isInteger(input?.every) && input.every >= 1
      ? Math.min(input.every, MAX_ROUNDS)
      : 1
    const injection = {
      id: randomUUID(),
   ...
   *   - 本回合是出现轮（countdown===0）→ 有限次数消耗一次（无限不消耗）；
   *     还有剩余则 countdown = every - 1（间隔 every-1 个不出现的回合）；
   *   - 非出现轮 → countdown 递减；
  ...
  tickTurn() {
    ...
    for (const injection of data.injections) {
      ...
      if (countdown === 0) {
        // 出现轮结束：有限次数消耗一次；无限（null）不消耗、不移除
      ...
      try {
        ...
        if (match) {
          ...
          if (req.method === 'POST' && sub === 'inject') {
            ...
            if (!Number.isInteger(every) || every < 1) throw new Error('every 必须是 ≥1 的整数')
