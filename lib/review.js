...
/**
 * True when the agent's most recent turn was message-triggered. DSH core's
 * turn/start event carries only `{ turn }` (no trigger field); a missing
 * trigger counts as a message turn (see the issue #24 notes inside
 * reviewTurnCounter for why). Shared by the review counter and the write
 * watchdog so both agree on what counts as a "user turn".
 * @param {object} agent - the agent whose session events are scanned.
 * @returns {boolean} whether the latest turn/start was message-triggered.
 */
function lastTurnWasMessage(agent) {
  ...
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    if (event?.type === 'turn/start') {
      ...
      return triggerKind === undefined || triggerKind === 'message'
    }
  }
  return false
}
...
export function writeGapCounter(ctx, isEnabled) {
  /** agentId → completed message turns since that session's last daily/project write. */
  const gaps = new Map()
  ...
  const onTurnStopping = (payload) => {
    try {
      ...
      if (!lastTurnWasMessage(agent)) return
      const state = gaps.get(agent.id) ?? { turns: 0 }
      state.turns += 1
  ...
  return {
    gapOf: (agent) => gaps.get(agent?.id)?.turns ?? 0,
    noteWrite: (agent) => { if (agent?.id) gaps.delete(agent.id) },
  }
}
