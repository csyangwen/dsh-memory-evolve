...
export function buildReviewPrompt(digest, config, store) {
  ...
  const trackInstruction = `记忆写入通道（全局轨，严格）：
...
**格式纪律（全局轨）：不要在内容中自行添加任何时间/日期前缀（如 "[2026-08-05]"）——你无法确知当前日期，程序会自动添加准确的时间戳，直接从内容主体开始写。**`
...
1. 全局事实：根据要点提取，用户透露的个人信息、偏好、沟通/工作方式、期待 → 用 ${config.suggestToolName} 建议写入 user 轨（target: "user"）；非平凡的环境/技术事实、工具要点、惯例 → 建议写入 memory 轨（target: "memory"）——严格（宁缺毋滥），最多 2 条，没有就不提。
...
export function installReview(ctx, getRuntime, store) {
  ...
  const trigger = (agent) => {
    ...
    // Whitelist: the suggest tool in suggest mode; the memory tool in auto
    // mode (global writes go through user approval); the skill tool when the
    // skill track is enabled; plus — when the dsh-session-search plugin
    // registered it — the session reader so the reviewer can trace into the
    // full conversation. Everything else is invisible AND refused inside the
    // review child. (An unknown name in the allow list would fail the spawn,
    // hence the existence check.) Daily/project writes are NOT part of the
    // review round: the main session performs them every turn.
    const allow = []
    if (runtime.reviewMode === 'auto' || runtime.autoApproveGlobal) allow.push(runtime.toolName)
