...
export function archiveSuggestions(archive, queue, indices) {
  return queue.mutate((entries) => {
    ...
    entries.forEach((entry, index) => {
      ...
      const stamped = `[${todayStamp()}] ${entry.content}${entry.reason ? `\n（归档理由：${entry.reason}）` : ''}`
      const outcome = archive.append(entry.target, stamped)
      if (outcome.ok) {
 ...
 * re-stamps it with the current date). The archived entry is removed on
 * success.
 * @param {import('./store.js').MemoryStore} store - the memory store.
 * @param {import('./store.js').ArchiveStore} archive - the archive store.
 * @param {string} target - 'memory' | 'user'.
 * @param {string} match - a substring uniquely identifying one archived entry.
 * @returns {{ok: boolean, message: string}} the outcome.
 */
export function promoteArchived(store, archive, target, match) {
  const entries = archive.entriesOf(target)
  ...
  archive.remove(target, match)
  return { ok: true, message: `已转正写入 ${target}（${content.length} 字符），归档条目已移除` }
}
 ...
 */
export function reviewCommand(config, store, archive, queue) {
