...
export function installReview(ctx, getRuntime, store) {
  ...
  const trigger = (agent) => {
    ...
    const slice = Array.isArray(events) && fromSeq > 0 ? events.slice(fromSeq) : events
    const digest = buildDigest({ events: slice }, {
    ...
    // not grow without bound on repeated reviews of the same content.
    if (Array.isArray(events)) lastReviewed.set(agent.id, events.length)
