...
3. 全局事实：用户透露的个人信息、偏好、沟通/工作方式、期待 → 用 ${config.suggestToolName} 建议写入 user 轨（target: "user"）；非平凡的环境/技术事实、工具要点、惯例 → 建议写入 memory 轨（target: "memory"）——严格，最多 2 条，没有就不提。
...
export function installReview(ctx, getRuntime, store) {
  ...
  const trigger = (agent) => {
    ...
    const digest = buildDigest(agent.session, {
      ...
      includeToolOutput: runtime.reviewDigestIncludeToolOutput,
    })
    if (!digest) {
      release()
      return false
