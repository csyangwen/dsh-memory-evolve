 ...
 */
export function approveSuggestions(store, todoStore, queue, indices, agent, edits) {
  return queue.mutate((entries) => {
    const kept = []
    const lines = []
    entries.forEach((entry, index) => {
      ...
      const outcome = isTodo
        ? todoStore.addTodo(entry.target.slice(5), content, {}, entry.cwd ?? agent?.session?.header?.cwd)
        : store.add(entry.target, content, writeAgent)
      if (outcome.ok) {
        lines.push(`✓ #${number} [${entry.target}] 已写入${isTodo ? '待办' : '记忆'}`)
