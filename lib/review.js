 ...
 * @param {string | undefined} cwd - project cwd (required for 'key').
 * @returns {{ok: boolean, message: string}} the outcome.
 */
export function promoteArchived(store, archive, target, match, cwd) {
  ...
  const outcome = store.add(target, content, writeAgent)
