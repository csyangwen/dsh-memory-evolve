...
import { installNotify, installFeishuSend } from './notify.js'
...
const DEFAULTS = {
  ...
  notifyEnabled: false,
  // 飞书直发（de_feishu_send）：**独立子模块**（2026-08-10 用户拍板——与
  // notify 同一纪律：独立领域独立开关）。AI 主动发送文本/图片/文件到飞书
  // （DSH→飞书单向，不带「非对话」通知标注）。渠道能力同样来自 dsh-feishu
  // 的 globalThis 注册表（sendMedia 槽位，插件版本需支持附件）。**默认开启**
  // （用户拍板要的功能，开箱即用；与 notifyEnabled 互不影响：直发 vs 通知，
  // 语义不同、开关粒度独立）。
  feishuSendEnabled: true,
}
...
export const RUNTIME_KEYS = [
  ...
  'notifyEnabled', 'feishuSendEnabled',
]
 ...
 */
function memoryTool(ctx, config, store, queue, getRuntime, archive) {
  return {
...
export function apply(ctx, rawConfig = {}) {
  ...
  const applyRuntimePatch = (patch) => {
    ...
    feishuSendCtrl.sync() // 飞书直发随 feishuSendEnabled 即时安装/卸载（独立开关，与 notify 互不影响）
    searchDocsCtrl.sync()
  ...
  // 7.4.1 飞书直发（de_feishu_send）：**独立子模块**（与 notify 同一纪律——
  //     独立领域独立开关；2026-08-10 用户拍板二者都做：通知 + 直发）。
  //     feishuSendEnabled 独立开关（默认**开**，用户拍板要的功能开箱即用）：
  //     开启时注册 de_feishu_send 工具（AI 主动发送文本/图片/文件到飞书，
  //     不带「非对话」通知标注）；关闭时整体卸载。与 notifyEnabled 互不影响。
  let feishuSendDispose = null
  const feishuSendCtrl = {
    sync() {
      const enabled = runtime.feishuSendEnabled === true
      if (enabled && feishuSendDispose === null) {
        feishuSendDispose = installFeishuSend(ctx).dispose
      } else if (!enabled && feishuSendDispose !== null) {
        feishuSendDispose()
        feishuSendDispose = null
      }
    },
  }
  feishuSendCtrl.sync()
  ...
  const uiSettingsCtrl = {
    sync() {
      ...
      if (enabled && uiSettingsDispose === null) {
        ...
        uiSettingsDispose = installed.dispose
