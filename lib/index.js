...
export function renderSnapshot(config, store, agent) {
  ...
  const writeSection = writeTargets.length > 0
    ? (isSubagent
        ? `写入要求（子代理）：在输出最终回复的消息中，**先写出完整文本，然后在同一条消息的文本之后附带工具调用**——仅在完成**独立成果**时（一项实质产出、一个关键决策或踩坑结论），用 memory 工具向对应轨写入 1 条，保持简洁；没有独立成果就跳过，不要为写而写。
${writeTargets.join('\n')}`
        : `写入要求（重要）：**执行时机：回合的最后一步**——在你输出最终回复的那条消息中：**先写出完整的回复文本，然后在同一条消息的文本之后附带工具调用**（系统会在你的回复文本之后执行这些工具，不会改变回复内容）。**严禁先调用工具再写回复**：
...
**执行时机：回合的最后一步**——在你输出最终回复的那条消息中：**先写出完整的回复文本，然后在同一条消息的文本之后附带工具调用**，按顺序执行（先写入、后审查）：
1. 如本回合有实际产出，先按「按需记忆」要求用 memory 工具写入 project/daily（本回合做了什么）；
2. 调用 memory_review_status（action=check）查询是否到期；
3. 未到期（due=false）→ 不做任何审查动作（**也不要调用 complete**——complete 只用于完成审查后复位）；
4. 到期（due=true）→ **静默执行审查**（全部为工具操作，不要写进最终回复）：
...
5. 格式纪律（全局轨）：不要在建议/写入内容中自行添加时间/日期前缀（如 "[2026-08-05]"）——你无法确知当前日期，程序会自动添加准确的时间戳。
工具执行完毕后直接结束回合（只输出极简确认如"✅"，不要重复正文）。`)
  }
  return parts.join('\n\n')
}
