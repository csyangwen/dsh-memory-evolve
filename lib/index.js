...
export function apply(ctx, rawConfig = {}) {
  ...
  const buildRunningSnapshot = () => {
    ...
    // 按工作区 id 统计 running 数（sessionIds 精确归属）。
    ...
    for (const agent of roots) {
      ...
      else runningByWorkspace.set(owner.workspaceId, (runningByWorkspace.get(owner.workspaceId) ?? 0) + 1)
    }
    const groups = []
    for (const w of workspaces) {
      const running = runningByWorkspace.get(w.workspaceId) ?? 0
      groups.push({ title: w.title, workspaceId: w.workspaceId, running })
    }
