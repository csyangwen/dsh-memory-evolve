...
export function renderSnapshot(config, store, agent) {
  ...
  const writeSection = writeTargets.length > 0
    ? (isSubagent
        ? `写入要求（子代理）：在完成全部工作、输出最终回复之前，仅在完成**独立成果**时（一项实质产出、一个关键决策或踩坑结论），用 memory 工具向对应轨写入 1 条，保持简洁；没有独立成果就跳过，不要为写而写。
${writeTargets.join('\n')}`
        : `写入要求（重要）：**这是回合的最后一步**——先完成用户的全部请求并写好最终回复；在输出最终回复之前的最后一个动作，调用一次 memory 工具检查并记录本回合。**严禁提前执行**（它记录的是本回合刚完成的工作，回合开头没有任何内容可记）：
...
**执行时机：回合的最后一步**——先完成用户的全部请求并写好最终回复；在输出最终回复之前，与「按需记忆」的写入检查一起按顺序执行（先写入、后审查）：
