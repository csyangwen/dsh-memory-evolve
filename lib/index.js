...
export const RUNTIME_KEYS = [
  ...
  'searchDocsEnabled', 'coiEnabled', 'promptsEnabled',
]
...
${tail}${dueWarning}`)
  ...
  // 时文本不变、不追加尾部（缓存友好）。coiEnabled=false 或数据缺失时
  // 整个段消失（零开销）。
...
export function buildCoiSnapshotBlock(config) {
  try {
    ...
    for (const t of running) {
      const mins = Math.max(1, Math.round((now - (t.startedAt ?? now)) / 60000))
      lines.push(`- ⏳ ${t.status === 'queued' ? '排队中' : '运行中'}：${t.id} · ${t.coi ?? t.adapterId} · ${short(t.prompt, 40)}（已 ${t.status === 'queued' ? '排队' : '运行'} ${mins} 分钟）`)
    }
    for (const t of done) {
      const mark = t.status === 'completed' ? '✅' : '❌'
      const summary = short(t.summary, 120)
      lines.push(`- ${mark} ${t.status}：${t.id} · ${t.coi ?? t.adapterId} · ${short(t.prompt, 30)}${summary !== '' ? ` → ${summary}` : ''}`)
    }
    if (lines.length === 0) return null
    return `## COI 任务状态（de_coi 后台任务：完成后摘要自动出现在这里，无需轮询查询；完整输出见「CLI调度」Tab 任务详情）\n${lines.join('\n')}`
  } catch {
