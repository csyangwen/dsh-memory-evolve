...
export const inject = ['tools', 'systemPrompt', 'agents', 'workspace', 'sessionTitle', 'settings', 'llm']
...
export const RUNTIME_KEYS = [
  ...
  'sessionSearchEnabled', 'sessionEnabled',
]
...
export function apply(ctx, rawConfig = {}) {
  ...
  // 3b. 模型配置模块（de_models 工具 + 「模型配置」Tab 数据面）：常驻无
  //     开关——纯本地只读查询 + 插件自有配置（models.json），不占外部资源；
  //     Web API 路由挂在 installApi 的 handler 下（deps.modelsStore）。
  const modelsModule = installModels(ctx, config)
  ...
  ctx.inject(['httpServer'], (webCtx) => {
    ...
    webCtx.effect(() => installApi(webCtx, {
      ...
      modelsStore: modelsModule.store,
      buildModelsSnapshot: () => buildModelsSnapshotAsync(ctx, modelsModule.store),
