 ...
 */
export function renderSnapshot(config, store, agent, counter, wsCoordStore = null, wsCoordMeta = null, wsCoordArchived = null, sessionTitleService = null, syncLine = '') {
  const parts = []
    ...
    ctx.effect(() => ctx.systemPrompt.context({
      ...
      text: (context) => {
        ...
        const syncLine = runtime.syncEnabled === true
          ...
          : ''
        return renderSnapshot(runtime, store, context.agent, counter, wsCoordStoreRef, wsCoordMetaRef, wsCoordArchivedRef !== null ? wsCoordArchivedRef() : null, ctx.sessionTitle, syncLine)
      },
  ...
  const notifyCtrl = {
    sync() {
      ...
      if (enabled && notifyDispose === null) {
        const installed = installNotify(ctx, { memoryDir: config.memoryDir })
