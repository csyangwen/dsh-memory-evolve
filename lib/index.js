...
- 待办（dtodo）：收尾时调用 dtodo list 检查到期（默认视图：今日到期/逾期优先，最多 8 条）——有到期未完成项就在回复末尾提醒用户；用户口述"记住/我要做 X"时用 dtodo add 直写——**用户明确说了类别（工作/生活/项目/每日）就用对应 target**（如"工作上的事"→ target=work），没说才用缺省：有工作目录用 project，无 cwd 用 work；不要主动展开全部待办清单，除非用户询问；查每日过往待办（昨天及更早）用 dtodo list past=true，要含已过期的遗留再加 expired=true（默认不含过期、不增加信息负担）；审查/工作中自建待办用 memory_suggest target=todo-* 进待确认队列，经用户确认后生效。`)
