...
export const DEFAULTS = {
  ...
  // 写入看门狗（2026-08-31）：长会话指令稀释——固定提示词逐渐失效，模型
  // 连续多轮不写 daily/project 且程序侧毫无反馈，遗漏被静默吞掉。开启后
  // 程序按会话统计"连续完成多少个用户回合未写 daily/project"，缺口达到
  // writeGuardThreshold 时快照注入置顶提醒（粘性，写入即消）；threshold=2
  // 意为容忍 1 轮遗漏、第 2 轮起提醒。运行时可调（记忆 Tab / API）。
  perTurnWriteGuard: true,   // false = 关闭看门狗（不计数、不提醒）
...
${st('snap.readHint')}${branchHint}${todoEnabled ? `\n${st('snap.todoHint')}` : ''}`)
  ...
  if (writeTargets.length > 0 || keyDuty || reviewOn) {
    ...
    const writeWarning = writeGapDue
      ? st('snap.writeGuardWarning', { threshold: writeGuardThreshold })
      : ''
