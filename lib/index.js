...
import { ArchiveStore, MemoryStore, SuggestionQueue, extractEntryDate, gitBranch, gitBranchList, parseEntryBranches, todayStamp } from './store.js'
  ...
  if (writeTargets.length > 0 || keyDuty || reviewOn) {
    ...
    if (writeTargets.length > 0 || keyDuty) {
      if (isSubagent) {
        ...
        if (writeTargets.length > 0) {
          duties.push('本回合真人用户输入有明显情绪（正面/负面）时，各条目带 feedback 参数（sentiment/category/quote/note，程序自动生成【反馈】行）——daily 的 category 写通用分类（如 编程/后端/数据库，至少一级可到三级），project 的 category 写本项目内分层（如 记忆模块/写入链路，按项目实际结构）；中性任务指令或其他会话 AI 消息不带 feedback')
        }
