...
import { MemoryStore, SuggestionQueue, parseEntries } from './store.js'
 ...
 */
function memoryTool(ctx, config, store) {
  return {
    ...
    async execute(args, exec) {
      ...
      if (origin === 'subagent' && (target === 'memory' || target === 'user')) {
        if (config.reviewMode !== 'auto') {
          return {
            ok: false,
            message: `后台/子代理写入全局记忆被拒绝：请改用 ${config.suggestToolName} 提出建议（项目记忆与每日日志可直接写入）`,
            target,
          }
        }
...
export function apply(ctx, rawConfig = {}) {
  ...
  if (config.injectMemory) {
    ctx.effect(() => ctx.systemPrompt.context({
      ...
      text: (context) => renderSnapshot(config, store, context.agent),
    }), 'dsh-memory-evolve: memory snapshot')
  }

  // 2. The memory tool (always registered; subagent writes are gated).
  ctx.effect(() => ctx.tools.register(memoryTool(ctx, config, store)), 'dsh-memory-evolve: memory tool')
  ...
  if (config.reviewEnabled) {
    ...
    reviewApi = installReview(ctx, config, store)
  }

  // 4. Commands: the review command works even with review off (users may
