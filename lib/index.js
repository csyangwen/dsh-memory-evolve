...
export function renderSnapshot(config, store, agent) {
  ...
  // stable hint below keeps the model aware they exist; content is read on
  // demand through the memory tool (target=project / daily).
  parts.push('## 按需记忆\n项目记忆与今日日志可随时用 memory 工具读取（target=project / daily），未注入此处。')
  return parts.join('\n\n')
}
