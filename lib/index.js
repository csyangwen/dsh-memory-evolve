...
export function renderSnapshot(config, store, agent, counter) {
  ...
  if (keyEntries.length > 0) {
    const head = branch !== undefined
      ? `## 项目关键记忆（target=key，已注入；当前分支：${branch}，仅注入匹配分支的条目）`
      : '## 项目关键记忆（target=key，已注入，无需读取）'
  ...
  const branchHint = branch !== undefined
    ...
    : ''
  parts.push(`## 记忆 memory-evolve
- 读取：需要时用 memory 工具读取 target=project（项目约定/进展）与 target=daily（今日日志），不要凭猜测回答。项目关键记忆（target=key）已注入上下文，无需读取。${branchHint}
- 待办（dtodo）：收尾时调用 dtodo list 检查到期（默认视图：今日到期/逾期优先，最多 8 条）——有到期未完成项就在回复末尾提醒用户；用户口述"记住/我要做 X"时用 dtodo add 直写——**用户明确说了类别（工作/生活/项目/每日）就用对应 target**（如"工作上的事"→ target=work），没说才用缺省：有工作目录用 project，无 cwd 用 work；不要主动展开全部待办清单，除非用户询问；审查/工作中自建待办用 memory_suggest target=todo-* 进待确认队列，经用户确认后生效。`)
...
function memoryTool(ctx, config, store, queue, getRuntime) {
  return {
    ...
    description: '读写长期记忆（跨会话持久，随上下文快照对模型可见）。target=memory 存全局环境/项目事实，target=user 存用户事实，target=project 存当前工作目录的项目日志（仅当前项目会话可见），target=key 存当前项目的关键长期记忆（自动注入上下文，仅当前项目会话可见；支持 branches 限定 git 分支范围，缺省=全部；**写入需用户确认**：add 会进入待确认队列，确认后生效），target=daily 追加今日日志（按需读取，不注入）。add 追加条目；replace 用唯一子串片段替换整个条目；remove 用唯一子串片段删除条目；list 查询条目——支持 filter（关键词过滤）、since/until（日期范围 YYYY-MM-DD，daily 可跨文件查历史日志）、limit（最多条数，配合 recent 取最近 N 条）、recent（最新在前）、branch（key 轨：只返回该分支可见的条目）。写入立即落盘，模型上下文将在下一次刷新时更新。',
    parameters: {
      type: 'object',
      properties: {
        action: {
...
export function apply(ctx, rawConfig = {}) {
  ...
  ctx.inject(['httpServer'], (webCtx) => {
    ...
    webCtx.effect(() => installApi(webCtx, {
      store, archive, queue, todoStore, getRuntime, updateRuntime, resolveRevealTarget: resolveReveal, revealPath,
      config,
