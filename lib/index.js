...
export function renderSnapshot(config, store, agent) {
  ...
  // the stable hint below (fixed text, never varies) requires the model to
  ...
  // current without waiting for a review round. Subagent sessions get a
...
格式纪律：不要在内容中自行添加任何时间/日期前缀（如 "[2026-08-05]"、"[00:00]"）——你无法确知当前日期，程序会自动添加准确的时间戳，直接从内容主体开始写。`
    : `## 按需记忆（project / daily：每回合主动记录）
以下两轨不注入本会话（为缓存与隔离），需要时请主动用 memory 工具读取（action=list），不要凭猜测回答：
- target=project：当前项目的约定、进展、历史决策、踩坑记录——任务涉及项目上下文、或对项目情况不确定时，先读取再动手/回答；
- target=daily：今天做了什么、当天进展——用户问起今天/最近做了什么、或需要当天信息时读取。

写入要求（重要）：**每个回合结束前主动检查一次**本回合是否产生了值得记录的新事实（关键决策、实质进展、完成事项、踩坑教训、用户透露的偏好或事实）。有 → 立即用 memory 工具写入对应轨（项目相关 → target=project；当天进展 → target=daily；与已有条目同主题用 replace 更新、全新事实用 add），1 条左右、保持简洁，无需用户确认；**没有 → 明确跳过，不要为写而写**（不要记录"继续处理 X"这类无信息量流水）。
