...
import { installCoi } from './coi/index.js'
...
export const RUNTIME_KEYS = [
  ...
  'searchDocsEnabled', 'coiEnabled', 'promptsEnabled', 'scratchEnabled',
]
...
${tail}${dueWarning}`)
  ...
  // 下一次生成时自动获知结果，**无需轮询、无需阻塞等待**。只注入运行中
  // （最多 3 条）与最近终态（最多 2 条）各一行，保持克制；任务状态无变化
  // 时文本不变、不追加尾部（缓存友好）。**按查看者过滤**（与任务列表同
  // 规则）：临时/会话=仅本会话发起的；项目=本会话工作区（ownerCwd，旧
  // 任务回退任务 cwd）派发的；全局=全显。coiEnabled=false 或数据缺失时
  // 整个段消失（零开销）。文案面向 AI（模型消费），不含 GUI 指引。
 ...
 * **一次性通知**：终态任务只注入一次（注入后写回 notified 标记，之后不再
 * 重复出现——模型没收到就靠 de_coi_status 查询）；运行中任务每次显示。
...
export function buildCoiSnapshotBlock(config, viewer = {}) {
  try {
    ...
    const running = tasks
      ...
      .slice(0, 3)
    // 终态只取「未通知过」的（notified !== true）——一次性通知
    const done = tasks
      .filter((t) => visible(t) && t.status !== 'running' && t.status !== 'queued' && t.notified !== true)
      .sort((a, b) => (b.finishedAt ?? 0) - (a.finishedAt ?? 0))
      .slice(0, 2)
    const lines = []
    for (const t of running) {
      // 运行中行是固定文本（不含耗时）——快照文本只在任务状态变化时改变，
      // 否则"已运行 N 分钟"每轮都变、整个快照每回合重注入（缓存浪费）
      lines.push(`- ⏳ ${t.status === 'queued' ? '排队中' : '运行中'}：${t.id} · ${t.coi ?? t.adapterId} · ${short(t.prompt, 40)}`)
    }
    for (const t of done) {
      // 终态行：完整摘要（1KB，不截断）用 4 反引号围栏包裹，让 AI 明确
      // 这是完整摘要内容（摘要里若含 4 连反引号则降级为 3 连，防破坏围栏）
      const mark = t.status === 'completed' ? '✅' : '❌'
      const head = `- ${mark} ${t.status}：${t.id} · ${t.coi ?? t.adapterId} · ${short(t.prompt, 40)}`
      const raw = cleanSummary(t.summary)
      const summary = raw.replace(/````/g, '```')
      lines.push(summary !== '' ? `${head}\n\`\`\`\`${summary}\n\`\`\`\`` : head)
    }
    // 展示过的终态任务标记 notified 并落盘（一次性通知；写失败忽略，下轮重试）
    if (done.length > 0) {
      try {
        for (const t of done) t.notified = true
        const payload = Array.isArray(parsed) ? tasks : { ...parsed, tasks }
        writeFileSync(file, JSON.stringify(payload, null, 2) + '\n')
      } catch { /* 通知标记写失败不影响本轮展示 */ }
    }
    if (lines.length === 0) return null
    return `## COI 任务状态（CLI 后台代理任务：完成后自动通知一次，无需轮询查询；摘要为输出尾部，完整输出用 de_coi_status 查询）\n${lines.join('\n')}`
  } catch {
 ...
 * @param {object} [opts] - { cwd, branch } 任务工作目录与分支。
 * @returns {string} 拼接好的上下文文本（无内容返回空串）。
 */
export function buildMemoryContext(store, { cwd, branch: declaredBranch } = {}) {
  const parts = []
  const memoryEntries = store.entriesOf('memory')
