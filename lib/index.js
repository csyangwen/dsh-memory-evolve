...
import { installMemorySync, makeProjectDirResolver, renderSyncLine } from './sync/index.js'
 ...
 */
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null, syncLine = '') {
  const parts = []
...
${tail}${dueWarning}`)
  ...
  // 工作区活动感知（ws-coord）：2026-08-13 用户拍板——**移出整体快照**。
  // DSH 快照按整体文本 diff 注入，公告板一行变化会连带记忆/纪律等其他段
  // 一起作为新尾部重注入（AI 被迫重看已注入多次的无关段，噪声）；且归档
  // 调研确认 DSH 核心不支持逐段投影，插件无法让单段独立变更检测。改为
  // ws-coord 模块内部状态变化检测 + 向工作区会话投递独立消息（inject 不
  // 唤醒，内容与旧公告板行一致），见 lib/coi/ws-coord.js installWsCoord。
  // 记忆同步状态行（施工图 §6；文本状态驱动稳定，变化才变）
  if (typeof syncLine === 'string' && syncLine !== '') parts.push(syncLine)
  return parts.join('\n\n')
}
...
export function apply(ctx, rawConfig = {}) {
  ...
  if (config.injectMemory) {
    ctx.effect(() => ctx.systemPrompt.context({
      ...
      text: (context) => {
        ...
        // 记忆同步状态行：仅 syncEnabled 时计算（未初始化项目返回空串）
        const syncLine = runtime.syncEnabled === true
          ? renderSyncLine(runtime, context.agent?.session?.header?.cwd)
          : ''
        return renderSnapshot(runtime, store, context.agent, counter, ctx.sessionTitle, syncLine)
      },
  ...
  // 7.9 项目记忆跨设备同步（/memory_sync）：**独立子模块**（施工图 §7 第 6
  //     步；独立领域独立开关纪律——syncEnabled 默认关）。开启时安装命令组
  //     + 快照状态行 + API；关闭时整体卸载（记忆目录/仓库全保留）。store 的
  //     entryIdMode 随开关动态切换（仅项目轨受影响，未启用零变化）。
  const syncCtrl = {
    sync() {
      const enabled = runtime.syncEnabled === true
      store.entryIdMode = enabled ? 'on' : 'off'
      // 命令组**始终注册**（2026-08-11 实测反馈修复）：syncEnabled=false 时
      // /memory_sync 也要可用——status 提示未启用、setup 引导初始化（并自动
      // 打开开关）。与 memory_review 命令"works even with review off"同款
      // 惯例（slash command 不占模型工具列表，无成本）。快照状态行由
      // renderSnapshot 按 syncEnabled 控制（未启用不显示）。
      if (syncDispose === null) {
      ...
      if (advisorRef === null) {
        const installed = installAdvisor(ctx, advisorConfigOf(), {
          ...
          sessionName: (sessionId) => {
            ...
            return alias ?? ctx.sessionTitle?.get?.(agent?.session)?.title ?? null
          },
