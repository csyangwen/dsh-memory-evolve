...
export function memoryTool(ctx, config, store, queue, getRuntime, archive, writeGap = null) {
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      // U+FFFD 写入检测（store.write 记录、随写随报、消费一次）：损坏字符来自
      // 上游写入链路的有损字节→文本解码，写它的会话当场看到才能当场修复。
      const corruptWrite = store.takeCorruptWrite?.()
      if (corruptWrite && result?.ok) result.message += st('store.replacementChars', corruptWrite)
      return result
    },
  }
}
