 ...
 *   AGENTS.md（全局规则）+ 长期记忆 + 用户档案（所有任务注入）；
 *   项目关键记忆仅在有 cwd 时注入，且按 git 分支过滤（与 keyBranchFilter
 *   一致：只注入无标记或覆盖当前分支的条目）。项目日志/每日日志不注入
 *   （流水太长，与 DSH 快照策略一致）。
 * @param {MemoryStore} store - 记忆 store。
 * @param {object} [opts] - { cwd, branch } 任务工作目录与分支。
 * @returns {string} 拼接好的上下文文本（无内容返回空串）。
 */
export function buildMemoryContext(store, { cwd } = {}) {
  const parts = []
  try {
    const dshHome = process.env.DSH_HOME || join(homedir(), '.dsh')
    const agentsFile = join(dshHome, 'AGENTS.md')
    if (existsSync(agentsFile)) {
      const text = readFileSync(agentsFile, 'utf8').trim()
      if (text !== '') parts.push(`【全局规则 AGENTS.md】\n${text}`)
    }
  } catch { /* 读取失败忽略 */ }
  const memoryEntries = store.entriesOf('memory')
