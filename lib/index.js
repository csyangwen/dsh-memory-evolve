...
export const RUNTIME_KEYS = [
  ...
  'reviewProvider', 'reviewModel', 'autoApproveGlobal',
]
...
const BOOLEAN_KEYS = [
  ...
  'entryDatePrefix', 'reviewFinalOnDispose',
]
const STRING_KEYS = [
 ...
 * slow-moving tracks (global memory/user, optional read-only Hermes) are
 * rendered here: project memory and the daily log change on every review,
...
export function renderSnapshot(config, store, agent) {
  ...
  // stable hint below (fixed text, never varies) keeps the model aware they
  // exist; content is read on demand through the memory tool.
  parts.push('## 按需记忆\n涉及当前项目约定、进展或今日动态时，可用 memory 工具按需读取：target=project（当前项目）、target=daily（今日日志）。二者不注入此处。')
  return parts.join('\n\n')
}
