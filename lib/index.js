...
export const RUNTIME_KEYS = [
  ...
  'memoryTabEnabled', 'perTurnProjectWrites', 'perTurnDailyWrites', 'perTurnKeyWrites',
]
...
export function renderSnapshot(config, store, agent, counter) {
  ...
  const keyEntries = keyAgent ? store.entriesOf('key', keyAgent) : []
  if (keyEntries.length > 0) {
    parts.push(`## 项目关键记忆（target=key，已注入，无需读取）\n${keyEntries.map((entry) => `- ${entry}`).join('\n')}`)
  }
...
- 读取：需要时用 memory 工具读取 target=project（项目约定/进展）与 target=daily（今日日志），不要凭猜测回答。项目关键记忆（target=key）已注入上下文，无需读取。`)
  ...
  if (writeTargets.length > 0 || keyDuty || reviewOn) {
    ...
    if (writeTargets.length > 0 || keyDuty) {
      if (isSubagent) {
        ...
        steps.push(keyDuty
          ? `${base}；重要结论可另写 target=key；没有独立成果就跳过，不要为写而写。`
        ...
        if (keyDuty) duties.push('本轮出现重要项目事实（长期约定/决策/架构/踩坑）时另向 target=key 写 1 条，没有则跳过')
...
function memoryTool(ctx, config, store, getRuntime) {
  return {
    ...
    description: '读写长期记忆（跨会话持久，随上下文快照对模型可见）。target=memory 存全局环境/项目事实，target=user 存用户事实，target=project 存当前工作目录的项目日志（仅当前项目会话可见），target=key 存当前项目的关键长期记忆（自动注入上下文，仅当前项目会话可见），target=daily 追加今日日志（按需读取，不注入）。add 追加条目；replace 用唯一子串片段替换整个条目；remove 用唯一子串片段删除条目；list 查询条目——支持 filter（关键词过滤）、since/until（日期范围 YYYY-MM-DD，daily 可跨文件查历史日志）、limit（最多条数，配合 recent 取最近 N 条）、recent（最新在前）。写入立即落盘，模型上下文将在下一次刷新时更新。',
    parameters: {
      type: 'object',
      properties: {
        action: {
    ...
    async execute(args, exec) {
      ...
      try {
        switch (action) {
          case 'list': {
            const stats = {}
            const entries = store.query(target, exec.agent, {
              filter: args.filter,
              since: args.since,
              until: args.until,
              limit: args.limit,
