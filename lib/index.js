...
export function apply(ctx, rawConfig = {}) {
  ...
  //    左侧会话列表「仅显示进行中」筛选）。纯客户端实现（CSS + DOM 增强，
  //    注入逻辑在 src/client/session-filter.ts），宿主端只提供独立开关
  //    uiSettingsEnabled（默认关）与状态探测端点 GET /api/ui-settings/state
  //    （关闭时 404，客户端探测失败即不注入任何东西）。开关在「设置」Tab
  //    的「配置」里切换（applyRuntimePatch sync 链即时安装/卸载）。
