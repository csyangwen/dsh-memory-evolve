  ...
  switch (key) {
    ...
    case 'searchDocsMode':
      if (key === 'searchDocsMode') {
        if (!['all', 'filename', 'content', 'off'].includes(value)) {
          throw new Error('dsh-memory-evolve: searchDocsMode 必须是 all / filename / content / off 之一')
        }
        return
      }
 ...
 */
export function renderSnapshot(config, store, agent, counter, wsCoordStore = null, wsCoordMeta = null) {
  const parts = []
  ...
  // 会话别名（用户设置，≤10 字）：有别名时注入"别名 + ID"——AI 不仅
  // 知道自己的 ID 还知道自己的友好名称（拟人化，广播协作时识别对方更
  // 直观）；别名与 ID 都会在消息比对时用到。
  if (agent?.session?.id) {
    const aliases = readAliases(config.memoryDir ?? '')
    const alias = aliases[agent.session.id]
    if (alias) {
      parts.push(`## 你的会话（记住它们：用别名/ID 与各模块消息里的 session id 比对判断是谁；回复时把别名与 ID 告知对方）\n- 你的会话别名：${alias}\n- 你的会话 ID：${agent.session.id}`)
    } else {
...
${tail}${dueWarning}`)
  ...
  if (config.wsCoordEnabled === true && wsCoordStoreRef !== null && wsCoordMetaRef !== null) {
...
export function apply(ctx, rawConfig = {}) {
  ...
  if (config.injectMemory) {
    ctx.effect(() => ctx.systemPrompt.context({
      ...
      text: (context) => renderSnapshot(getRuntime(), store, context.agent, counter, wsCoordStoreRef, wsCoordMetaRef),
