 ...
 * standalone dsh-skill-browser plugin (~/.dsh/plugins/dsh-skill-browser/
 * state.json) is imported once, so uninstalling the standalone plugin does
 * not reset the user's enable/disable choices.
 *
...
/** Default legacy state of the standalone dsh-skill-browser plugin. */
const DEFAULT_LEGACY_STATE_FILE = join(homedir(), '.dsh', 'plugins', 'dsh-skill-browser', 'state.json')
 ...
 * import the standalone dsh-skill-browser's state once (so the merged plugin
 * keeps the user's enable/disable choices) and persist it immediately.
 */
function loadState(stateFile, legacyStateFile) {
  const own = readStateFile(stateFile)
  if (own !== null) return own
  const state = { disabled: [], customDirs: [] }
  const legacy = readStateFile(legacyStateFile)
 ...
 * @param {string} [options.legacyStateFile] - one-time import source (standalone plugin).
 */
export function installSkillsManager(ctx, options = {}) {
  const stateFile = options.stateFile
  const legacyStateFile = options.legacyStateFile ?? DEFAULT_LEGACY_STATE_FILE
    ...
    skillCtx.inject(['httpServer', 'workspace'], (webCtx) => {
      const handler = async (req, res) => {
        ...
        if (pathname === '/skills-manager/api/skills' && req.method === 'GET') {
          try {
            let cwd = query.get('cwd') || undefined
            if (cwd === undefined) {
        ...
        if (pathname === '/skills-manager/api/browse' && req.method === 'GET') {
          try {
            const { roots } = await collectRoots(webCtx, query.get('cwd') || undefined)
