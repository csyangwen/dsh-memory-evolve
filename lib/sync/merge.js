...
import { ensureEntryIds, extractEntryId } from './entryid.js'
...
export function mergeEntries(baseFiles, oursFiles, theirsFiles) {
  ...
  const index = (files) => {
    ...
    for (const [location, rawEntries] of Object.entries(files ?? {})) {
      // 补发先行：无 ID 条目获得确定性 ID（双设备一致），再建索引
      const { entries } = ensureEntryIds(rawEntries ?? [])
      for (const text of entries) {
        const key = extractEntryId(text) ?? text // 有 ID 按 ID，无 ID 整条兜底
  ...
  for (const key of keys) {
    ...
    if (decision.kind === 'keep') push(decision.location, decision.text)
  ...
  return { files: out, conflicts, removed }
}
