 ...
 * 元数据文件（PROVENANCE/.gitignore/CONFLICTS.md）所有文件集通用；
 * 各文件集的记忆文件由 resolveFilesetFiles 动态展开（全局轨二期并入一期）。
 */
export const STAGE_META = ['PROVENANCE', '.gitignore', 'CONFLICTS.md']
...
export function resolveFilesetFiles(dir, fileset = 'project') {
  ...
  const addDirFiles = (sub, suffix, list, excludeTodo = false) => {
    const d = join(dir, sub)
    if (!existsSync(d)) return
    for (const name of readdirSync(d)) {
      if (!name.endsWith(suffix)) continue
      if (excludeTodo && name.endsWith('.todo.md')) continue
      list.push(`${sub}/${name}`)
    }
  }
  for (const p of spec.memory) {
    if (p === 'daily') addDirFiles('daily', '.md', out.memory, true)
 ...
 */
export async function stagePaths(dir, fileset = 'project') {
  ...
  const tracked = await runGit(dir, ['ls-files', '--', ...filePaths, ...STAGE_META])
...
export async function ensureGlobalRepo({ dir, url }) {
  ...
  const gitignoreContent = [
    ...
    '!.gitignore', '!PROVENANCE', '!CONFLICTS.md',
  ...
  if (!existsSync(provenancePath)) {
    const key = normalizeRemoteUrl(url) ?? url
    const globalId = createHash('sha1').update(key).digest('hex').slice(0, 12)
    const meta = {
      projectId: globalId, displayName: url, version: PROVENANCE_VERSION, url,
      enabled: true,
  ...
  // ── 7. remote 绑定（幂等；URL 变化 → set-url 切换——全局远端切换后
  //    各轨在旧 URL 上的分支留档，由首次 sync 对账新 URL）──
  const origin = await runGit(dir, ['remote', 'get-url', 'origin'])
  if (!origin.ok || origin.stdout.trim() === '') {
  ...
  } else if (normalizeRemoteUrl(origin.stdout.trim()) !== normalizeRemoteUrl(url)) {
...
export async function readTreeFiles(dir, ref, fileset = 'project') {
  ...
  if (!ls.ok) return { files, provenance, invalid }
  ...
  return { files, provenance, invalid }
}
