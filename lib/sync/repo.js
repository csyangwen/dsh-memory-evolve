...
/**
 * 模式 B 共享记忆仓库分支决策（2026-08-11 用户拍板：一个私有仓库装所有
 * 项目的记忆，每个项目使用专属分支 dsh-shared/<projectId>）。
 *
 * 判定树：
 *   1. 共享分支 dsh-shared/<projectId> 存在 → shared（本项目续接 / 第二台
 *      设备接入）；
 *   2. main 分支不存在 → fresh（全新共享仓库，用共享分支）；
 *   3. main 存在 → 拉取 main 读 PROVENANCE 判定归属：
 *        projectId == 本项目 → legacy（**老单项目仓库**，继续用 main——
 *        一期模式 B 兼容，零迁移）；
 *        其他（别的项目 / 无 PROVENANCE / 损坏）→ shared-fresh（共享仓库
 *        里已有其他项目、或 main 归属不明——本项目用专属分支，绝不触碰
 *        main，防串项目）。
 *
 * 串项目防护：决策只认 PROVENANCE.projectId 精确匹配；后续
 * deviceBConnect 的 expectedProjectId 校验与 worker 的 checkProvenance
 * 再兜底（三层防线）。
 *
 * 全部网络命令（ls-remote/fetch）异步执行、GIT_TERMINAL_PROMPT=0、超时
 * 保护；本函数由 sync-worker 子进程调用（主进程绝不跑网络命令）。失败时
 * 目录可能残留 .git（fetch 前需 init）——无害：deviceBConnect 的非空目录
 * 守卫放行 .git/.gitignore/PROVENANCE/.memory.lock。
 *
 * @param {object} p
 * @param {string} p.dir - 记忆仓库目录（可能尚不存在/未 init）。
 * @param {string} p.remoteUrl - 共享记忆仓库 URL。
 * @param {string} p.projectId - 本项目身份 id（12 hex）。
 * @returns {Promise<{ok: boolean, kind: 'shared'|'fresh'|'legacy'|'shared-fresh'|'error',
 *   branch: string | null, message: string}>}
 */
export async function decideModeBBranch({ dir, remoteUrl, projectId }) {
  mkdirSync(dir, { recursive: true })
  const sharedBranch = sharedBranchFor(projectId)

  // ── 1. 探共享分支：存在 → 本项目在共享仓库里已有分支（续接/第二台设备）──
  const probeShared = await runGit(dir, ['ls-remote', remoteUrl, `refs/heads/${sharedBranch}`], { network: true })
  if (!probeShared.ok) {
    return { ok: false, kind: 'error', branch: null, message: `无法连接共享记忆仓库（${classifyRemoteError(probeShared.stderr)}）：${probeShared.stderr.trim().split('\n')[0] ?? ''}。请检查网络/凭证后重试` }
  }
  if (probeShared.stdout.trim() !== '') {
    return { ok: true, kind: 'shared', branch: sharedBranch, message: `共享仓库已有本项目的专属分支 ${sharedBranch}，直接接入` }
  }

  // ── 2. 探 main：不存在 → 全新共享仓库（或仓库还没有任何记忆分支）──
  const probeMain = await runGit(dir, ['ls-remote', remoteUrl, 'refs/heads/main'], { network: true })
  if (!probeMain.ok) {
    return { ok: false, kind: 'error', branch: null, message: `无法连接共享记忆仓库（${classifyRemoteError(probeMain.stderr)}）：${probeMain.stderr.trim().split('\n')[0] ?? ''}。请检查网络/凭证后重试` }
  }
  if (probeMain.stdout.trim() === '') {
    return { ok: true, kind: 'fresh', branch: sharedBranch, message: `全新共享记忆仓库——本项目使用专属分支 ${sharedBranch}` }
  }

  // ── 3. main 存在：拉取 main 读 PROVENANCE，判断它是本项目的（老单项目
  //      仓库）还是别的项目/仓库默认分支（共享仓库）──
  // fetch 需要仓库：先 init（幂等；后续 ensureMemoryRepo/deviceBConnect
  // 在同一目录再 init 会安全跳过）
  const init = await runGit(dir, ['init', '-q', '-b', 'main'])
  if (!init.ok) {
    const initPlain = await runGit(dir, ['init', '-q'])
    if (!initPlain.ok) {
      return { ok: false, kind: 'error', branch: null, message: `git init 失败：${initPlain.stderr.trim().split('\n')[0] ?? ''}` }
    }
    const sym = await runGit(dir, ['symbolic-ref', 'HEAD', 'refs/heads/main'])
    if (!sym.ok) return { ok: false, kind: 'error', branch: null, message: `无法设置默认分支 main：${sym.stderr.trim().split('\n')[0] ?? ''}` }
  }
  const fetchMain = await runGit(dir, ['fetch', '--no-tags', remoteUrl, 'refs/heads/main:refs/remotes/origin/main'], { network: true })
  if (!fetchMain.ok) {
    // 拉取 main 失败（分支损坏等）：不阻塞——保守走共享分支（后续首次
    // push 与 main 互不干扰）
    return { ok: true, kind: 'shared-fresh', branch: sharedBranch, message: `无法读取远端 main 分支（${fetchMain.stderr.trim().split('\n')[0] ?? ''}）——保守使用本项目专属分支 ${sharedBranch}` }
  }
  let mainProjectId = null
  const show = await runGit(dir, ['show', 'refs/remotes/origin/main:PROVENANCE'])
  if (show.ok) {
    try {
      mainProjectId = JSON.parse(show.stdout.trim()).projectId ?? null
    } catch { /* 损坏视同缺失（归属不明 → 保守共享分支） */ }
  }
  // 清理临时 remote-tracking ref（保持目录状态干净；接入流程会重拉目标分支）
  await runGit(dir, ['update-ref', '-d', 'refs/remotes/origin/main'])
  if (typeof mainProjectId === 'string' && mainProjectId === projectId) {
    return { ok: true, kind: 'legacy', branch: 'main', message: '远端 main 分支是本项目的记忆（老单项目仓库）——继续使用 main，保持兼容' }
  }
  return {
    ok: true, kind: 'shared-fresh', branch: sharedBranch,
    message: mainProjectId
      ? `远端 main 属于其他项目（${mainProjectId}）——本项目使用专属分支 ${sharedBranch}，绝不触碰其他项目的分支`
      : `远端 main 无法确认归属（无本项目 PROVENANCE）——保守使用专属分支 ${sharedBranch}（不触碰 main）`,
  }
}
