...
/** 记忆文件白名单（bootstrap 补发只处理这些；TODOS.md 不在此列——项目
 *  待办有自己的 tag id（[id: xxxx]），不需要行首身份证补发，由合并器按
 *  文件分流处理）。 */
...
export async function stagePaths(dir) {
  ...
  const cmds = []
  if (existing.length > 0) cmds.push(['add', '-f', '--ignore-errors', '--', ...existing])
  // 已跟踪但已删除的白名单文件 → git rm（--ignore-unmatch 防"从未跟踪"误报）
  const tracked = await runGit(dir, ['ls-files', '--', ...STAGE_PATHS])
