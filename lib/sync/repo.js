...
export async function decideModeBBranch({ dir, remoteUrl, projectId }) {
  ...
  for (const legacy of [MODE_A_BRANCH, 'main']) {
    ...
    try {
      await ensureInit()
      const fetch = await runGit(dir, ['fetch', '--no-tags', remoteUrl, `refs/heads/${legacy}:refs/remotes/origin/${legacy}`], { network: true })
      if (fetch.ok) {
        const show = await runGit(dir, ['show', `refs/remotes/origin/${legacy}:PROVENANCE`])
        if (show.ok) {
          try { belongs = JSON.parse(show.stdout.trim()).projectId === projectId } catch { /* 损坏视同缺失 */ }
        }
        // 清理临时 remote-tracking ref（保持目录状态干净；接入流程会重拉）
        await runGit(dir, ['update-ref', '-d', `refs/remotes/origin/${legacy}`])
      }
      // fetch 失败（分支损坏等）→ belongs 保持 false，跳过该候选
    } catch (error) {
 ...
 */
export const STAGE_PATHS = ['KEY.md', 'KEY-archive.md', 'MEMORY.md', 'PROVENANCE', '.gitignore', 'CONFLICTS.md', 'TODOS.md', 'logs/']
...
export async function ensureMemoryRepo({ dir, memoryDir, cwd, projectId, displayName, remoteUrl, remoteBranch = 'dsh-shared/memory' }) {
  ...
  const gitignoreContent = [
    ...
    '!CONFLICTS.md', '!TODOS.md', '!logs/', '!logs/**', '',
  ].join('\n')
...
/**
 * 序列化 TODO 记忆文件（header + § 条目）。与 todo.js 的写盘格式一致
 * （TODO_HEADER 常量），保证同步写回后 todo 解析器仍能正常读取。
 * @param {string[]} entries - 条目数组。
 * @returns {string} 完整文件文本。
 */
export function serializeTodoEntries(entries) {
  return `${TODO_HEADER}${entries.join(ENTRY_DELIMITER)}\n`
}
...
export async function readTreeFiles(dir, ref) {
  ...
  for (const name of names) {
    ...
    const show = await runGit(dir, ['show', `${ref}:${name}`])
    if (show.ok) {
      // TODO 文件用专用解析（剥 header；tag 行 id 由合并器识别）；
      // 普通记忆文件 § 切分（合并器输入）
      files[name] = name === TODO_FILE ? parseTodoEntries(show.stdout) : parseEntries(show.stdout)
    }
  }
