...
import { locateLegacyDir, normalizeRemoteUrl, resolveMainRemote, resolveProjectId } from './identity.js'
import { countConflicts, CONFLICTS_FILE, parseConflicts, resolveConflict } from './worker.js'
import { deviceBConnect, ensureMemoryRepo, sharedBranchFor } from './repo.js'
...
export function projectSyncInfo(config, cwd) {
  ...
  // 统一模式缺省：每项目专属分支 dsh-shared/<projectId>；老 PROVENANCE
  // 记录的 remoteBranch（dsh-shared/memory / main）优先（老配置兼容）
  let remoteBranch = sharedBranchFor(identity.id)
...
export function syncStatusSync(config, cwd) {
  ...
  return {
    ...
    // 轨级开关（第 3 层）：一期唯一轨=项目记忆（KEY/日志/归档/项目待办）；
    // 全局轨二期独立开关（仅共享记忆仓库可用）
    tracks: { project: provenanceTrackProject(dir) },
    // 记忆远端类型（统一模式，2026-08-11）：sync 仓库 origin 与主仓库 remote
    // 归一化相等 = 主代码仓库（默认）；否则 = 共享记忆仓库（用户指定）。
    // UI 状态卡片据此显示"记忆放哪了"。
    remoteKind: (syncOrigin !== null && identity.remoteUrl !== undefined
      && normalizeRemoteUrl(syncOrigin) === normalizeRemoteUrl(identity.remoteUrl)) ? 'main-repo' : 'shared-repo',
    uncommitted,
    behind,
...
export function installMemorySync(ctx, deps) {
  ...
  const registerCommand = () => {
    ...
    commandDispose = ctx.get('commands')?.register?.({
      name: 'memory_sync',
      description: '项目记忆跨设备同步（Git）：setup [url] 初始化（无参=记忆远端用主代码仓库，零配置；url=共享记忆仓库——一个仓库装所有项目的记忆，全局记忆的唯一去处；每项目自动使用专属分支，老配置自动兼容），sync 拉取并合并，sync --push 同步并推送（推送需你显式触发），off 停用同步，status 查看状态，conflict list 列出待处理冲突，conflict resolve <编号> ours|theirs|both 解决冲突，migrate 查看可迁移的旧记忆目录',
...
export async function handleCommand(op, rest, cwd, { config, getRuntime, applyRuntimePatch }) {
  ...
  switch (op) {
    case 'setup': {
      ...
      // 试探地址优先级：显式 url > 已配置的记忆仓库 origin（重复 setup 的
      // 真实传输通道）> 主仓库 identity.remoteUrl（默认零配置）
      const existingOrigin = runGitSync(dir, ['remote', 'get-url', 'origin'])
      const probeUrl = url ?? existingOrigin ?? identity.remoteUrl
      if (!probeUrl) {
      ...
      if (connect.mode === 'adopt') {
        ...
        return { kind: 'success', text: `已接入远端记忆（${branch}）：${connect.message}。执行 /memory_sync sync 拉取合并` }
      }
