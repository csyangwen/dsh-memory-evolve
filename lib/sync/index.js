    ...
    case 'setup': {
      ...
      if (switching) {
        ...
        if (meta !== null) {
          ...
          return { kind: 'success', text: `记忆远端已切换（${probeUrl}，分支 ${branch}）。本地记忆完整保留——执行 /memory_sync sync 完成首次对账（推送需 --push）` }
        }
      ...
      if (connect.mode === 'adopt') {
        ...
        return { kind: 'success', text: `已接入远端记忆（${branch}）：${connect.message}。执行 /memory_sync sync 拉取合并${legacyWarn}` }
      }
