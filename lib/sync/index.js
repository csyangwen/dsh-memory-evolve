...
import { deviceBConnect, ensureMemoryRepo, MODE_A_BRANCH, sharedBranchFor } from './repo.js'
import { GLOBAL_FILESET_KEYS, globalBranchFor } from './filesets.js'
...
function globalSyncStatus(memoryDir) {
  ...
  if (meta === null || !existsSync(join(memoryDir, '.git'))) {
    return { initialized: false, url: '', tracks: {}, uncommitted: 0, remoteKind: 'none' }
  }
  // 全局仓库 origin（绑定 URL）
  const origin = runGitSync(memoryDir, ['remote', 'get-url', 'origin'])
  const dirty = runGitSync(memoryDir, ['status', '--porcelain'])
  const uncommitted = dirty === null ? 0 : dirty.split('\n').filter((l) => l.trim() !== '').length
  return {
    initialized: true,
    url: meta.url ?? origin ?? '',
    tracks: { memory: meta.tracks?.memory === true, user: meta.tracks?.user === true, daily: meta.tracks?.daily === true, todo: meta.tracks?.todo === true },
    uncommitted,
...
export function installMemorySync(ctx, deps) {
  ...
  const registerCommand = () => {
    ...
    commandDispose = ctx.get('commands')?.register?.({
      name: 'memory_sync',
      description: '项目记忆跨设备同步（Git）：setup [url] 初始化（无参=记忆远端用主代码仓库，零配置；url=共享记忆仓库——一个仓库装所有项目的记忆，也是全局记忆的唯一去处；每项目自动使用专属分支，老配置自动兼容），sync 拉取并合并，sync --push 同步并推送（推送需你显式触发），off 停用同步，status 查看状态，global status|on|off <memory|user|daily|todo>|sync [--push] 全局记忆同步（全局记忆/用户档案/每日日志/待办四轨，仅共享记忆仓库可用），conflict list 列出待处理冲突，conflict resolve <编号> ours|theirs|both 解决冲突，migrate 查看可迁移的旧记忆目录',
      input: { syntax: 'setup [url] | sync [--push] | off | status | global status|on|off <memory|user|daily|todo>|sync [--push] | conflict list | conflict resolve <n> ours|theirs|both | migrate', hint: '缺省=status' },
  ...
  return {
    ...
    ops: {
      ...
      setGlobalTrack(track, on) {
        ...
        if (gMeta === null) return { kind: 'error', text: '全局记忆仓库尚未初始化——先用 /memory_sync setup <共享记忆仓库地址> 初始化' }
        const tracks = { ...(gMeta.tracks ?? {}) }
        tracks[track] = on === true
        gMeta.tracks = tracks
        writeFileSync(join(config.memoryDir, 'PROVENANCE'), `${JSON.stringify(gMeta)}\n`)
      ...
      async globalSync(push) {
        ...
        return { kind: 'success', text: `${push ? '全局轨同步并推送完成' : '全局轨同步完成'}：\n${results.join('\n')}` }
      },
    },
  }
}
...
export async function handleCommand(op, rest, cwd, { config, getRuntime, applyRuntimePatch }) {
  ...
  switch (op) {
    case 'setup': {
      ...
      if (!probeUrl) {
        return { kind: 'error', text: '当前项目没有可共享的 git 远端——请先为主仓库配置 remote（或用 /memory_sync setup <url> 指定共享记忆仓库）' }
      }
      ...
      if (switching) {
        ...
        const meta = readProvenance(dir)
        if (meta !== null) {
          if (meta.remoteBranch !== branch) {
            meta.remoteBranch = branch
          ...
          return { kind: 'success', text: `记忆远端已切换（${probeUrl}，分支 ${branch}）。本地记忆完整保留——执行 /memory_sync sync 完成首次对账（推送需 --push）${gNote}` }
        }
      ...
      if (connect.mode === 'adopt') {
        ...
        // **并复位项目级 enabled=true（Kimi P1-5：off 后按官方提示 setup
        // 重新启用，enabled=false 必须复位，否则 sync 仍被拒）**
        applyRuntimePatch({ syncEnabled: true })
        const meta = readProvenance(dir)
        ...
        const legacyWarn = (() => {
          const legacy = locateLegacyDir(config.memoryDir, cwd, identity.id)
          return legacy === null ? '' : `；注意：发现旧记忆目录 ${legacy}（尚未并入，可用 /memory_sync migrate 查看）`
        })()
        const gNote = await initGlobalRepo(config.memoryDir, url)
        return { kind: 'success', text: `已接入远端记忆（${branch}）：${connect.message}。执行 /memory_sync sync 拉取合并${legacyWarn}${gNote}` }
      }
      ...
      // 走 bootstrap）同样复位 enabled（Kimi P1-5）
      applyRuntimePatch({ syncEnabled: true })
      const meta = readProvenance(dir)
      if (meta !== null && meta.enabled === false) {
        meta.enabled = true
    ...
    case 'global': {
      ...
      if (sub === 'on' || sub === 'off') {
        ...
        const gMeta = readProvenance(config.memoryDir)
        if (gMeta === null) {
          return { kind: 'error', text: '全局记忆仓库尚未初始化——先用 /memory_sync setup <共享仓库地址> 初始化' }
        }
        const tracks = { ...(gMeta.tracks ?? {}) }
        tracks[trackName] = sub === 'on'
        gMeta.tracks = tracks
        writeFileSync(join(config.memoryDir, 'PROVENANCE'), `${JSON.stringify(gMeta)}\n`)
      ...
      if (sub === 'sync') {
        const gMeta = readProvenance(config.memoryDir)
        if (gMeta === null) return { kind: 'error', text: '全局记忆仓库尚未初始化——先用 /memory_sync setup <共享仓库地址> 初始化' }
        const push = rest.includes('--push')
        // 逐个启用轨对账（每轨独立分支，互不干扰）；一个失败不阻断其余
        const results = []
        for (const fileset of GLOBAL_FILESET_KEYS) {
          const track = GLOBAL_TRACK_NAMES[fileset]
          if (gMeta.tracks?.[track] !== true) continue
        ...
        if (results.length === 0) return { kind: 'error', text: '未开启任何全局轨——先 /memory_sync global on <memory|user|daily|todo> 开启' }
        const head = push ? '全局轨同步并推送完成' : '全局轨同步完成'
        return { kind: 'success', text: `${head}：\n${results.join('\n')}` }
      }
      if (sub === 'status') {
        const g = globalSyncStatus(config.memoryDir)
        if (!g.initialized) {
          return { kind: 'success', text: '全局记忆同步未初始化——全局记忆（用户档案/每日日志/待办）仅共享记忆仓库可用：执行 /memory_sync setup <共享记忆仓库地址> 初始化' }
        }
        const lines = [
          `全局记忆远端：${g.url}`,
          `全局轨开关：${renderGlobalTracks(g.tracks)}`,
          `未提交变更：${g.uncommitted} 条`,
    ...
    case 'status': {
      ...
      const lines = [
        ...
        `同步范围：${info.tracks.project ? '项目记忆轨（KEY/日志/归档/项目待办）' : '未选择任何轨（该轨退出同步）'}`,
      ...
      if (info.global?.initialized === true) {
        ...
        lines.push(`未提交变更：${info.global.uncommitted} 条`)
        lines.push('提示：/memory_sync global sync 同步全部已开启的轨；global sync --push 同步并推送')
      }
      return { kind: 'success', text: lines.join('\n') }
    }
