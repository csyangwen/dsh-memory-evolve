...
import { asyncWithLock, isMemoryFile, readTreeFiles, runGit } from './repo.js'
...
export async function runSync({ dir, remoteBranch, push = false }) {
  ...
  const merged = await asyncWithLock(dir, async () => {
    ...
    // 提交：git add -A → write-tree → commit-tree（双父）→ update-ref
    // index 永不 unmerged（我们从不 git merge）；无变化时跳过提交
