...
import { isProjectSyncEnabled, parseEntries, serializeEntries, readProvenance } from '../store.js'
import { genEntryId, TODO_ID_RE } from './entryid.js'
import { mergeEntries } from './merge.js'
import { asyncWithLock, isMemoryFile, parseTodoEntries, readTreeFiles, runGit, serializeTodoEntries, stagePaths, TODO_FILE } from './repo.js'
...
export async function runSync({ dir, remoteBranch, push = false }) {
  ...
  // 远端尚无此分支 = **首次推送场景**（bootstrap 后第一次 sync --push）：
  // 远端视为空（theirs 空、无 merge-base、单父提交），本地合并后由阶段 3
  // 的 push 创建远端分支。绝不在这种合法场景报错退出。
  ...
  const merged = await asyncWithLock(dir, async () => {
    ...
    // 写回（todo.js 解析器依赖 TODO_HEADER）
    for (const [path, entries] of Object.entries(result.files)) {
      const abs = join(dir, path)
      mkdirSync(join(dir, dirname2(path)), { recursive: true })
      const tmp = `${abs}.tmp.${process.pid}`
      writeFileSync(tmp, path === TODO_FILE ? serializeTodoEntries(entries) : serializeEntries(entries))
      renameSync(tmp, abs)
    }
...
function readWorkingMemoryFiles(dir) {
  ...
  if (existsSync(todoPath)) files[TODO_FILE] = parseTodoEntries(readFileSync(todoPath, 'utf8'))
...
function cleanRefLocks(dir, remoteBranch) {
  ...
  const targets = [join(originDir, `${remoteBranch}.lock`), join(dir, '.git', 'packed-refs.lock')]
...
export async function resolveConflict({ dir, index, choice }) {
  ...
  return asyncWithLock(dir, async () => {
    ...
    // TODO 文件用专用解析/序列化（header 保留，todo 解析器依赖）
    ...
    const tmp = `${abs}.tmp.${process.pid}`
    writeFileSync(tmp, isTodo ? serializeTodoEntries(entries) : serializeEntries(entries))
    renameSync(tmp, abs)
 ...
 *  记忆条目：行首 [id:xxxx]；TODO 条目：tag 行内 [id: xxxx]（带空格）——
 *  两种形态都要换（同 id 不能并存，联合索引唯一性约束）。 */
function stripIdToNew(entry) {
  if (entry === null) return null
  const todoM = TODO_ID_RE.exec(entry)
  if (todoM !== null) return entry.replace(TODO_ID_RE, `[id: ${genEntryId()}]`)
