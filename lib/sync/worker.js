...
import { asyncWithLock, extractTodoHeader, isMemoryFile, parseTodoEntries, readTreeFiles, runGit, serializeTodoEntries, stagePaths, TODO_FILE } from './repo.js'
...
export async function runSync({ dir, remoteBranch, push = false }) {
  ...
  const merged = await asyncWithLock(dir, async () => {
    ...
    const headNow = await runGit(dir, ['rev-parse', '--verify', 'HEAD'])
    ...
    const stageResult = await stagePaths(dir)
    ...
    if (staged.ok) {
      ...
      const isAncestor = await runGit(dir, ['merge-base', '--is-ancestor', rb, 'HEAD'])
      ...
      const commit = await runGit(dir, ['commit-tree', '-p', 'HEAD', '-p', rb, '-m', `memory: sync ${stamp} [merge]`, tree.stdout.trim()])
      if (!commit.ok) return { ...result, committed: false, commitError: 'commit-tree 失败' }
      const upd = await runGit(dir, ['update-ref', 'refs/heads/main', commit.stdout.trim()])
...
/** 工作树记忆文件读取（锁内调用）。TODO 文件用专用解析（剥 header）。
...
function readWorkingMemoryFiles(dir) {
  ...
  for (const name of ['KEY.md', 'KEY-archive.md', 'MEMORY.md']) {
    ...
    if (existsSync(p)) readMemory(p, name)
  }
  const todoPath = join(dir, TODO_FILE)
  if (existsSync(todoPath)) readMemory(todoPath, TODO_FILE)
  const logsDir = join(dir, 'logs')
  if (existsSync(logsDir) && readdirSync(logsDir).length > 0) {
    for (const name of readdirSync(logsDir)) {
      if (name.endsWith('.md')) readMemory(join(logsDir, name), `logs/${name}`)
    }
  }
...
export async function resolveConflict({ dir, index, choice }) {
  ...
  return asyncWithLock(dir, async () => {
    ...
    const isTodo = target.file === TODO_FILE
