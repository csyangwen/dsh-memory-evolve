...
import { genEntryId, TODO_ID_RE } from './entryid.js'
import { mergeEntries } from './merge.js'
import { asyncWithLock, extractTodoHeader, isMemoryFile, parseTodoEntries, readTreeFiles, resolveFilesetFiles, runGit, serializeTodoEntries, stagePaths } from './repo.js'
 ...
 */
export async function runSync({ dir, remoteBranch, push = false, fileset = 'project', localBranch = 'main' }) {
  ...
  if (remoteMissing) {
    ...
    if (staleTracking.ok) {
      ...
      await runGit(dir, ['update-ref', '-d', rb])
      return { ok: false, code: 1, message: `远端分支 ${remoteBranch} 已不存在（本地曾有陈旧跟踪，已清理）。远端记忆可能被删除或迁移——请检查远端后重新 /memory_sync setup`, committed: false, conflicts: 0, stats: {} }
    }
  ...
  const merged = await asyncWithLock(dir, async () => {
    ...
    // 提交：allowlist stage（审查 P1-12：白名单外文件绝不入库；Grok P1-1
    // 对账：已删除的跟踪文件 git rm）→ write-tree → commit-tree（双父）→
    // update-ref。index 永不 unmerged（我们从不 git merge）。**每步检查
    // 结果（Codex P0-4）：git 写入失败绝不得虚报"已提交并推送"。**
    // **先重建 index（全局轨多轨共享同一 .git）：`rm --cached` 清掉 index
    // 里其他轨的路径（工作树不动），再 stage 本 fileset——否则 write-tree
    // 会把其他轨的文件带进本轨提交树（todo 分支混入 MEMORY.md 等）。**
    const resetIndex = await runGit(dir, ['rm', '-r', '-q', '-f', '--cached', '--ignore-unmatch', '.'])
    if (!resetIndex.ok) return { ...result, committed: false, commitError: `index 重建失败：${resetIndex.stderr.trim().split('\n')[0] ?? ''}` }
    const stageResult = await stagePaths(dir, fileset)
    if (!stageResult.ok) return { ...result, committed: false, commitError: `stage 失败：${stageResult.stderr.trim().split('\n')[0] ?? ''}` }
    const staged = await runGit(dir, ['diff', '--cached', '--quiet'])
    if (staged.ok) {
      // 无内容变化（Codex P1-3）：远端是 HEAD 祖先 → 已收敛，跳过提交；
      // 树相同但历史分叉（对端同树提交）→ 必须建双父提交收敛历史，
      // 否则 ahead/behind 死锁、push 永远 non-fast-forward
      ...
      const commit = await runGit(dir, ['commit-tree', '-p', lb, '-p', rb, '-m', `memory: sync ${stamp} [merge]`, tree.stdout.trim()])
      ...
      if (!upd.ok) return { ...result, committed: false, commitError: 'update-ref 失败' }
 ...
 * 专用解析（剥 header）。
 * 返回 { files, invalid }——invalid 为格式预检失败清单（Codex P0-2：
 * CRLF/手工编辑的文件 parse→serialize 不能往返，绝不能被重写破坏）。 */
...
/**
 * PROVENANCE 校验（施工图 §9 → Codex 终审 P0-1 严格化）：本地有合法
 * projectId 且远端分支存在（fetch 成功）时，远端必须 PROVENANCE 存在、
 * JSON 合法、projectId 精确相等——缺失/损坏一律拒绝（防把 A 项目记忆
 * 并进 B 项目；fail-open 已复现可串项目）。首次推送场景（远端分支尚不
 * 存在）远端侧为空 → 放行（本地合法即可）。
 */
function checkProvenance(dir, rb) {
  ...
  // 本地有合法身份 + 远端分支可读 → 远端必须匹配（严格，不再 fail-open）
  if (local && typeof local.projectId === 'string' && remoteReadable) {
    if (remote === null || typeof remote.projectId !== 'string' || remote.projectId !== local.projectId) {
 ...
 */
export async function resolveConflict({ dir, index, choice, fileset = 'project', localBranch = 'main' }) {
  ...
  // 侧车路径白名单校验（Grok P2-5）：CONFLICTS.md 的 file 字段只能指向
  // 同步记忆文件——防手工/异常侧车写穿到任意路径
  if (!isMemoryFile(target.file)) {
  ...
  return asyncWithLock(dir, async () => {
    ...
    const entries = existsSync(abs)
      ? (isTodo ? parseTodoEntries(readFileSync(abs, 'utf8')) : parseEntries(readFileSync(abs, 'utf8')))
      : []
    if (choice === 'both') {
      // 两个版本都要：ours 原样 + theirs 换新随机 ID（同 ID 不并存）。
      // isTodo 显式传参（Codex P1-9）：TODO 换 tag 行内 id，记忆换行首 id，
      // 不再按正文内容猜格式（正文里出现 [id: xxxx] 字样也不会误判）
      const theirsWithNewId = stripIdToNew(target.theirs, isTodo)
    ...
    // ── 2. 从 CONFLICTS.md 移除该条（清空则删文件；原子写，Kimi P2-3）──
    ...
    // **每步检查（Codex P0-4 / Kimi P1-10）：git 写入失败必须如实报错，
    // 绝不假报"已解决并提交"**；先重建 index（多轨共享同一 .git 时防止
    // 把其他轨的文件带进提交树）
    const resetIndex = await runGit(dir, ['rm', '-r', '-q', '-f', '--cached', '--ignore-unmatch', '.'])
    if (!resetIndex.ok) {
      return { ok: false, message: `解决冲突时 index 重建失败：${resetIndex.stderr.trim().split('\n')[0] ?? ''}` }
    }
    const stageResult = await stagePaths(dir, fileset)
    if (!stageResult.ok) {
      return { ok: false, message: `解决冲突时 stage 失败：${stageResult.stderr.trim().split('\n')[0] ?? ''}。冲突条目已写回但未提交，请重试` }
    }
    const staged = await runGit(dir, ['diff', '--cached', '--quiet'])
    if (!staged.ok) {
      const tree = await runGit(dir, ['write-tree'])
      if (!tree.ok) return { ok: false, message: `解决冲突时 write-tree 失败：${tree.stderr.trim().split('\n')[0] ?? ''}` }
      const lb = `refs/heads/${localBranch}`
      const headRef = await runGit(dir, ['rev-parse', '--verify', lb])
