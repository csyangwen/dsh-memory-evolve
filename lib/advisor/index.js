...
export function installAdvisor(ctx, config, deps = {}) {
  ...
  // 2026-08-13 用户反馈：全局 advisorEnabled 是模块**总闸**——全局关闭时
  // 会话级 override 不越权（一律 false），关闭即整体停评；全局开启后
  // override 才恢复会话级粒度（override ?? 全局默认开启）。
  const effectiveEnabled = (sessionId) => (
    config.advisorEnabled === true
      && (overrides.has(sessionId) ? overrides.get(sessionId) : true)
  )
