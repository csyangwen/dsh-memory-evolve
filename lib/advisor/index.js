 ...
 * - HTTP API（installAdvisorApi，ctx.inject(['httpServer']) 条件注册）；
...
export function installAdvisor(ctx, config, deps = {}) {
  ...
  // B3：effectiveEnabled 只表达"开关结果"（override ?? 全局默认）；
  // 模型可运行性由 gateStatus 单独表达（配置不完整时开关仍可翻转）
  const effectiveEnabled = (sessionId) => (overrides.has(sessionId) ? overrides.get(sessionId) : config.advisorEnabled === true)
  ...
  const ctrl = {
    ...
    resetConversation(sessionId) {
      ...
      if (result !== null) logger.info('advisor: conversation reset', { sessionId, epoch: result.epoch })
      return result
    },
