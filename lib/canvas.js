...
/** 便签单条内容上限（与 scratch 512KiB 同量级但更小，画板是轻量陈列）。 */
...
export function aiSlotPlacement(nodes) {
  ...
  return {
    x: CANVAS_AI_ZONE.x + 20 + col * 270,
    y: CANVAS_AI_ZONE.y + 36 + row * 120,
    width: 260,
    height: 100,
    zIndex: 1,
  }
    ...
    const cancel = webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // 真实本地搜索：GET /search?q=&dir=&limit=（复用 search-docs provider / walk 兜底）
