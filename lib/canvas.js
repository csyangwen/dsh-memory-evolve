...
export function openNodeFile(config, nodeId) {
  ...
  try {
    if (process.platform === 'darwin') {
      spawn('open', [real], { detached: true, stdio: 'ignore' }).unref()
    } else if (process.platform === 'win32') {
      spawn('cmd', ['/c', 'start', '', real], { detached: true, stdio: 'ignore' }).unref()
    } else {
      spawn('xdg-open', [real], { detached: true, stdio: 'ignore' }).unref()
    }
    return { ok: true }
  } catch (error) {
