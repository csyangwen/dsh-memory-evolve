  ...
  return /* @__PURE__ */ (0, import_jsx_runtime18.jsxs)("div", { className: "bm-panel", children: [
    ...
    feature === "list" && /* @__PURE__ */ (0, import_jsx_runtime18.jsxs)(import_jsx_runtime18.Fragment, { children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime18.jsxs)("div", { className: "bm-list", children: [
        bookmarks === null && /* @__PURE__ */ (0, import_jsx_runtime18.jsx)("div", { className: "bm-empty", children: t2("bookmark.loading") }),
        bookmarks !== null && bookmarks.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime18.jsx)("div", { className: "bm-empty", children: t2("bookmark.empty") }),
        bookmarks !== null && bookmarks.map((bm) => /* @__PURE__ */ (0, import_jsx_runtime18.jsxs)(
          "div",
          {
            className: "bm-item",
            role: "article",
...
function selectTurnBookmark(owner) {
  const { nodes, seq } = owner;
  let turn = null;
  const closing = nodes.find((n) => n.kind === "assistant" && n.seq === seq);
  if (closing && closing.kind === "assistant" && typeof closing.turn === "number") {
    turn = closing.turn;
  }
  let summary = "";
  const closingIndex = nodes.findIndex((n) => n.seq === seq);
  const searchEnd = closingIndex >= 0 ? closingIndex : nodes.length;
  for (let i = searchEnd - 1; i >= 0; i -= 1) {
    const node = nodes[i];
    if (node === void 0) continue;
    if (node.kind === "user") {
      summary = extractTextPreview(node.content);
      break;
    }
    if (node.kind === "assistant" && turn !== null && typeof node.turn === "number" && node.turn < turn) {
      break;
    }
  }
  return { seq, turn, summary };
}
...
function TurnBookmarkButton(props) {
  const { matched, sessionId, t: t2 } = props;
  const { seq, turn, summary } = matched;
...
function apply(ctx) {
  ...
  void fetch("/memory-evolve/api/bookmarks/state").then((res) => res.ok ? res.json() : Promise.reject(new Error(`HTTP ${res.status}`))).then((data) => {
    ...
    disposeTurnStar = ctx.slots.inject("conversation.chat.turnTail", () => ctx.slots.register({
