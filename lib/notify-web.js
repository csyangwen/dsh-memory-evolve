...
export class NotificationStore {
   ...
   * @param {Record<string,string>} aliases - 会话别名字典。
   */
  list(type, aliases) {
    ...
    return wanted.map((m) => ({
      id: m.id,
      sender: m.sender ?? '',
      // 发送方显示名：别名优先 → 系统 → 短 ID（完整 ID 由前端悬停展示）。
      senderName: m.sender
        ? (aliases?.[m.sender] ?? String(m.sender).slice(0, 8))
        : 'system',
 ...
 * @param {object} deps - { store: NotificationStore, memoryDir: string }。
 * @returns {() => void} 卸载句柄。
 */
export function installNotifyWebApi(ctx, deps) {
  const { store, memoryDir } = deps
  let cancel = null
    ...
    cancel = webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // 列表（unread/all）；sender 名称用别名字典映射。
          if (req.method === 'GET' && path === `${base}/list`) {
            ...
            return sendJson(res, 200, { items: store.list(type, aliases) })
          }
          // 通知原文（含长内容文件）。
