...
export class SessionOrch {
   ...
   * 导致 cwd 正确但新会话仍显示在「未分组」。失败不影响创建（仅分组）。
   */
  async #attachToWorkspace(sessionId, cwd) {
    if (!cwd || !this.workspace) return
    try {
      const ws = await this.workspace.resolveByPath(cwd)
      if (ws !== undefined) await ws.attachSession(sessionId)
    } catch (error) {
      console.warn(`[dsh-memory-evolve] 会话 ${sessionId} 挂接工作区失败（忽略，仅影响左侧分组）: ${errText(error)}`)
    }
  }
   ...
   * @returns {{ok:boolean, sessionId?:string, provider?:string|null, message?:string}}
   */
  async spawn({ prompt, cwd, roomId, model, provider, by, requester } = {}) {
    ...
    // cwd 正确但会话显示在「未分组」）
    await this.#attachToWorkspace(sessionId, resolvedCwd)
    ...
    return {
      ...
      sessionId,
      provider: resolvedProvider ?? null,
      message: `已创建会话 ${sessionId} 并开始执行任务${roomNote}${providerNote}`,
    }
  }
