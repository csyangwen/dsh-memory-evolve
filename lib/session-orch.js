...
export class SessionOrchStore {
  constructor(dir) {
    ...
    /** @type {Array<{sessionId:string, spawnedBy:string, prompt:string, cwd:string|null, provider:string|null, model:string|null, roomId:string|null, createdAt:number}>} */
    this.records = []
    this.#load()
  }
...
export class SessionOrch {
  ...
  constructor(ctx, deps) {
    ...
    this.workspace = ctx.workspace // 工作区注册表（左侧会话列表"项目"分组依据）
  ...
  me(requester) {
    ...
    let title = null
    try {
      title = this.sessionTitle?.get?.(agent.session)?.title ?? null
    } catch { /* 标题读取失败置 null */ }
   ...
   * @returns {Promise<{ok:boolean, workspaceId?:string|null, workspaceTitle?:string|null, error?:string, attempts:number}>}
   */
  ...
  /**
   * 启动自愈：历史 spawn 记录里不在任何 workspace 的会话补挂接。
   * ⚠️ 2026-08-12 踩坑：spawn 时 attach 失败曾被子静默吞掉（workspace 不会
   * 自动补录），遗留会话永久未分组。只处理**本模块 spawn 记录**的会话
   * （不碰 GUI 会话），失败只 warn 不影响启动。
   * @returns {Promise<{healed:number, failed:number}>}
   */
  async healSpawnedAttachments() {
    const healed = { healed: 0, failed: 0 }
    try {
      const recs = this.store.list()
      if (recs.length === 0) return healed
      const wsList = this.workspace?.list?.() ?? []
      const accounted = new Set(wsList.flatMap((w) => Array.from(w.sessionIds ?? [])))
      for (const rec of recs) {
        // 已在某分组或记录无 cwd（无分组概念）→ 跳过
        if (!rec.cwd || accounted.has(rec.sessionId)) continue
        const r = await this.attachWorkspace(rec.sessionId, rec.cwd)
        if (r.ok) healed.healed += 1
        else {
          healed.failed += 1
          console.warn(`[dsh-memory-evolve] 自愈挂接会话 ${rec.sessionId} 失败（跳过）: ${r.error}`)
        }
      }
    } catch (error) {
      console.warn(`[dsh-memory-evolve] 自愈挂接失败（整体跳过）: ${errText(error)}`)
    }
    return healed
  }
   ...
   * @param {{prompt:string, cwd?:string, roomId?:string, model?:string, provider?:string, by?:string, requester?:object}} input
   ...
   * @returns {{ok:boolean, sessionId?:string, provider?:string|null, attach?:object|null, message?:string}}
   *   attach：挂接工作区结果 { ok, workspaceId?, workspaceTitle?, error?, attempts }
   *   （cwd 为 null 时不挂接 → attach=null；失败不阻断 spawn，message 带警告）
   */
  async spawn({ prompt, cwd, roomId, model, provider, by, requester } = {}) {
    ...
    // 落盘 spawn 记录（list 追溯用；provider/model/cwd 留档=创建时用的配置）
    this.store.add({
      sessionId,
      spawnedBy: by ?? '',
      prompt: text,
    ...
    return {
      ...
      provider: resolvedProvider ?? null,
      attach,
      message: `已创建会话 ${sessionId} 并开始执行任务${roomNote}${providerNote}${attachNote}`,
    }
  }
...
export function sessionToolDefinition(orch) {
  return {
    name: 'de_session',
    description: '会话编排（独立模块，开关见记忆 Tab 运行时配置「会话编排」）：程序化创建/唤醒/标记 DSH 会话——spawn：**新建一个标准会话**（与 GUI 手动打开完全同构：同样的系统提示词/工具/记忆快照/持久化，会出现在左侧会话列表可随时接管；**自动挂到工作目录对应的工作区分组**（挂接失败会自动重试，结果见返回的 attach 字段；失败不阻断创建但要在汇报里说明）），prompt=**完整提示词（自由组合的长文本：角色/任务/要求一次写清，如"你是美工，负责网站视觉…现在开始执行任务：…"）**，创建后立即自动开跑（等价替用户发消息）；**默认继承发起会话的 provider/model 与 cwd 工作目录**；可选 roomId（加入广播房间，需广播模块启用；房间不存在/未启用只提示不阻断）、model/cwd（显式覆盖）。⚠️ **显式传 model 时按模型名自动解析所属 provider**（与 GUI 模型选择框同源：已配置 provider 的模型目录，如 qwen3.7-plus → qwen-token-plan-cn）——曾只换 model 不换 provider，把 qwen3.7-plus 发给 deepseek 接口报 INVALID_REQUEST；解析不到时 provider 沿用发起会话并在返回里提示风险。也可显式传 provider 强制指定路由（如 deepseek-official、qwen-token-plan-cn）。wake：**唤醒已有会话**——sessionId + prompt（要对方做的事），等价替用户给对方发一条消息，对方 AI 自动醒来处理（正在忙则排队；忙完前不要重复派活）；进程重启后的会话自动从持久化恢复再唤醒，**用会话自己的模型配置**；跨实例会话无法唤醒会明确报错。**⚠️ wake 只保证送达不保证成功**：返回"指令已送达"只代表已入队，不代表对方真的跑起来了——离线恢复失败/模型配置缺失时对方回合会失败（如 {{model}} 无值报错）。**派发后等几秒再用 status 确认对方 running**，不要看了返回就假定对方在干活。status：查单个会话（sessionId）状态——running=正在生成 / idle=空闲 / offline=不在本进程，附 lastActiveAt（最后活动时间）。list：列出全部 live 会话（含 GUI 手动开的）与本模块创建过的会话（**每个会话都带名称 title 与别名 alias**，一眼认出"这是谁"；附角色提示词/任务/所属房间/状态/lastActiveAt——offline 会话 title=null，别名不受限）。rename：**修改会话名称与/或别名做标记**——sessionId + title（新名称，左侧列表标题，需会话 live）+ alias（新别名 ≤10 字，广播/快照显示，空串=清除），两者至少给一个，可同时改。me：**查当前会话自身信息**（"我是谁"）——session ID / 会话名称 / 别名 / 所属项目分组 / cwd / git 分支（非 git 目录为 null）/ provider+model / 创建时间 / 父会话 / 创建者（spawn 记录）/ 状态，AI 确认自己身份或把 ID/别名告知他人协作时用它（快照「你的会话」段是常驻参考，工具查询更即时准确）。**员工编排纪律（重要）：本工具不会自动唤醒任何会话——必须由你（拍板人）有意识地 list/status 查状态、发现员工 idle/offline 后用 wake 主动唤醒，不要自作主张批量唤醒（会造成管理混乱）。**',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['spawn', 'wake', 'status', 'list', 'rename', 'me'], description: 'spawn=新建会话；wake=唤醒已有会话；status=查单个会话状态；list=列出会话；rename=改会话名称/别名；me=查当前会话自身信息（ID/名称/别名/cwd/模型）' },
        ...
        properties: {
            ...
            oneOf: [
              {
                ...
                properties: {
                  ...
                  attempts: { type: 'integer', description: '实际尝试次数（含重试）' },
                },
                required: ['ok'],
              },
              { type: 'null' },
          ...
          sessions: {
            ...
            items: {
              ...
              required: ['sessionId', 'status', 'cwd', 'spawnedBy', 'prompt', 'roomId', 'provider', 'model', 'createdAt', 'lastActiveAt', 'title', 'alias'],
            },
          },
          live: {
            type: 'array',
      ...
      render: (_args, value) => {
        ...
        if (Array.isArray(value.sessions)) {
          const lines = value.sessions.map((s) => {
            ...
            return `${mark} ${s.sessionId} ${s.status}${route}${nameBits ? ` ${nameBits}` : ''}${s.spawnedBy ? ` · 创建者 ${s.spawnedBy}` : ''} · 最后活动 ${when}\n    ${promptHead}`
          })
      ...
      try {
        switch (action) {
          case 'spawn':
            ...
            return await orch.spawn({ prompt: args.prompt, cwd: args.cwd, roomId: args.roomId, model: args.model, provider: args.provider, by, requester: ctx?.agent })
          case 'wake':
...
export function installSession(ctx, config, deps) {
  ...
  try {
    ...
    // ⚠️ 启动自愈（2026-08-12 踩坑修复）：延迟到服务稳定后，对历史 spawn
    // 记录里不在任何 workspace 的会话补挂接（曾因 attach 失败被静默吞掉
    // 导致会话"创建后不在分组、重启后永久未分组"）。失败只 warn 不阻塞；
    // 卸载时清除定时器。延迟可用 config.healDelayMs 覆盖（测试用）。
    const healDelayMs = typeof config.healDelayMs === 'number' ? config.healDelayMs : 1500
    const healTimer = setTimeout(() => {
      instance.healSpawnedAttachments().catch((error) => {
        console.warn(`[dsh-memory-evolve] 自愈挂接异常（忽略）: ${errText(error)}`)
      })
    }, healDelayMs)
    disposers.push(() => clearTimeout(healTimer))
  } catch (error) {
  ...
  return {
    ...
    orch: () => orch,
    store,
    /** 手动触发自愈（测试用；installSession 已内置延迟自动执行）。 */
    heal: () => orch?.healSpawnedAttachments() ?? Promise.resolve({ healed: 0, failed: 0 }),
    dispose() {
