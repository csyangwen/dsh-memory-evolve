...
export function sessionToolDefinition(orch) {
  return {
    name: 'de_session',
    description: '会话编排（独立模块，开关见记忆 Tab 运行时配置「会话编排」）：程序化创建/唤醒/标记 DSH 会话——spawn：**新建一个标准会话**（与 GUI 手动打开完全同构：同样的系统提示词/工具/记忆快照/持久化，会出现在左侧会话列表可随时接管；自动挂到工作目录对应的工作区分组），prompt=**完整提示词（自由组合的长文本：角色/任务/要求一次写清，如"你是美工，负责网站视觉…现在开始执行任务：…"）**，创建后立即自动开跑（等价替用户发消息）；**默认继承发起会话的 provider/model 与 cwd 工作目录**；可选 roomId（加入广播房间，需广播模块启用；房间不存在/未启用只提示不阻断）、model/cwd（显式覆盖）。wake：**唤醒已有会话**——sessionId + prompt（要对方做的事），等价替用户给对方发一条消息，对方 AI 自动醒来处理（正在忙则排队；忙完前不要重复派活）；进程重启后的会话自动从持久化恢复再唤醒，**用会话自己的模型配置**；跨实例会话无法唤醒会明确报错。status：查单个会话（sessionId）状态——running=正在生成 / idle=空闲 / offline=不在本进程，附 lastActiveAt（最后活动时间）。list：列出全部 live 会话（含 GUI 手动开的）与本模块创建过的会话（角色提示词/任务/所属房间/状态/lastActiveAt）。rename：**修改会话名称与/或别名做标记**——sessionId + title（新名称，左侧列表标题，需会话 live）+ alias（新别名 ≤10 字，广播/快照显示，空串=清除），两者至少给一个，可同时改。me：**查当前会话自身信息**（"我是谁"）——session ID / 会话名称 / 别名 / 所属项目分组 / cwd / git 分支（非 git 目录为 null）/ provider+model / 创建时间 / 父会话 / 创建者（spawn 记录）/ 状态，AI 确认自己身份或把 ID/别名告知他人协作时用它（快照「你的会话」段是常驻参考，工具查询更即时准确）。**员工编排纪律（重要）：本工具不会自动唤醒任何会话——必须由你（拍板人）有意识地 list/status 查状态、发现员工 idle/offline 后用 wake 主动唤醒，不要自作主张批量唤醒（会造成管理混乱）。**',
    parameters: {
      type: 'object',
      properties: {
