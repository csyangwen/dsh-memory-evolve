        ...
        properties: {
          ...
          sessions: {
            ...
            items: {
              ...
              required: ['sessionId', 'status', 'cwd', 'spawnedBy', 'prompt', 'roomId', 'provider', 'model', 'createdAt', 'lastActiveAt'],
            },
          },
          live: {
            type: 'array',
      ...
      render: (_args, value) => {
        ...
        if (Array.isArray(value.sessions)) {
          const lines = value.sessions.map((s) => {
            ...
            return `${mark} ${s.sessionId} ${s.status}${route}${s.spawnedBy ? ` · 创建者 ${s.spawnedBy}` : ''} · 最后活动 ${when}\n    ${promptHead}`
          })
