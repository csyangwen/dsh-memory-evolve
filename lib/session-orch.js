...
export class SessionOrch {
  ...
  async spawn({ prompt, cwd, roomId, model, by, requester } = {}) {
    ...
    // ⚠️ 仅当模型一致时继承：显式覆盖了不同 model 时**不带** reasoningEffort
    // （目标模型可能不支持思考等级，避免报错）。
    const requesterHeader = requester?.session?.requestHeader?.()
    let seed
    if (requesterHeader?.config?.provider && requesterHeader.config.model) {
      const sameModel = resolvedModel === requesterHeader.config.model
      // 同模型：完整继承（含 reasoningEffort 等运行配置）；
      // 换模型：header 用新模型的 provider/model、**不带** reasoningEffort
      // （目标模型可能不支持思考等级），也不带 adapterDefaults 标记
      const config = sameModel
        ? { ...requesterHeader.config }
        : { provider: base.provider ?? requesterHeader.config.provider, model: resolvedModel }
      seed = [{
        type: 'request/header',
        ...
        data: {
          header: {
            ...(sameModel && requesterHeader.adapterDefaults ? { adapterDefaults: requesterHeader.adapterDefaults } : {}),
            config,
          },
          reason: 'initial',
        },
