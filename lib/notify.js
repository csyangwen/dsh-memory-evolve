/**
 * 渠道通知模块（de_notify + de_feishu_send）——**独立子模块**（用户拍板纪律：
 ...
 * ② de_feishu_send（独立开关 feishuSendEnabled，默认开）：AI **主动发送**
 ...
 * 列表，需要时再开）；feishuSendEnabled（默认**开**，2026-08-10 用户拍板
 * 要的功能，开箱即用——de_feishu_send 与 de_notify 语义不同：直发 vs 通知，
 * 开关粒度独立，互不影响）。
 *
...
/**
 * 发送内核（de_feishu_send）：飞书**直发**——主动发送文本/图片/文件，
 * 固定 feishu 渠道，统一走 entry.sendMedia 槽位（不带「非对话」通知标注，
 * 语义=直接把内容发给用户，与 de_notify 的通知语义区分）。
 * @param {object} args - { content?, attachments?, target? }。
 ...
 */
export async function sendFeishuDirect(args) {
  const entry = getRegistry().feishu
  const results = []
  const fail = (error) => {
    results.push({ channel: 'feishu', ok: false, error, messageId: '', target: '' })
  }
  // 渠道未注册 / 无 sendMedia 槽位（旧版 dsh-feishu）：如实报错
  if (!entry) {
    fail('渠道未注册：飞书插件未安装，或插件版本不含通知钩子')
    return { results, summary: `飞书直发：${0}/${results.length} 个发送成功` }
  }
  if (typeof entry.sendMedia !== 'function') {
    fail('飞书插件版本不支持主动发送（需升级 dsh-feishu 到支持 sendMedia 的版本）')
    return { results, summary: `飞书直发：${0}/${results.length} 个发送成功` }
  }
  const target = resolveTarget(entry, args.target)
  if (!target) {
    fail('无发送目标：飞书没有最近交互的对话（可传 target 显式指定，如 p2p:oc_xxx）')
    return { results, summary: `飞书直发：${0}/${results.length} 个发送成功` }
  }
  const targetLabel = `${target.kind}:${target.id}`
  ...
  if (!content && attachments.length === 0) {
    fail('至少提供 content（文本）或 attachments（附件）之一')
    return { results, summary: `飞书直发：${0}/${results.length} 个发送成功` }
  }
  // 组合发送序列：
  //  - 无附件：content 单独作为文本消息；
  //  - 有附件：content 并入第一条附件的 caption（已有 caption 则换行拼接），
  //    不单独发文本——飞书里一条图文/文件消息即「说明 + 附件」，体验一致。
  ...
  for (const media of sends) {
    results.push(await sendOne('feishu', target, targetLabel, () => entry.sendMedia(target, media, {})))
  }
  const okCount = results.filter((r) => r.ok).length
  return { results, summary: `飞书直发：${okCount}/${results.length} 个发送成功` }
}
...
/**
 * 附件参数 schema（de_notify / de_feishu_send 共用）。
...
/**
 * de_feishu_send 工具定义（2026-08-10 用户拍板新增：DSH→飞书单向
 * 主动发送文本/图片/文件）。独立开关 feishuSendEnabled（默认开）控制，
 * 与 de_notify 语义区分：直发不带「非对话」通知标注。
 */
export function feishuSendToolDefinition(send) {
  return {
    name: 'de_feishu_send',
    description: '飞书直发：**主动发送**文本/图片/文件到飞书（DSH→飞书单向，不带「这是通知」标注，语义=直接把内容发给你）。content=文本正文；attachments=附件列表（图片或文件，来源三选一：path=本地文件绝对路径 / url=远程地址自动下载 / base64=内联内容必须配 fileName；可带 caption 说明文字；**有 content + 附件时 content 自动作为第一条附件的说明**，不单独发文本）；target 可选：缺省=最近交互的飞书对话，也可显式 p2p:oc_xxx。飞书限制：图片≤10MB、文件≤30MB；渠道插件（dsh-feishu）未装/版本不支持时会如实报错。适合「把生成的图片/文档/报告发给我」场景；与 de_notify 的区别：de_notify 是通知（带标注、多渠道、需开启通知开关），本工具是直发（不带标注、仅飞书、默认开启）。',
    parameters: {
      type: 'object',
      properties: {
        content: { type: 'string', description: '文本正文（无附件时单独发一条文本；有附件时自动作为第一条附件的说明文字）' },
        attachments: ATTACHMENTS_SCHEMA,
        target: { type: 'string', description: '可选：缺省=最近交互的飞书对话；显式传 p2p:oc_xxx 或 group:oc_xxx' },
      },
      required: [],
    },
    output: {
    ...
    // 直接转发直发内核（sendFeishuDirect 永不抛错、逐条回执）
    execute: (args) => send(args),
  }
}

/** 渠道发送结果 output schema（de_notify / de_feishu_send 共用，与返回严格一致）。 */
...
/**
 * 渠道发送结果 render（de_notify / de_feishu_send 共用）：时间锚点（调用
...
/**
 * 安装飞书直发模块（feishuSendEnabled 开关控制，独立装配——与 de_notify
 * 语义不同：直发 vs 通知，开关粒度独立，2026-08-10 用户拍板）。
 * @param {object} ctx - 插件上下文（tools 已注入）。
 * @returns {{ dispose: Function, sendFeishuDirect: Function }}
 */
export function installFeishuSend(ctx) {
  const dispose = ctx.effect(() => {
    const tool = feishuSendToolDefinition(sendFeishuDirect)
    return ctx.tools.register(tool)
  }, 'dsh-memory-evolve: de_feishu_send tool')

  return { dispose, sendFeishuDirect }
}
