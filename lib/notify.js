...
export function notifyToolDefinition(send) {
  return {
    name: 'de_notify',
    description: '渠道通知：把消息通过 IM 渠道（一期支持飞书）**主动发给你**，让你在电脑前/手机上立刻知道（任务完成等需要用户知晓的节点用）。channels 选渠道：feishu/qq/weixin/wecom/all（缺省 feishu；**渠道未安装会如实报错**，不会假装成功）；content 为消息正文（建议带上任务/结果概要，如「XX 任务已完成：…」）；target 可选：缺省=该渠道「最近交互的对话」（零配置，最常用），也可显式传 chatKey（如 p2p:oc_xxx）发给指定对话。**随时可发、无频率限制（用户拍板）**；但注意消息是发给真实用户的，只在该发的时候发（任务完成/重要进展/需要用户处理），不要为无关琐事刷屏。返回逐渠道结果（成功/失败原因/消息 id），失败如实呈现。',
    parameters: {
      type: 'object',
      properties: {
