...
export function channelSendToolDefinition(send) {
  return {
    ...
    description: '渠道直发：**主动发送**文本/图片/文件到 IM 渠道（DSH→渠道单向，不带「这是通知」标注，语义=直接把内容发给你）。channels 选渠道：feishu/qq/weixin/wecom（缺省 feishu；all=全部已注册渠道；**渠道插件未装会如实报错**）。content=文本正文；attachments=附件列表（图片或文件，来源五选一：path=本地文件绝对路径 / url=远程地址自动下载 / base64=内联内容必须配 fileName / **sessionImage=true 引用本会话最近一张图（用户在 DSH 输入框粘贴/拖入发送的图，需 260810+ 快照）** / **attachmentId=本会话某张图的引用 id（先查 de_session_images 获取，需 260810+ 快照）**；可带 caption 说明文字；**有 content + 附件时 content 自动作为第一条附件的说明**，不单独发文本）；target 可选：缺省=该渠道最近交互的对话，也可显式传 chatKey（如 p2p:oc_xxx）。各渠道限制：飞书图片≤10MB/文件≤30MB；QQ 本地文件≤10MB（更大用 url 来源）；企微 ≤50MB；微信由平台限制。适合「把生成的图片/文档/报告发给我」以及「把 DSH 输入框里贴的图转给渠道」场景；与 de_notify 的区别：de_notify 是通知（带标注、需开启通知开关），本工具是直发（不带标注、默认开启）。',
    parameters: {
      type: 'object',
      properties: {
        channels: { type: 'string', enum: ['feishu', 'qq', 'weixin', 'wecom', 'all'], description: '发送渠道（缺省 feishu；all=全部已注册渠道；未注册渠道会如实报错）' },
...
export function installNotify(ctx) {
  ...
  // ctx 闭包绑定（发送内核需要 agents/attachments 服务）。
  const dispose = ctx.effect(() => {
    const tool = notifyToolDefinition((args, exec) => sendChannelNotify(args, { ctx, exec }))
...
export function installChannelSend(ctx) {
  ...
  // installNotify 同款：sessionImage/attachmentId 来源需要它；ctx 闭包绑定。
  const dispose = ctx.effect(() => {
    const tool = channelSendToolDefinition((args, exec) => sendChannelDirect(args, { ctx, exec }))
    return ctx.tools.register(tool)
